...
% pdfLaTeX cannot typeset Unicode emoji; these mirror the manuscript legend in tables.
\newcommand{\tmarkDone}{\textcolor{green!55!black}{\faCheck}}
\newcommand{\tmarkNo}{\textcolor{red!75!black}{\faTimes}}
\newcommand{\tmarkPartial}{\textcolor{orange!85!black}{\faHourglassHalf}}
\newcommand{\tmarkPlanned}{\textcolor{blue!75!black}{\faCircle}}
...
% Decision-tree node helpers for Design Decisions figure
...
% ── Figure: Multi-Directional Money Flow Web ──────────────────────────────────
% Comprehensive banking system capital-flow diagram (replaces simple Core System
% Graph).  Shows all five flow types in the Crypto World Bank:
%   (1) Downward capital allocation  WB→NB→LB→Borrower
%   (2) Upward surplus repatriation  LB→NB→WB
%   (3) Same-tier interbank lending  NB↔NB,  LB↔LB
%   (4) Multi-entity distributions   (multiple NBs, multiple LBs per NB)
%   (5) Group / solidarity lending   pool aggregation at Local Bank tier
% Font size 9pt throughout; academic thesis colour palette.
% ──────────────────────────────────────────────────────────────────────────────
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  ...
  >=Stealth,
  %% ── tier node styles ──────────────────────────────────────────────────────
  wb/.style={draw=PrimaryBlue, fill=PrimaryBlue!18, rounded corners=4pt,
             minimum width=2.6cm, minimum height=0.9cm,
             align=center, font=\fontsize{9}{10}\selectfont\bfseries, inner sep=4pt},
  nb/.style={draw=AccentBlue!80, fill=AccentBlue!12, rounded corners=3pt,
             minimum width=2.4cm, minimum height=0.85cm,
             align=center, font=\fontsize{9}{10}\selectfont\bfseries, inner sep=3pt},
  lb/.style={draw=GreenAccent!80, fill=GreenAccent!10, rounded corners=3pt,
             minimum width=2.2cm, minimum height=0.82cm,
             align=center, font=\fontsize{9}{10}\selectfont\bfseries, inner sep=3pt},
  br/.style={draw=AmberAccent!80, fill=AmberAccent!10, rounded corners=3pt,
             minimum width=2.0cm, minimum height=0.80cm,
             align=center, font=\fontsize{9}{10}\selectfont, inner sep=3pt},
  grp/.style={draw=VioletAccent!70, fill=VioletAccent!8, rounded corners=3pt,
              minimum width=2.0cm, minimum height=0.80cm,
              align=center, font=\fontsize{9}{10}\selectfont, inner sep=3pt,
              dashed},
  pool/.style={draw=GreenAccent!60, fill=GreenAccent!6, rounded corners=3pt,
               minimum width=2.2cm, minimum height=0.75cm,
               align=center, font=\fontsize{9}{10}\selectfont\itshape, inner sep=3pt},
  %% ── arrow styles ──────────────────────────────────────────────────────────
  down/.style={->, draw=PrimaryBlue!80, line width=0.9pt},
  up/.style={->, draw=GreenAccent!80, line width=0.8pt, dashed},
  peer/.style={<->, draw=AccentBlue!60, line width=0.8pt,
               dash pattern=on 3pt off 1.5pt},
  repay/.style={->, draw=AmberAccent!70, line width=0.7pt,
                dash pattern=on 2pt off 1pt},
  %% ── label style ───────────────────────────────────────────────────────────
  lbl/.style={font=\fontsize{7.5}{8.5}\selectfont, fill=white,
              inner sep=1.2pt, draw=none},
]

%% ═══════════════════════════════════════════════════════
%% TIER 1 — World Bank Reserve  (top centre)
%% ═══════════════════════════════════════════════════════
\node[wb] (WB) at (6.0, 0)
  {World Bank Reserve\\{\fontsize{7.5}{8.5}\selectfont Tier~1 · 1{,}000{,}000 ETH}\\
   {\fontsize{7.5}{8.5}\selectfont Lends @ 3\% APR}};

%% ═══════════════════════════════════════════════════════
%% TIER 2 — National Banks  (spread horizontally)
%% ═══════════════════════════════════════════════════════
\node[nb] (NB1) at (1.8, -2.6)
  {National Bank A\\{\fontsize{7.5}{8.5}\selectfont 200k ETH · 5\% APR}};
\node[nb] (NB2) at (6.0, -2.6)
  {National Bank B\\{\fontsize{7.5}{8.5}\selectfont 200k ETH · 5\% APR}};
\node[nb] (NB3) at (10.2, -2.6)
  {National Bank C\\{\fontsize{7.5}{8.5}\selectfont 200k ETH · 5\% APR}};

%% ── Downward: WB → NBs ──
\draw[down] (WB.south west) -- node[lbl,left]{capital} (NB1.north);
\draw[down] (WB.south)      -- node[lbl,right]{capital} (NB2.north);
\draw[down] (WB.south east) -- node[lbl,right]{capital} (NB3.north);

%% ── Upward surplus: NBs → WB ──
\draw[up] (NB1.north) ++(0.35,0) -- ++(0, 2.1)
  node[lbl,right,pos=0.5]{surplus} -- (WB.south west) ++(-0.1,0);
\draw[up] (NB3.north) ++(-0.35,0) -- ++(0, 2.1)
  node[lbl,left,pos=0.5]{surplus} -- (WB.south east) ++(0.1,0);

%% ── Same-tier interbank: NB ↔ NB ──
\draw[peer] (NB1.east) -- node[lbl,above]{interbank} (NB2.west);
\draw[peer] (NB2.east) -- node[lbl,above]{interbank} (NB3.west);

%% ═══════════════════════════════════════════════════════
%% TIER 3 — Local Banks  (two per National Bank shown)
%% ═══════════════════════════════════════════════════════
\node[lb] (LB1a) at (0.2, -5.0)
  {Local Bank A1\\{\fontsize{7.5}{8.5}\selectfont 10k ETH · 8\%}};
\node[lb] (LB1b) at (3.2, -5.0)
  {Local Bank A2\\{\fontsize{7.5}{8.5}\selectfont 10k ETH · 8\%}};

\node[lb] (LB2a) at (5.0, -5.0)
  {Local Bank B1\\{\fontsize{7.5}{8.5}\selectfont 10k ETH · 8\%}};
\node[lb] (LB2b) at (7.2, -5.0)
  {Local Bank B2\\{\fontsize{7.5}{8.5}\selectfont 10k ETH · 8\%}};

\node[lb] (LB3a) at (9.0, -5.0)
  {Local Bank C1\\{\fontsize{7.5}{8.5}\selectfont 10k ETH · 8\%}};
\node[lb] (LB3b) at (11.4,-5.0)
  {Local Bank C2\\{\fontsize{7.5}{8.5}\selectfont 10k ETH · 8\%}};

%% ── Downward: NBs → LBs ──
\draw[down] (NB1.south) -- ++(0,-0.35) -| (LB1a.north);
\draw[down] (NB1.south) -- ++(0,-0.35) -| (LB1b.north);
\draw[down] (NB2.south) -- ++(0,-0.35) -| (LB2a.north);
\draw[down] (NB2.south) -- ++(0,-0.35) -| (LB2b.north);
\draw[down] (NB3.south) -- ++(0,-0.35) -| (LB3a.north);
\draw[down] (NB3.south) -- ++(0,-0.35) -| (LB3b.north);

%% ── Upward surplus: selected LBs → NBs ──
\draw[up] (LB1b.north) ++(-0.2,0) -- ++(0,0.7)
  node[lbl,right,pos=0.55]{surplus} -- (NB1.south) ++(0.2,0);
\draw[up] (LB2b.north) ++(-0.2,0) -- ++(0,0.7)
  node[lbl,right,pos=0.55]{surplus} -- (NB2.south) ++(0.2,0);
\draw[up] (LB3a.north) ++(0.2,0) -- ++(0,0.7)
  node[lbl,left,pos=0.55]{surplus} -- (NB3.south) ++(-0.2,0);

%% ── Same-tier interbank: LB ↔ LB (cross-bank example) ──
\draw[peer] (LB1b.east) -- node[lbl,above]{\scriptsize peer} (LB2a.west);
\draw[peer] (LB2b.east) -- node[lbl,above]{\scriptsize peer} (LB3a.west);

%% ═══════════════════════════════════════════════════════
%% TIER 4 — Borrowers + Group Lending Pool
%% ═══════════════════════════════════════════════════════
\node[br] (BR1) at (0.2, -7.4)
  {Borrower\\{\fontsize{7.5}{8.5}\selectfont 0.1--10 ETH}};
\node[br] (BR2) at (3.2, -7.4)
  {SME\\{\fontsize{7.5}{8.5}\selectfont 1--100 ETH}};
\node[br] (BR3) at (6.0, -7.4)
  {Borrower\\{\fontsize{7.5}{8.5}\selectfont 0.1--10 ETH}};
\node[br] (BR4) at (8.6, -7.4)
  {Corporate\\{\fontsize{7.5}{8.5}\selectfont 50--500 ETH}};

\node[grp] (GRP) at (11.4,-7.4)
  {Group Loan\\{\fontsize{7.5}{8.5}\selectfont 3--20 members}\\
   {\fontsize{7.5}{8.5}\selectfont mutual liability}};

%% ── Downward loans: LBs → Borrowers ──
\draw[down] (LB1a.south) -- node[lbl,left]{loan} (BR1.north);
\draw[down] (LB1b.south) -- node[lbl,right]{loan} (BR2.north);
\draw[down] (LB2a.south) -- ++(0,-0.4) -| node[lbl,above,pos=0.3]{loan} (BR3.north);
\draw[down] (LB2b.south) -- ++(0,-0.4) -| node[lbl,above,pos=0.7]{loan} (BR4.north);
\draw[down] (LB3b.south) -- node[lbl,right]{group disbursement} (GRP.north);

%% ── Repayments: Borrowers → LBs (orange dashed) ──
\draw[repay] (BR1.north) ++(0.3,0) -- ++(0,1.6)
  node[lbl,right,pos=0.5]{repay+int.} -- (LB1a.south) ++(0.2,0);
\draw[repay] (BR2.north) ++(-0.3,0) -- ++(0,1.6)
  node[lbl,left,pos=0.5]{repay+int.} -- (LB1b.south) ++(-0.2,0);
\draw[repay] (BR3.north) ++(0.2,0) -- ++(0,1.0)
  -| (LB2a.south) ++(-0.2,0);
\draw[repay] (GRP.north) ++(-0.2,0) -- ++(0,1.6)
  node[lbl,left,pos=0.5]{repay+int.} -- (LB3b.south) ++(-0.2,0);

%% ── Cross-NB institutional: corporate borrower → NB2 direct ──
\draw[down, color=AccentBlue!70] (NB2.south) ++(0.4,0) -- ++(0,-0.4)
  -| node[lbl,right,pos=0.2]{institutional} (BR4.north) ++(0.2,0);

%% ═══════════════════════════════════════════════════════
%% Group Lending Pool node
%% ═══════════════════════════════════════════════════════
\node[pool] (POOL) at (11.4,-9.0)
  {Shared Collateral\\Pool};
\draw[->, draw=VioletAccent!70, line width=0.7pt, dashed]
  (GRP.south) -- node[lbl,right]{enforce} (POOL.north);

%% ═══════════════════════════════════════════════════════
%% LEGEND  (bottom-left)
%% ═══════════════════════════════════════════════════════
\begin{scope}[shift={(0.0,-9.8)}, font=\fontsize{7.5}{8.5}\selectfont]
  \draw[down]  (0,0)   -- ++(1.0,0) node[right]{Downward capital allocation};
  \draw[up]    (0,-0.5)-- ++(1.0,0) node[right]{Upward surplus repatriation};
  \draw[peer]  (0,-1.0)-- ++(1.0,0) node[right]{Same-tier interbank lending};
  \draw[repay] (0,-1.5)-- ++(1.0,0) node[right]{Repayment with interest};
  \draw[->, draw=VioletAccent!70, line width=0.7pt, dashed]
               (0,-2.0)-- ++(1.0,0) node[right]{Group mutual-liability enforcement};
\end{scope}

\end{tikzpicture}
\caption{Multi-directional capital flow web of the Crypto World Bank: (1)~downward capital allocation across four tiers (World Bank Reserve $\to$ National Banks $\to$ Local Banks $\to$ Borrowers); (2)~upward surplus repatriation (excess reserves flow back up the hierarchy); (3)~same-tier interbank lending between National Banks and between Local Banks; (4)~multi-entity distribution (multiple National Banks and Local Banks per tier); (5)~group/solidarity lending with shared collateral pool and mutual liability enforcement.}
...
\small
\textbf{Reading guide.}
\textbf{Solid blue arrows} ($\downarrow$) denote downward capital allocation — the primary lending channel.
\textbf{Green dashed arrows} ($\uparrow$) denote upward surplus repatriation — excess reserves deposited at higher tiers.
\textbf{Double-headed blue dashed arrows} ($\leftrightarrow$) denote same-tier interbank lending pools (National $\leftrightarrow$ National, Local $\leftrightarrow$ Local).
\textbf{Orange dashed arrows} denote repayments with interest cascading upward.
\textbf{Violet dashed arrows} denote group lending mutual-liability enforcement through the shared collateral pool.
The complete attribute schema for all 15 entities is shown in the ERD (Figure~\ref{fig:erd}) and EER model (Figure~\ref{fig:eer}).
\end{highlightbox}
...
\begin{figure}[p]
\centering
\caption{Entity-Relationship Diagram (ERD) for the Crypto World Bank database: all 15 normalized tables (3NF) showing primary keys (PK), foreign keys (FK), key data types, and cardinality. Tables are grouped in four functional rows: banking hierarchy, user/limit entities, lending lifecycle, and terminal records.}
\label{fig:erd}

\vspace{4pt}
% ── Scaled to fit one page; landscape-like wide layout ────────────────────────
\begin{tikzpicture}[
  ...
  etitle/.style={%
    ...
    minimum width=3.4cm, minimum height=0.46cm,
    inner sep=3pt, align=center, rounded corners=1.5pt 1.5pt 0pt 0pt
  },
  %% ── attribute body ────────────────────────────────────────────────────────
  eattr/.style={%
    draw=#1!45, fill=white,
    font=\fontsize{7.5}{9}\selectfont,
    minimum width=3.4cm,
    inner sep=2.5pt, align=left, rounded corners=0pt 0pt 1.5pt 1.5pt
  },
  %% ── relationship arrows ───────────────────────────────────────────────────
  rel/.style={draw=gray!65, ->, line width=0.5pt},
  relD/.style={draw=gray!65, ->, line width=0.5pt, dashed},
  %% ── cardinality label ─────────────────────────────────────────────────────
  crd/.style={font=\fontsize{6.5}{8}\selectfont, fill=white, inner sep=0.8pt,
              draw=none},
]

%%═══════════════════════════════════════════════════════════════════════════════
%% ROW A — Banking hierarchy (left) + Independent tables (right)
%%  Column spacing: 0, 4.0, 8.0, 12.0, 16.0
%%  Each entity block: title height ≈0.46, attr height ≈1.4–2.0  → ~2.0cm total
%%═══════════════════════════════════════════════════════════════════════════════

%% WORLD_BANK (col 0, row A)
\node[etitle=PrimaryBlue] (WBt) at (0,0) {WORLD\_BANK};
\node[eattr=PrimaryBlue, below=0pt of WBt, anchor=north, text width=3.3cm]
  (WBa) {\underline{PK} world\_bank\_id\\
          FK \textemdash{}\\
          name (UK);\; total\_reserve\\
          created\_at;\; updated\_at};

%% NATIONAL_BANK (col 4.0, row A)
\node[etitle=PrimaryBlue] (NBt) at (4.0,0) {NATIONAL\_BANK};
\node[eattr=PrimaryBlue, below=0pt of NBt, anchor=north, text width=3.3cm]
  (NBa) {\underline{PK} national\_bank\_id\\
          FK  world\_bank\_id\\
          name;\; country;\; address\\
          total\_borrowed;\; total\_lent\\
          created\_at;\; updated\_at};

%% LOCAL_BANK (col 8.0, row A)
\node[etitle=orange!80!black] (LBt) at (8.0,0) {LOCAL\_BANK};
\node[eattr=orange!80!black, below=0pt of LBt, anchor=north, text width=3.3cm]
  (LBa) {\underline{PK} local\_bank\_id\\
          FK  national\_bank\_id\\
          name;\; city;\; address\\
          total\_borrowed;\; total\_lent\\
          created\_at;\; updated\_at};

%% AI_CHATBOT_LOG (col 12.0, row A)
\node[etitle=GreenAccent] (ACLt) at (12.0,0) {AI\_CHATBOT\_LOG};
\node[eattr=GreenAccent, below=0pt of ACLt, anchor=north, text width=3.3cm]
  (ACLa) {\underline{PK} log\_id\\
           user\_wallet;\; user\_type\\
           question;\; response\\
           intent;\; created\_at};

%% MARKET_DATA (col 16.0, row A)
\node[etitle=VioletAccent] (MDt) at (16.0,0) {MARKET\_DATA};
\node[eattr=VioletAccent, below=0pt of MDt, anchor=north, text width=3.3cm]
  (MDa) {\underline{PK} market\_data\_id\\
          cryptocurrency\_type\\
          price\_usd;\; volume\_24h\\
          market\_cap;\; recorded\_at};

%%═══════════════════════════════════════════════════════════════════════════════
%% ROW B — User and limit entities   (y = -3.6)
%%═══════════════════════════════════════════════════════════════════════════════

%% BORROWER (col 0, row B)
\node[etitle=GreenAccent] (BRt) at (0,-3.6) {BORROWER};
\node[eattr=GreenAccent, below=0pt of BRt, anchor=north, text width=3.3cm]
  (BRa) {\underline{PK} borrower\_id\\
          wallet\_address (UK)\\
          name;\; email (UK);\; phone\\
          country;\; city\\
          is\_first\_time;\; can\_multi\_loans\\
          consecutive\_paid\_loans\\
          created\_at;\; updated\_at};

%% BANK_USER (col 4.0, row B)
\node[etitle=PrimaryBlue] (BUt) at (4.0,-3.6) {BANK\_USER};
\node[eattr=PrimaryBlue, below=0pt of BUt, anchor=north, text width=3.3cm]
  (BUa) {\underline{PK} bank\_user\_id\\
          wallet\_address (UK)\\
          bank\_type;\; role\\
          FK national\_bank\_id\\
          FK local\_bank\_id\\
...
%% INCOME_PROOF (col 8.0, row B)
\node[etitle=AmberAccent] (IPt) at (8.0,-3.6) {INCOME\_PROOF};
\node[eattr=AmberAccent, below=0pt of IPt, anchor=north, text width=3.3cm]
  (IPa) {\underline{PK} proof\_id\\
          FK  borrower\_id\\
          FK  reviewed\_by (BANK\_USER)\\
          file\_path;\; amount\\
          status;\; uploaded\_at};

%% BORROWING_LIMIT (col 12.0, row B)
\node[etitle=VioletAccent] (BLt) at (12.0,-3.6) {BORROWING\_LIMIT};
\node[eattr=VioletAccent, below=0pt of BLt, anchor=north, text width=3.3cm]
  (BLa) {\underline{PK} limit\_id\\
          FK  borrower\_id (UK — 1:1)\\
          six\_month\_limit;\; remaining\\
          one\_year\_limit;\; remaining\\
          last\_calculated\_at\\
          updated\_at};

%% PROFILE_SETTINGS (col 16.0, row B)
\node[etitle=AmberAccent] (PSt) at (16.0,-3.6) {PROFILE\_SETTINGS};
\node[eattr=AmberAccent, below=0pt of PSt, anchor=north, text width=3.3cm]
  (PSa) {\underline{PK} profile\_id\\
          user\_type;\; user\_id\\
          terms\_accepted\\
          preferences\\
          created\_at;\; updated\_at};
...
%%═══════════════════════════════════════════════════════════════════════════════
%% ROW C — Lending lifecycle   (y = -7.8)
%%═══════════════════════════════════════════════════════════════════════════════

%% LOAN_REQUEST (col 0, row C)
\node[etitle=RedAccent] (LRt) at (0,-7.8) {LOAN\_REQUEST};
\node[eattr=RedAccent, below=0pt of LRt, anchor=north, text width=3.3cm]
  (LRa) {\underline{PK} loan\_id\\
          FK  borrower\_id;\; local\_bank\_id\\
          amount;\; purpose\\
          cryptocurrency\_type;\; status\\
          FK approved\_by;\; rejected\_by\\
          is\_installment;\; total\_installments\\
          blockchain\_tx\_hash (UK)\\
          deadline;\; created\_at};

%% TRANSACTION (col 4.0, row C)
\node[etitle=GreenAccent] (TXt) at (4.0,-7.8) {TRANSACTION};
\node[eattr=GreenAccent, below=0pt of TXt, anchor=north, text width=3.3cm]
  (TXa) {\underline{PK} transaction\_id\\
          FK  borrower\_id\\
          FK  local\_bank\_id\\
          FK  national\_bank\_id\\
          FK  related\_loan\_id\\
          transaction\_type;\; amount\\
          blockchain\_tx\_hash (UK)\\
          transaction\_date;\; created\_at};

%% CHAT_MESSAGE (col 8.0, row C)
\node[etitle=VioletAccent] (CMt) at (8.0,-7.8) {CHAT\_MESSAGE};
\node[eattr=VioletAccent, below=0pt of CMt, anchor=north, text width=3.3cm]
  (CMa) {\underline{PK} message\_id\\
          FK  loan\_id\\
          sender\_type;\; sender\_id\\
          receiver\_type;\; receiver\_id\\
          message\_text;\; is\_read\\
          sent\_at};

%%═══════════════════════════════════════════════════════════════════════════════
%% ROW D — Terminal / weak entities   (y = -12.0)
%%═══════════════════════════════════════════════════════════════════════════════

%% INSTALLMENT (col 0, row D)
\node[etitle=orange!80!black] (INt) at (0,-12.0) {INSTALLMENT};
\node[eattr=orange!80!black, below=0pt of INt, anchor=north, text width=3.3cm]
  (INa) {\underline{PK} installment\_id\\
          FK  loan\_id (identifying)\\
          installment\_number\\
          amount\_due;\; amount\_paid\\
          status;\; due\_date\\
          blockchain\_tx\_hash (UK)\\
          created\_at;\; updated\_at};

%% AI_ML_SECURITY_LOG (col 4.0, row D)
\node[etitle=RedAccent] (MLt) at (4.0,-12.0) {AI\_ML\_SECURITY\_LOG};
\node[eattr=RedAccent, below=0pt of MLt, anchor=north, text width=3.3cm]
  (MLa) {\underline{PK} security\_log\_id\\
          FK  loan\_id\\
          FK  transaction\_id\\
          FK  reviewed\_by (BANK\_USER)\\
          assoc\_wallet\\
          risk\_type;\; risk\_score\\
          action\_taken;\; detected\_at};
...
%% WB → NB  (1:N)
\draw[rel] (WBa.south) -- ++(0,-0.20)
  node[crd,right,pos=0.5]{1:N} -| (NBt.north);

%% NB → LB  (1:N)
\draw[rel] (NBa.south) -- ++(0,-0.20)
  node[crd,right,pos=0.5]{1:N} -| (LBt.north);

%% NB → BU  (1:N) — vertical drop to row B
\draw[rel] (NBa.south) ++(0.4,0) -- ++(0,-1.55)
  node[crd,right,pos=0.5]{1:N} -| (BUt.north);

%% LB → BU  (1:N)
\draw[rel] (LBa.south) ++(-0.4,0) -- ++(0,-1.55)
  node[crd,left,pos=0.5]{1:N} -| (BUt.north);

%% NB/LB → LR via their respective FKs in LOAN_REQUEST
\draw[rel] (LBa.south) ++(0.4,0) -- ++(0,-4.2)
  node[crd,right,pos=0.7]{1:N} -| (LRt.north);

%% BR → LR  (1:N)
\draw[rel] (BRa.south) -- ++(0,-0.35)
  node[crd,right,pos=0.5]{1:N} -| (LRt.north);

%% BR → IP  (1:N)
\draw[rel] (BRa.east) -- node[crd,above]{1:N} (IPt.west);

%% BR → BL  (1:1)
\draw[rel] (BRa.east) ++(0,-0.4) -- ++(3.6,0)
  node[crd,above,pos=0.5]{1:1} -| (BLt.north);

%% BR → TX  (1:N)
\draw[rel] (BRa.south) ++(0.3,0) -- ++(0,-4.4)
  node[crd,right,pos=0.6]{1:N} -| (TXt.north);

%% LR → IN  (1:N, identifying)
\draw[rel, double, double distance=0.8pt]
  (LRa.south) -- node[crd,left]{1:N} (INt.north);

%% LR → CM  (1:N)
\draw[rel] (LRa.south) ++(0.5,0) -- ++(0,-0.55)
  node[crd,right,pos=0.5]{1:N} -| (CMt.north);

%% LR → ML  (1:N)
\draw[rel] (LRa.south) ++(0.9,0) -- ++(0,-4.35)
  node[crd,right,pos=0.6]{1:N} -| (MLt.north);

%% TX → ML  (1:N)
\draw[rel] (TXa.south) -- ++(0,-0.30)
  node[crd,right,pos=0.5]{1:N} -| (MLt.north);

%% BU → IP  (reviews — dashed)
\draw[relD] (BUt.east) ++(0,-0.3) -- node[crd,above]{reviews} (IPt.west) ++(0,-0.3);

\end{tikzpicture}

\vspace{4pt}
{\fontsize{7.5}{9}\selectfont
\textbf{Key:} \underline{PK} = Primary Key;\quad FK = Foreign Key;\quad (UK) = Unique Constraint;\quad
double line = identifying (weak-entity) relationship.\\
Solid arrows ($\to$) = referential integrity (FK $\to$ PK, 1:N unless labelled 1:1).
Dashed arrow = role/review relationship (BU reviews IP).
Independent tables AI\_CHATBOT\_LOG, MARKET\_DATA, and PROFILE\_SETTINGS carry no FK to the lending core.
}
\end{figure}
...
\begin{figure}[p]
\centering
\caption{Enhanced Entity-Relationship (EER) diagram: full data model showing generalization/specialization (BANK\_USER $\to$ National/Local subtypes, disjoint), weak entities (INSTALLMENT, BORROWING\_LIMIT), multi-valued attribute (INCOME\_PROOF), aggregation (Loan-Centric cluster), and participation constraints. Dashed panel borders group related constructs.}
\label{fig:eer}
...
\begin{tikzpicture}[
  ...
  ent/.style={draw=PrimaryBlue!70, fill=LightBlue,
              ...
              font=\fontsize{7.5}{9}\selectfont\bfseries, inner sep=3pt},
  ...
  panel/.style={draw=gray!40, dashed, rounded corners=4pt,
                inner sep=8pt, fill=gray!3},
  %% ── relationships ─────────────────────────────────────────────────────────
  rel/.style={draw=gray!60,  -,  line width=0.6pt},
  relA/.style={draw=PrimaryBlue!70, ->, line width=0.6pt},
  relD/.style={draw=gray!55, ->, line width=0.6pt, dashed},
  %% ── cardinality label ─────────────────────────────────────────────────────
  card/.style={font=\fontsize{6.5}{8}\selectfont, fill=white, inner sep=0.8pt, draw=none},
]

%%═══════════════════════════════════════════════════════════════════════════════
%% PANEL A — Independent Entities  (top-left, col 0–3.6, row 0)
%%═══════════════════════════════════════════════════════════════════════════════
\begin{scope}
\node[panel,
      label={[font=\fontsize{7}{8}\selectfont\bfseries]above left:Independent Entities},
      minimum width=3.6cm, minimum height=5.6cm]
  at (-0.4, 1.0) {};

\node[ent] (MD)  at (0.0,  3.0) {MARKET\_DATA\\{\fontsize{6.5}{8}\selectfont PK: market\_data\_id}\\{\fontsize{6.5}{8}\selectfont crypto\_type, price\_usd}};
\node[ent] (ACL) at (0.0,  1.2) {AI\_CHATBOT\_LOG\\{\fontsize{6.5}{8}\selectfont PK: log\_id}\\{\fontsize{6.5}{8}\selectfont user\_wallet, intent}};
\node[ent] (PS)  at (0.0, -0.6) {PROFILE\_SETTINGS\\{\fontsize{6.5}{8}\selectfont PK: profile\_id}\\{\fontsize{6.5}{8}\selectfont user\_type, user\_id}};
\end{scope}

%%═══════════════════════════════════════════════════════════════════════════════
%% PANEL B — Banking Hierarchy  (top-centre, col 4–7)
%%═══════════════════════════════════════════════════════════════════════════════
\begin{scope}
\node[panel,
      label={[font=\fontsize{7}{8}\selectfont\bfseries]above:Banking Hierarchy},
      minimum width=3.6cm, minimum height=5.8cm]
  at (5.8, 1.0) {};

\node[ent] (WB) at (5.8,  3.2) {WORLD\_BANK\\{\fontsize{6.5}{8}\selectfont PK: world\_bank\_id}\\{\fontsize{6.5}{8}\selectfont total\_reserve, name}};
\node[ent] (NB) at (5.8,  1.2) {NATIONAL\_BANK\\{\fontsize{6.5}{8}\selectfont PK: national\_bank\_id}\\{\fontsize{6.5}{8}\selectfont FK: world\_bank\_id}\\{\fontsize{6.5}{8}\selectfont country, total\_borrowed}};
\node[ent] (LB) at (5.8, -0.8) {LOCAL\_BANK\\{\fontsize{6.5}{8}\selectfont PK: local\_bank\_id}\\{\fontsize{6.5}{8}\selectfont FK: national\_bank\_id}\\{\fontsize{6.5}{8}\selectfont city, total\_lent}};

\draw[relA] (WB) -- node[card,right]{1:N} (NB);
\draw[relA] (NB) -- node[card,right]{1:N} (LB);
\end{scope}

%%═══════════════════════════════════════════════════════════════════════════════
%% PANEL C — Generalisation: BANK_USER  (top-right, col 10–14)
%%═══════════════════════════════════════════════════════════════════════════════
\begin{scope}
\node[panel,
      label={[font=\fontsize{7}{8}\selectfont\bfseries]above right:Generalisation: BANK\_USER},
      minimum width=5.4cm, minimum height=4.6cm]
  at (12.0, 1.0) {};

\node[ent] (BU)  at (12.0, 2.8) {BANK\_USER\\{\fontsize{6.5}{8}\selectfont PK: bank\_user\_id}\\{\fontsize{6.5}{8}\selectfont wallet\_address, role}\\{\fontsize{6.5}{8}\selectfont discriminator: bank\_type}};
\node[ent] (NBU) at (10.4, 0.4) {NAT.\ BANK\_USER\\{\fontsize{6.5}{8}\selectfont FK: national\_bank\_id}};
\node[ent] (LBU) at (13.6, 0.4) {LOCAL\_BANK\_USER\\{\fontsize{6.5}{8}\selectfont FK: local\_bank\_id}};

% disjoint generalisation fork
\draw[rel] (BU.south) -- ++(0,-0.45) coordinate (gmid);
\draw[rel] (gmid) -| (NBU.north);
\draw[rel] (gmid) -| (LBU.north);
\node[font=\fontsize{7}{8}\selectfont, fill=white, inner sep=1pt] at (gmid) {d};
\end{scope}

%%═══════════════════════════════════════════════════════════════════════════════
%% PANEL D — User Layer  (middle-left, col 0–5, row -3.6)
%%═══════════════════════════════════════════════════════════════════════════════
\begin{scope}
\node[panel,
      label={[font=\fontsize{7}{8}\selectfont\bfseries]above left:User Layer},
      minimum width=5.8cm, minimum height=4.4cm]
  at (1.6,-4.6) {};

\node[ent] (BR)  at (2.8,-3.2) {BORROWER\\{\fontsize{6.5}{8}\selectfont PK: borrower\_id}\\{\fontsize{6.5}{8}\selectfont wallet\_address, country}\\{\fontsize{6.5}{8}\selectfont consecutive\_paid\_loans}};
\node[ent] (IP)  at (0.0,-5.4) {INCOME\_PROOF\\{\fontsize{6.5}{8}\selectfont PK: proof\_id}\\{\fontsize{6.5}{8}\selectfont FK: borrower\_id}\\{\fontsize{6.5}{8}\selectfont status, amount}};
\node[went](BL)  at (4.2,-5.4) {BORROWING\_LIMIT\\{\fontsize{6.5}{8}\selectfont PK: limit\_id}\\{\fontsize{6.5}{8}\selectfont FK: borrower\_id (UK)}\\{\fontsize{6.5}{8}\selectfont 6mo / 1yr limits}};

\draw[relA] (BR.south) -- ++(-0.8,-0.5) -- (IP.north)  node[card,pos=0.5,left]{1:N};
\draw[relA] (BR.south) -- ++(0.8,-0.5)  -- (BL.north)  node[card,pos=0.5,right]{1:1};

% LOCAL_BANK serves BORROWER
\draw[relA] (LB.south) -- ++(0,-0.5) -- ++(0,-1.2) -| node[card,pos=0.2,left]{1:N serves} (BR.north);
\end{scope}

%%═══════════════════════════════════════════════════════════════════════════════
%% PANEL E — Loan-Centric Aggregation  (middle-right, col 6.5–14.5, row -3.0)
%%═══════════════════════════════════════════════════════════════════════════════
\begin{scope}
\node[panel,
      label={[font=\fontsize{7}{8}\selectfont\bfseries]above:Weak Entity / Aggregation — Loan-Centric},
      minimum width=8.0cm, minimum height=6.8cm]
  at (10.2,-4.2) {};

\node[went] (LR)  at (10.2,-2.8) {LOAN\_REQUEST\\{\fontsize{6.5}{8}\selectfont PK: loan\_id}\\{\fontsize{6.5}{8}\selectfont FK: borrower\_id, local\_bank\_id}\\{\fontsize{6.5}{8}\selectfont amount, status, deadline}};
\node[ent]  (IN)  at (7.2, -5.0) {INSTALLMENT\\{\fontsize{6.5}{8}\selectfont PK: loan\_id+installment\_no}\\{\fontsize{6.5}{8}\selectfont FK: loan\_id (identifying)}\\{\fontsize{6.5}{8}\selectfont amount\_due, due\_date}};
\node[ent]  (TX)  at (10.6,-5.0) {TRANSACTION\\{\fontsize{6.5}{8}\selectfont PK: transaction\_id}\\{\fontsize{6.5}{8}\selectfont FK: borrower\_id}\\{\fontsize{6.5}{8}\selectfont type, amount}};
\node[ent]  (CM)  at (13.4,-5.0) {CHAT\_MESSAGE\\{\fontsize{6.5}{8}\selectfont PK: message\_id}\\{\fontsize{6.5}{8}\selectfont FK: loan\_id}\\{\fontsize{6.5}{8}\selectfont sender\_type, text}};
\node[ent]  (ML)  at (10.6,-7.2) {AI\_ML\_SECURITY\_LOG\\{\fontsize{6.5}{8}\selectfont PK: security\_log\_id}\\{\fontsize{6.5}{8}\selectfont FK: loan\_id, transaction\_id}\\{\fontsize{6.5}{8}\selectfont risk\_type, risk\_score}};

% BR → LR (requests)
\draw[relA] (BR.east) -- ++(1.0,0) |- node[card,pos=0.8,right]{\fontsize{6.5}{8}\selectfont N:1} (LR.north west);
% LB → LR (receives)
\draw[relA] (LB.east) -- ++(0.5,0) |- node[card,pos=0.7,right]{\fontsize{6.5}{8}\selectfont 1:N} (LR.north);
% LR → IN (identifying weak)
\draw[rel, double, double distance=0.8pt]
  (LR.south west) -- node[card,pos=0.5,left]{\fontsize{6.5}{8}\selectfont 1:N} (IN.north);
% LR → TX
\draw[relA] (LR.south) -- node[card,pos=0.5,right]{\fontsize{6.5}{8}\selectfont 1:N} (TX.north);
% LR → CM
\draw[relA] (LR.south east) -- node[card,pos=0.5,right]{\fontsize{6.5}{8}\selectfont 1:N} (CM.north);
% TX → ML
\draw[relA] (TX.south) -- node[card,pos=0.5,right]{\fontsize{6.5}{8}\selectfont monitors} (ML.north);
% CM → ML (dashed, reviews)
\draw[relD] (CM.south) -- ++(0,-0.35) -| node[card,pos=0.3,right]{\fontsize{6.5}{8}\selectfont reviews} (ML.east);
% BU reviews LR (dashed, cross-panel)
\draw[relD] (BU.south) -- ++(0,-1.2) -| node[card,pos=0.2,right]{\fontsize{6.5}{8}\selectfont reviews} (LR.north east);
\end{scope}
...
Double-border box = weak entity;\quad
\textsf{d} = disjoint subtype constraint;\quad
Dashed line = optional/role relationship;\quad
Double line = identifying relationship (existence dependency).
}
\end{figure}
...
\begin{tikzpicture}[
  ...
  x=1.0cm, y=1.0cm,
  %% ── tier box styles ───────────────────────────────────────────────────────
  wb/.style={draw=PrimaryBlue, fill=PrimaryBlue!15,
             rounded corners=4pt, minimum width=3.0cm, minimum height=1.6cm,
             text width=2.8cm, align=center, inner sep=4pt,
             font=\fontsize{9}{10.5}\selectfont},
  nb/.style={draw=AccentBlue!80, fill=AccentBlue!10,
             rounded corners=3pt, minimum width=3.0cm, minimum height=1.5cm,
             text width=2.8cm, align=center, inner sep=4pt,
             font=\fontsize{9}{10.5}\selectfont},
  lb/.style={draw=GreenAccent!80, fill=GreenAccent!10,
             rounded corners=3pt, minimum width=3.0cm, minimum height=1.5cm,
             text width=2.8cm, align=center, inner sep=4pt,
             font=\fontsize{9}{10.5}\selectfont},
  br/.style={draw=AmberAccent!80, fill=AmberAccent!8,
             rounded corners=3pt, minimum width=3.0cm, minimum height=1.5cm,
             text width=2.8cm, align=center, inner sep=4pt,
             font=\fontsize{9}{10.5}\selectfont},
  %% ── arrow styles ──────────────────────────────────────────────────────────
  down/.style={->, draw=PrimaryBlue!80, line width=1.0pt},
  up/.style={->, draw=GreenAccent!80, line width=0.85pt, dashed},
  peer/.style={<->, draw=AccentBlue!60, line width=0.85pt,
               dash pattern=on 4pt off 2pt},
  repay/.style={->, draw=AmberAccent!70, line width=0.8pt,
                dash pattern=on 2pt off 1pt},
  lbl/.style={font=\fontsize{8}{9}\selectfont, fill=white, inner sep=1pt, draw=none},
]
...
%%─────────────────────────────────────────────────────────────────────────────
\node[wb] (WB) at (5.8, 0) {%
  \textbf{World Bank Reserve}\\
  {\fontsize{8}{9}\selectfont Tier 1 · 1,000,000 ETH}\\
  {\fontsize{8}{9}\selectfont\textit{Lends @ 3\% APR}}};

%%─────────────────────────────────────────────────────────────────────────────
%% TIER 2 — National Banks (two shown)
%%─────────────────────────────────────────────────────────────────────────────
\node[nb] (NB1) at (2.4, -3.0) {%
  \textbf{National Bank A}\\
  {\fontsize{8}{9}\selectfont 200k ETH · 5\% APR}};
\node[nb] (NB2) at (9.2, -3.0) {%
  \textbf{National Bank B}\\
  {\fontsize{8}{9}\selectfont 200k ETH · 5\% APR}};

% WB → NB (capital allocation, downward)
\draw[down] (WB.south west) -- node[lbl,left]{capital} (NB1.north);
\draw[down] (WB.south east) -- node[lbl,right]{capital} (NB2.north);

% NB → WB (surplus upward)
\draw[up] (NB1.north) ++(0.4,0) -- ++(0,2.4)
  node[lbl,right,pos=0.5]{surplus} -- (WB.south west) ++(0.1,0);
\draw[up] (NB2.north) ++(-0.4,0) -- ++(0,2.4)
  node[lbl,left,pos=0.5]{surplus} -- (WB.south east) ++(-0.1,0);

% NB1 ↔ NB2 (same-tier interbank)
\draw[peer] (NB1.east) -- node[lbl,above]{interbank lending} (NB2.west);

%%─────────────────────────────────────────────────────────────────────────────
%% TIER 3 — Local Banks (two per NB)
%%─────────────────────────────────────────────────────────────────────────────
\node[lb] (LB1a) at (0.6, -6.0) {%
  \textbf{Local Bank A1}\\
  {\fontsize{8}{9}\selectfont 10k ETH · 8\% APR}};
\node[lb] (LB1b) at (4.0, -6.0) {%
  \textbf{Local Bank A2}\\
  {\fontsize{8}{9}\selectfont 10k ETH · 8\% APR}};
\node[lb] (LB2a) at (7.6, -6.0) {%
  \textbf{Local Bank B1}\\
  {\fontsize{8}{9}\selectfont 10k ETH · 8\% APR}};
\node[lb] (LB2b) at (11.0,-6.0) {%
  \textbf{Local Bank B2}\\
  {\fontsize{8}{9}\selectfont 10k ETH · 8\% APR}};

% NB → LB (downward)
\draw[down] (NB1.south) -- ++(0,-0.3) -| node[lbl,left,pos=0.1]{capital} (LB1a.north);
\draw[down] (NB1.south) -- ++(0,-0.3) -| node[lbl,right,pos=0.1]{capital} (LB1b.north);
\draw[down] (NB2.south) -- ++(0,-0.3) -| node[lbl,left,pos=0.1]{capital} (LB2a.north);
\draw[down] (NB2.south) -- ++(0,-0.3) -| node[lbl,right,pos=0.1]{capital} (LB2b.north);

% LB → NB (upward surplus)
\draw[up] (LB1b.north) ++(0.3,0) -- ++(0,2.4)
  node[lbl,right,pos=0.5]{surplus} -- (NB1.south) ++(0.3,0);
\draw[up] (LB2a.north) ++(-0.3,0) -- ++(0,2.4)
  node[lbl,left,pos=0.5]{surplus} -- (NB2.south) ++(-0.3,0);

% LB ↔ LB (cross-bank same-tier)
\draw[peer] (LB1b.east) -- node[lbl,above]{\fontsize{7.5}{9}\selectfont peer pool} (LB2a.west);

%%─────────────────────────────────────────────────────────────────────────────
%% TIER 4 — Borrowers
%%─────────────────────────────────────────────────────────────────────────────
\node[br] (BR1) at (0.6, -9.0) {%
  Borrower\\
  {\fontsize{8}{9}\selectfont 0.1–10 ETH}};
\node[br] (BR2) at (4.0, -9.0) {%
  SME\\
  {\fontsize{8}{9}\selectfont 1–100 ETH}};
\node[br] (BR3) at (7.6, -9.0) {%
  Borrower\\
  {\fontsize{8}{9}\selectfont 0.1–10 ETH}};
\node[br] (BR4) at (11.0,-9.0) {%
  Corporate\\
  {\fontsize{8}{9}\selectfont 50–500 ETH}};

% LB → Borrower (loan disbursement)
\draw[down] (LB1a.south) -- node[lbl,left]{loan} (BR1.north);
\draw[down] (LB1b.south) -- node[lbl,right]{loan} (BR2.north);
\draw[down] (LB2a.south) -- node[lbl,left]{loan} (BR3.north);
\draw[down] (LB2b.south) -- node[lbl,right]{loan} (BR4.north);

% Repayments (upward, cascades)
\draw[repay] (BR1.north) ++(0.4,0) -- ++(0,1.6)
  node[lbl,right,pos=0.5]{repay+int.} -- (LB1a.south) ++(0.3,0);
\draw[repay] (BR2.north) ++(-0.4,0) -- ++(0,1.6)
  node[lbl,left,pos=0.5]{repay+int.} -- (LB1b.south) ++(-0.3,0);
\draw[repay] (BR3.north) ++(0.4,0) -- ++(0,1.6)
  node[lbl,right,pos=0.5]{repay+int.} -- (LB2a.south) ++(0.3,0);
\draw[repay] (BR4.north) ++(-0.4,0) -- ++(0,1.6)
  node[lbl,left,pos=0.5]{repay+int.} -- (LB2b.south) ++(-0.3,0);

%%─────────────────────────────────────────────────────────────────────────────
%% LEGEND
%%─────────────────────────────────────────────────────────────────────────────
\begin{scope}[font=\fontsize{8}{9}\selectfont]
  \draw[down]  (0.0,-10.6) -- ++(1.2,0) node[right]{Downward capital allocation};
  \draw[up]    (0.0,-11.2) -- ++(1.2,0) node[right]{Upward surplus repatriation};
  \draw[peer]  (0.0,-11.8) -- ++(1.2,0) node[right]{Same-tier interbank lending ($\leftrightarrow$)};
  \draw[repay] (0.0,-12.4) -- ++(1.2,0) node[right]{Repayment with interest (cascades upward)};
\end{scope}

\end{tikzpicture}
\caption{Four-tier capital flow with multi-directional lending: (1)~downward capital allocation (World Bank Reserve $\to$ National Banks $\to$ Local Banks $\to$ Borrowers); (2)~upward surplus repatriation (excess reserves deposited back up the hierarchy); (3)~same-tier interbank lending pools (National Bank~A $\leftrightarrow$ National Bank~B; Local Bank~A2 $\leftrightarrow$ Local Bank~B1); (4)~cascading repayments with interest flowing upward. Interest rate spreads increase down the hierarchy (3\%~$\to$~5\%~$\to$~8\%), incentivising downward capital flow and upward surplus repatriation.}
\label{fig:four-tier}
\end{figure}
