...
DeFi has shown that lending, collateral, and interest can run on smart contracts with public audit trails. Aave~v3 holds roughly \$26.3~billion TVL~[27], and the broader DeFi lending market exceeds \$55~billion~[13]. Yet these protocols use flat pools: all lenders share one pool and all borrowers draw from it, with no institutional tiers, cross-tier flows, or client-via-local-bank paths typical of development finance.

\paragraph{What blockchain adds here.}
A \textbf{smart contract} is program code on a blockchain that runs the same way on every node. For CWB, three properties matter: (1)~\textbf{governed execution}---contract changes need a recorded vote and timelock (Section~\ref{sec:governance-framework}), reducing flash-governance attacks like the \$182M Beanstalk exploit~[158]; (2)~\textbf{atomic settlement}---multi-step operations complete fully or revert; (3)~\textbf{public audit}---every state change is logged on-chain. \textbf{Blockchain} adds a shared ledger so co-platform members see the same reserve and loan state without slow bilateral reconciliation~[25]. \textbf{DeFi} applies this to finance, but CWB keeps institutions, KYC, and ML off-chain while enforcing tier rules on-chain (Section~\ref{sec:blockchain-justification}). CWB extends flat DeFi in three ways: institutional hierarchy, multi-entity operations (group loans, syndication, interbank pools), and alignment with development-finance structures absent in current protocols~[1].
...
\item \textbf{Slow settlement and idle capital.} Cross-border transfers take 2--5 days and cost about \$42 per transaction~[25]. Pre-funded correspondent accounts lock capital out of lending~[24].
...
\item \textbf{Access and trust barriers.} Building trust between institutions is slow and costly, disadvantaging smaller banks and borrowers in developing markets~[28].
...
\item \textbf{Limited institutional DeFi}---Maple and Goldfinch serve institutional credit but without hierarchical capital flows or interbank facilities~[31, 32].
\end{itemize}
...
\begin{enumerate}[noitemsep]
\item \textbf{RQ1:} Can a four-tier on-chain architecture encode development-finance capital flows and governance, unlike flat DeFi pools? \textit{Evaluated by workflow mapping and protocol comparison (Section~\ref{sec:eval-method}).}
\item \textbf{RQ2:} Can on-chain transparency and programmable reserves reduce opacity and friction across tiers?
\item \textbf{RQ3:} Can a lightweight off-chain ML layer support practical, auditable fraud and lending monitoring?
\item \textbf{RQ4:} Can the architecture run on public testnets with the RBAC and reserve rules needed for institutional use? \textit{Testnet viability only; not a production-readiness claim.}
\item \textbf{RQ5:} Can deposit-funded and group lending work on-chain while staying feasible for retail users?
\end{enumerate}
...
\item[\textbf{Contribution 1 --- Four-Tier Hierarchical DeFi.}]
No prior work combines a four-tier reserve-enforced hierarchy, oracle-mediated explainable ML, and group lending in one open architecture. Partial precedents (Maple, Goldfinch, Morpho~V2, Aave Arc) lack full multilateral development-finance flows~[1, 43, 118]. Tier~1--3 contracts are specified in Solidity; cross-tier automation and testnet deployment are planned for Phases~I--II.

\item[\textbf{Contribution 2 --- On-Chain Group Lending.}]
No published work specifies programmable mutual-liability group lending in Solidity with formation consent, installment splits, and over-indebtedness controls. The design is an honest analogue to BRAC/Grameen solidarity models, not a direct copy---on-chain groups lack weekly meetings and field-officer social pressure~[119, 120]. Default rates are modeled conservatively (15--25\%); \texttt{InsuranceFund} sizing follows a 20\% stress scenario.

\item[\textbf{Contribution 3 --- Oracle-Mediated ML with Explainability.}]
Off-chain Random Forest, Isolation Forest, and SHAP feed on-chain lending via commit--reveal oracle relay~[3, 4, 6]. Pre-thesis metrics use a synthetic 79-feature validation run; BCCC-DeFiFraudTrans-2025 retraining follows York access approval (request submitted, Section~\ref{sec:ml-workload}). A Phase~III conversational agent (Qwen3-8B + MCP) extends this layer with 3--5 tools at MVT scope.

\item[\textbf{Contribution 4 --- ZKP Compliance Pathway (Future Work).}]
A two-circuit design (Groth16 KYC + zkAML) with W3C DID/VC is specified but deferred past MVT~[R8, R19, R20]. Retail onboarding at MVT uses tiered off-chain KYC and on-chain RBAC (Section~\ref{sec:tiered-kyc}).

\end{description}

CWB targets development banks and MFIs, not pension-fund DeFi allocators~[R21]. It is positioned as a composable lending layer compatible with emerging CBDC and tokenized-deposit rails (Section~\ref{sec:stablecoin-first}).

The remainder of this report: Chapter~2 reviews literature; Chapter~3 presents architecture; Chapter~4 covers methodology and MVT checklist; Chapter~5 covers feasibility; Chapter~6 concludes.
...
\caption[Six banking functions of the platform]{Six banking functions enforced across the four-tier core. Contract mappings (\texttt{SavingsVault}, \texttt{allocateCapital}, \texttt{IBLP}, etc.) are listed below and in Chapter~\ref{sec:high-level-arch}.}
...
\caption[Cross-tier lending flow directions]{Cross-tier lending: downward allocation, upward surplus repatriation, and same-tier interbank pools. Syndicated, group, and tranched co-funding are in Section~\ref{sec:multi-entity}.}
...
Tan~[5] and BIS CBDC work~[53, 134, 135] describe two-tier central-bank-to-bank-to-user distribution---parallel to CWB's four tiers. Bindseil~[133] supports asymmetric upward/downward rate spreads for \texttt{UpwardDepositFacility}. Project Pine~[131] and Dong et al.~[132] validate layered smart-contract factories and cross-tier capital flows in policy research, but no single DeFi spec combines them---the gap in Figure~\ref{fig:cross-tier-lending}.

\subsection{Group Lending and Microfinance}

Grameen and BRAC show solidarity groups can repay above 95\% without individual collateral, using weekly meetings and field officers that on-chain systems cannot replicate. CWB's \texttt{GroupLendingPool} encodes mutual liability in code with conservative default assumptions. Asaduzzaman et al.~[54] report up to 60\% admin savings from smart-contract microcredit. A 2025 IEEE microloan prototype~[156] is the closest technical analogue. Jaffer~[119] and Sherratt~[120] warn that mutual liability can become coercive off-chain---a risk CWB avoids but at the cost of weaker social monitoring.

\subsection{Smart Contract Security and Governance}

Atzei et al.~[7] define core EVM attack classes (reentrancy, access control), informing \texttt{ReentrancyGuard} and RBAC. Chaliasos et al.~[150] find tools prevented only 8\% of real exploit losses---motivating a layered Slither--Mythril--Foundry--Tenderly pipeline (Section~\ref{sec:defense-in-depth}). Flash-loan governance attacks (Beanstalk, \$182M)~[158] drive CWB's timelock and snapshot voting. MEV risk on liquidations is mitigated via commit--reveal triggers.
...
MiCA~[R36, R37] and the GENIUS Act~[R38] require stablecoin reserves and issuer authorization---supporting USDC-first retail lending (Section~\ref{sec:stablecoin-first}). Beniiche~[R1] and Pasdar~[R2] frame the oracle problem; Chainlink accuracy varies by chain~[157], supporting single-chain deployment with commit--reveal fallback.

BIS/FSB data~[21, 25, 33] document correspondent-banking delays and costs. Kim and Kim~[55] and Hartmann and Hasan~[48] inform gas-cost modeling. Buterin et al.~[R30] and MicroSave~[R31] motivate on-chain credit passports (Section~\ref{sec:credit-passport}). Ozili~[161] shows Nigeria's eNaira had 98.5\% wallet inactivity---barriers are trust-based, supporting MFI-led onboarding. Sygnum~[R21], mBridge~[R39], FundsChain~[39], and Kinexys~[40] show institutional blockchain adoption without retail lending hierarchies.

\subsection{Zero-Knowledge Proofs for Compliance-Aware DeFi Identity}
\label{sec:lit-zkp-defi}

Recent ACM papers~[153, 154] and Piper et al.~[R8] show ZKP + DID/VC can enable privacy-preserving KYC. Together with zkAML~[R19], this motivates Contribution~4, deferred past MVT (Section~\ref{sec:future-work}).
...
\textbf{Synthesis.} (1)~\textit{The gap.} DeFi is flat (Werner~[1]); policy research supports tiered distribution (Tan~[5], BIS~[53]) but no prior work combines four-tier reserves, explainable ML, and group lending in one spec---Contribution~1. (2)~\textit{ML and security.} Behavioral ML and SHAP are workable for lending governance~[3, 4]; security tools alone are insufficient~[150]---Contribution~3 and RQ3. (3)~\textit{Institutional context.} MiCA/GENIUS mandate stablecoin transparency; mBridge/Agora provide settlement without lending hierarchies---stablecoin-first, MFI-intermediated design answers RQ2. (4)~\textit{Deferred work.} GNN, federated learning, ZKP deployment, and retail payment rails are documented but scoped to Future Work.

In short, the literature supports a four-tier architecture with oracle-mediated RF + Isolation Forest + SHAP, multi-directional capital flows, layered contract security, and stablecoin retail lending---while GNN, FL, full ZKP, and payment extensions remain post-MVT.
...
\rowcolor{RowShade}
Oracle integration (Chainlink Functions) & \tmarkPlanned{} Stretch (Phase~III; if supported on testnet) \\
ZKP KYC compliance layer & \tmarkPlanned{} Future Work (circuit specified; Section~\ref{sec:future-work}) \\
\rowcolor{RowShade}
Testnet deployment (Polygon Amoy / Ethereum Sepolia) & \tmarkPlanned{} Planned (Phases~I--IV; manifest Phase~IV) \\
AI Agent / MCP tool server (minimal MVT: 3--5 tools) & \tmarkPlanned{} Planned (Phase~III Must; \textbf{retail client only}) \\
\rowcolor{RowShade}
Full MCP tool server (17 banking tools) & \tmarkPlanned{} Future Work / stretch (post-MVT) \\
Bank-authority AI agent (NB/WB aggregate read tools) & \tmarkPlanned{} Future Work (post-MVT) \\
EIP-7702 session key management & \tmarkPlanned{} Future Work (wallet signing at MVT) \\
...
\rowcolor{RowShade}
PostgreSQL event listener (event indexing) & \tmarkPlanned{} Planned (Phases~I--II; stability-first baseline) \\
...
\caption[Layered blockchain and application stack]{Layered blockchain and application stack: L1 settlement (chosen prototype EVM testnet: Polygon Amoy PoS and/or Ethereum Sepolia); L2 smart-contract platform (Solidity 0.8.20, OpenZeppelin~v5, UUPS, TimeLock, RBAC); L3 data (PostgreSQL~16 in 3NF, Redis, IPFS); L4 API and services (Express, FastAPI, WebSocket, EIP-712); L5 presentation (React + ERC-4337 account abstraction). Polygon zkEVM remains an evaluated design preference, not the assumed prototype chain.}
...
Network & Polygon Amoy PoS and/or Ethereum Sepolia (prototype implementation) & Zero-cost public testnets for stability-first deployment. Polygon zkEVM is retained as an evaluated option in the platform comparison tables, not the default prototype narrative chain. \\
Consensus & Public-testnet consensus (PoS/L1 testnet) & Prototype implementation targets stable public EVM testnets (Amoy/Sepolia). ZK validity proof settlement (Polygon zkEVM) is treated as a design preference evaluated in the comparison tables, not assumed by the prototype deployment. \\
...
Testnet availability & Amoy (Polygon PoS), Sepolia (Ethereum) & Free faucets; EVM-identical behaviour; no real cryptocurrency required for prototype. \\
Security model & Public-testnet security model & Prototype uses public testnets (PoS/L1 testnet). ZK validity proof settlement is a design preference evaluated under Polygon zkEVM in the comparison section. \\
...
\caption[Oracle architecture]{Oracle architecture: (a)~off-chain ML feature engineering and SHAP explainability, (b)~commit--reveal score commitment (MVT path) with optional Chainlink Functions DON upgrade, (c)~on-chain \texttt{LoanController} state machine gating approval on \texttt{SCORE\_REVEALED}, with Chainlink Automation, Price Feeds, and Proof of Reserve.}
...
\caption[Smart-contract security controls]{Smart-contract security controls: (a) UUPS (ERC-1822) upgrade path with TimeLock-gated authorization; (b) EIP-712 sign-in (typed-data v4 with domain separation and 15-minute JWT); (c) granular per-function pause registry indexed by the event listener (Graph optional later) for transparent governance.}
...
\caption[MVT deliverable status at pre-thesis]{MVT deliverable status at pre-thesis submission versus final thesis targets. Pre-thesis evidence includes synthetic ML pipeline validation (MVT~8 partial), schema and workload documentation (MVT~4--6), and architectural specifications; testnet E2E deployment, live commit--reveal oracle, real BCCC retrain, and agent demo remain Phase~II--IV Must items.}
...
\caption[ML oracle commit--reveal gate]{ML oracle commit--reveal gate (MVT item~7): FastAPI scoring writes an audit row, commits $\mathrm{keccak256}(s \,\|\, \text{nonce})$ on-chain, then reveals $(s, \text{nonce})$ before \texttt{approveLoan} may succeed in \texttt{SCORE\_REVEALED}.}
...
\begin{figure}[H]
\centering
\ThesisIncludeGraphics[width=\linewidth,keepaspectratio]{Ch4_dataset-timeline}
\caption[Dataset timeline]{Dataset timeline: synthetic pipeline validation at pre-thesis; BCCC retrain after York access is granted.}
\label{fig:dataset-timeline}
\end{figure}

\begin{figure}[H]
\centering
\ThesisIncludeGraphics[width=\linewidth,keepaspectratio]{Ch4_ml-evidence-pipeline}
\caption[ML evidence pipeline]{Reproducible evidence pipeline: \texttt{synthetic\_bccc.csv} $\to$ Mac~M4 \texttt{run.sh} $\to$ \texttt{metrics.json} and model artifacts $\to$ \texttt{Diagrams/fig-ml-*} $\to$ Figure~\ref{fig:ml-eval}.}
...
\midrule
BCCC-DeFiFraudTrans-2025~[R47,108] & Primary (supervised) & 1,026,867 annotated DeFi transactions, 79 features (York BCCC~2025); \textbf{access requested, pending}; target train/test split for Random Forest; report precision, recall, F1, ROC-AUC on held-out test \\
...
\caption[ML evaluation and explainability benchmarks]{ML evaluation and explainability benchmarks on a \textbf{synthetic BCCC-shaped stand-in} (pipeline validation only; \texttt{evidence/mac-m4/metrics.json}, \texttt{synthetic: true}, seed~42): held-out precision/recall/F1/AUC bars and confusion matrix (top), global SHAP feature importance from TreeExplainer (bottom). Source artifacts: \texttt{Diagrams/fig-ml-metrics-bars}, \texttt{fig-ml-confusion-matrix}, \texttt{fig-ml-shap-importance}. BCCC-DeFiFraudTrans-2025 access has been requested from York University; figures and metrics will be retrained on the approved corpus when available. Domain-transfer limits to hierarchical CWB lending are noted below; these metrics establish RQ3 pipeline viability, not production fraud-detection claims on the full CWB loan book or on real BCCC data.}
\label{fig:ml-eval}
\end{figure}
...
\caption[RQ2 latency and gas measurement plan]{RQ2 latency and gas measurement plan (Phase~IV): per-stage targets for loan request, ML inference, commit--reveal oracle transactions, disbursement, and repayment on Amoy or Sepolia testnet.}
...
\caption[Planned development and verification toolchain]{Planned development and verification toolchain: Solidity contracts (Hardhat deploy) $\to$ Foundry fuzz/invariants $\to$ CI pipeline. Certora CVL proofs remain Future Work. Testnet deployment targets the chosen prototype testnet (Amoy/Sepolia) in Phase~IV.}
...
\midrule
Total Addressable Market (TAM) & Global DeFi lending (\$55B+ TVL [13]); cross-border remittances (\$860B [26]); SME financing gap (\$4.5T [20]) --- \textit{literature context only}; not a single deployable market & \$55B -- 5T+ (context) \\
...
\caption[AI banking agent request path]{AI banking agent request path (Phase~III): browser UI $\to$ Vite dev proxy $\to$ CWB Express API (SSE) $\to$ MCP tool server (minimal MVT tool subset) $\to$ Qwen3-8B inference $\to$ human confirmation gate $\to$ write-tool execution $\to$ on-chain transaction.}
