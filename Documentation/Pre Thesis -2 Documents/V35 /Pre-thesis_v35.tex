...
\begin{itemize}[noitemsep]
\item \textbf{Verification} --- Hardhat tests in \texttt{test/}; Vitest suites in \texttt{backend/tests/}; Foundry fuzz (Section~\ref{sec:foundry}).
\item \textbf{Validation} --- supervisor gates G1--G4; BCCC benchmarks (Table~\ref{tab:ml-bccc-results}, Figure~\ref{fig:ml-eval}).
\end{itemize}
...
Figure~\ref{fig:ml-explainability} summarises the explainability and anomaly-detection flow.
...
\subsection{Foundry Invariant and Fuzz Test Suite}
\label{sec:foundry}
...
\subsection{Implementation Evidence and Traceability}
...
This thesis makes four contributions: (1)~a \textit{compound} four-tier hierarchical DeFi architecture (reserve enforcement + explainable ML + mutual liability); (2)~on-chain programmable joint-liability group lending with over-indebtedness controls (Section~\ref{sec:over-indebted}); (3)~oracle-mediated AI/ML integration with SHAP and anomaly reports (Section~\ref{sec:ml-explainability}), subject to the BCCC domain-transfer gap (Section~\ref{sec:ml-workload}); and (4)~a specified compliance-aware identity pathway (zkKYC, W3C DID/VC) deferred to Future Work. Examiner limitations G1--G6 are consolidated in Section~\ref{sec:research-limitations}.

With specified architecture (Chapter~\ref{ch:design-alternatives}), empirical evaluation (Chapter~\ref{ch:evaluation}), and economic analysis (Section~\ref{sec:economic-analysis}), and a phased implementation roadmap, CWB is a structurally distinct contribution to blockchain-based development finance at the \textit{specification and prototype} stage.
...
\texttt{allocateCapital} and \texttt{recordRepayment} both transfer ETH. OpenZeppelin's \texttt{nonReentrant} guard stops the contract from being called again before the first call finishes, which reduces reentrancy risk during those transfers. See Section~\ref{sec:kinked-rate} for the full safety review.

\textbf{Design annotation 2 --- \texttt{AccessControl} mapping to RBAC design.}
Each bank role (e.g., \texttt{NATIONAL\_BANK\_ROLE}) is assigned to specific wallet addresses by the World Bank admin. This implements the four-tier access rules in Section~\ref{sec:prototype-scope}: national banks call \texttt{recordRepayment}; only the World Bank admin calls \texttt{allocateCapital}.

\textbf{Design annotation 3 --- Events enable off-chain indexing for the analytics layer.}
The contract logs major actions such as \texttt{CapitalAllocated}, \texttt{RepaymentReceived}, and \texttt{ReserveRatioUpdated}. The backend listener copies these logs into PostgreSQL so dashboards and the fraud-detection model can use them without reading the blockchain on every query.
