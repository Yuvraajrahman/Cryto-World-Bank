...
from pipeline.preprocess import build_preprocessed_bundle
...
def load_and_subsample(tc: TrainConfig, cfg: dict) -> tuple[pd.DataFrame, str, list[str]]:
    ...
    # Keep full frame for group split (address); feature selection happens in build_preprocessed_bundle.
    return df, "_label", []


def run_preprocess(tc: TrainConfig, state: dict, cfg: dict) -> dict:
    ...
    df, label, _ = load_and_subsample(tc, cfg)
    bundle = build_preprocessed_bundle(df, label, cfg, tc.sample_size, tc.seed)
    feature_cols = bundle["feature_cols"]
    print(
        f"[preprocess] {bundle['sample_size']} rows × {len(feature_cols)} features "
        f"(fraud rate {bundle['fraud_rate']:.3f}, split={bundle['split_mode']})"
    )
...
def run_evaluate(tc, rf, iso, meta, bundle, state) -> dict:
    ...
    metrics = {
        ...
        "confusion_matrix": cm,
        "fraud_rate_sample": bundle.get("fraud_rate"),
        "split_mode": bundle.get("split_mode", "unknown"),
        "balanced_sample": bundle.get("balanced_sample", False),
        "note": "BCCC behavioral features; leakage columns (flag) removed; StandardScaler on train.",
    ...
    if bundle.get("scaler") is not None:
        joblib.dump(bundle["scaler"], tc.artifacts / "scaler.joblib")
...
def run_training(cfg: dict, csv_path: Path, synthetic: bool = False) -> dict:
    ...
    bundle = run_preprocess(tc, state, cfg)
    pbar.update(1)
