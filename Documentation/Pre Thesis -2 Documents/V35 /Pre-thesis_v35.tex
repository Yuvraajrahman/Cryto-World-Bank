...
Besides lending down to clients, banks in the hierarchy also need to move capital sideways (between peers at the same tier), upwards (returning surplus to a parent bank), and together (sharing large loans). Figure~\ref{fig:multi-entity-ops} gives an overview. The subsections below describe five on-chain mechanisms for these flows; batch settlement netting (\texttt{NettingEngine}) is left for future work (Section~\ref{sec:future-work}).

\subsubsection{InterBankLendingPool: Same-Tier Lending}
\label{sec:interbank-pool}

When two banks at the same tier need short-term liquidity from each other, they use an \texttt{InterBankLendingPool} (IBLP)---one pool per tier for National Banks (\texttt{IBLP\_NB}) and Local Banks (\texttt{IBLP\_LB}). Loans run for 1, 7, or 30~days in USDC, and only banks with an active role in that tier may participate (Section~\ref{sec:defense-in-depth}).

\paragraph{Interest rate.}
The pool borrow rate $r_{\text{IB}}(U)$ uses the same kinked curve as retail lending (Section~\ref{sec:kinked-rate}, Formulas~18--19), with utilisation kink $U^{*}_{\text{IB}} = 0.90$. It may never exceed the rate at which the parent tier lends downward, minus a small spread $\delta$ that prevents arbitrage:
\begin{equation}
r_{\text{IB}}(U) \le r_{\text{downward}}(U) - \delta
\end{equation}
\noindent $\delta$ is set by governance and checked on every rate update via \texttt{getCurrentBorrowRate()}.

\paragraph{When a borrower defaults.}
If an interbank loan is not repaid at maturity, losses are covered in order: first from the borrower's reserve buffer, then from the \texttt{InsuranceFund}, and finally by the parent tier. Serious cases can trigger a role suspension through the TimeLock.

\subsubsection{Upward Surplus Repatriation}
\label{sec:upward-flow}

A bank that holds more reserves than required (at least 20\% above the minimum) may place the extra funds with its parent bank through \texttt{UpwardDepositFacility} and earn deposit interest. The upward deposit rate must stay below the downward lending rate minus the same spread $\delta$:
\begin{equation}
r_{\text{up}} < r_{\text{down}} - \delta
\end{equation}
\noindent Withdrawals are blocked if they would push the bank below its minimum reserve ratio. Other limits: no more than 20\% of upward deposits may be withdrawn per day, and total upward exposure may not exceed 30\% of on-chain assets.

\subsubsection{Syndicated and Club Lending}
\label{sec:syndicated-lending}

A single bank may not fund a very large loan alone. In a \texttt{SyndicatedLoan}, several banks share the exposure: one Lead Arranger, one or more Co-Lenders, and the Borrower (always a retail client via a Local Bank). The deal closes only when total commitments reach at least 90\% of the loan amount and Co-Lenders representing at least 75\% of committed capital agree. Each bank's share is stored on-chain; principal, interest, and any recovery after default are split in proportion to those shares.

\subsubsection{Senior--Junior Tranched Lending Pools}
\label{sec:tranched-pools}

A \texttt{TranchedPool} divides pooled capital into two parts. Senior tranche holders take lower risk and receive lower returns; junior tranche holders take higher risk and receive higher returns but absorb losses first. On repayment, senior interest is paid before junior interest, then senior principal before junior principal. Losses follow the reverse order. The junior-to-senior ratio must fall between 10/90 and 30/70 when the pool is created.

\subsubsection{Cross-Tier Treasury FX Swap}
\label{sec:treasury-fx}

Banks holding different assets can exchange them atomically through \texttt{TreasurySwap}, using oracle prices:
\begin{equation}
\text{Amount}_{\text{to}} = \text{Amount}_{\text{from}} \times \frac{P_{\text{from}}}{P_{\text{to}}} \times (1 - \text{spread})
\end{equation}
\noindent $P_{\text{from}}$ and $P_{\text{to}}$ come from Chainlink feeds; the treasury charges a spread of 5--10~bps. After the swap, the bank must still meet its minimum reserve ratio. Swaps above \$1M require two signatures on a Safe multisig.

\subsubsection{Multilateral Settlement Netting Engine (Future Work)}
\label{sec:netting-settlement}

\noindent\textit{Future Work.} A \texttt{NettingEngine} would combine many bilateral obligations from IBLP and \texttt{TreasurySwap} into fewer net payments, with support for partial settlement and a dispute window. This is described in Section~\ref{sec:future-work}; only placeholder database tables (\texttt{NETTING\_BATCH}, \texttt{NETTING\_ENTRY}) exist in the schema for now.
...
Screen layouts for the retail borrower flow and the bank approver dashboard are kept in \textbf{Appendix~F} (Section~\ref{app:ui-prototypes}) as Figma exports; Figures~\ref{fig:ui-wireframe-retail} and~\ref{fig:ui-wireframe-approver} will be inserted there before final submission.
