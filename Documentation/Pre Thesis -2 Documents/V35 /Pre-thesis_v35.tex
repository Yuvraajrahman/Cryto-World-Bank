...
MiCA~[58, 59] and the GENIUS Act~[60] require stablecoin reserves and issuer authorization---supporting USDC-first retail lending (Section~\ref{sec:stablecoin-first}). Beniiche~[40] and Pasdar~[41] frame the oracle problem; Chainlink accuracy varies by chain~[92], supporting single-chain deployment with commit--reveal fallback.
...
\rowcolor{RowShade}
Chainlink accuracy [92] & Polygon high oracle accuracy vs Ethereum/zkSync under volatility & Single-chain Polygon deployment; oracle fallback & Infrastructure \\
...
\begin{enumerate}[noitemsep]
\item Deploy \texttt{SavingsVault} and \texttt{LoanContract} on Sepolia (Table~\ref{tab:phase-deliverables}).
