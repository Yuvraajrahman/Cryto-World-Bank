...
\item \textbf{Credit Passport coaching.} The agent explains exactly what the client needs to reach the next credit tier using \texttt{get\_credit\_score} combined with the tier schedule (Table~\ref{tab:credit-tier-schedule}): \emph{``You need 2~more on-time repayments to reach Gold tier. Your borrowing rate will then drop by 1.0\%.''}
