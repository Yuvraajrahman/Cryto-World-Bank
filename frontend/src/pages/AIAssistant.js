        ...
        if (/apply.*loan|loan.*apply|borrow\s+\d/i.test(body)) {
            ...
            try {
                ...
                if (res.type === "confirmation_required" && res.confirmationId) {
                    ...
                    setMessages((prev) => prev.map((x) => x.id === botId ? { ...x, body: res.message ?? "Please confirm to submit." } : x));
                }
                else {
