...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-blockchain-stack}
...
\vspace{1.5cm}
\noindent\textit{[Comments for this section: describe what happens inside the Express.js gateway once the request lands---middleware order (auth, validation, rate limiting), how the request is sequenced over the network (REST call vs.\ the SSE channel used in Section~\ref{sec:optional-agent} step~1), and how the gateway decides whether a request needs an on-chain call at all.]}
\vspace{1.5cm}
...
\begin{figure}[H]
\centering
\BalancedDiagram{1_InterBankLendingPool}
...
\begin{figure}[H]
\centering
\BalancedDiagram{2_UpwardDepositFacility}
...
\begin{figure}[H]
\centering
\BalancedDiagram{3_SyndicatedLoan}
...
\begin{figure}[H]
\centering
\BalancedDiagram{4_TranchedPool}
...
\begin{figure}[H]
\centering
\BalancedDiagram{5_TreasurySwap}
...
\begin{figure}[H]
\centering
\BalancedDiagram{6_NettingEngine}
