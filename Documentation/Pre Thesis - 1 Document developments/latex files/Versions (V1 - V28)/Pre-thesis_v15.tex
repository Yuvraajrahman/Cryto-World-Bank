%% =============================================================================
%% Pre-thesis 1 -- Crypto World Bank -- v15 (BRACU front matter + ACM body)
%% -----------------------------------------------------------------------------
...
%% Planned changes integrated this version (from Documentation/Planned changes):
%%   * Stablecoin-first lending denomination + MiCA/GENIUS regulatory anchor
%%   * On-chain Liquidation Engine (Health Factor enforcement)
%%   * SavingsVault and FixedDeposit deposit-mobilization modules
%%   * AI/ML wired into loan decisions via commit-reveal oracle
%%   * GNN extension for relational fraud detection
%%   * Federated learning across National/Local Banks
%%   * Certora formal verification of reserve invariants
%%   * Foundry invariant + fuzz testing alongside Hardhat
%%   * Five-layer defense-in-depth security architecture
%%   * Tiered KYC and ERC-4337 account abstraction for retail onboarding
%%   * zkAML circuit alongside ZKP KYC; W3C DID/VC layer
%%   * On-chain credit passport SBT (Buterin et al. 2022)
%%   * Cross-chain bridge architecture (LayerZero/Axelar/CCIP)
%%   * UUPS proxy, TimeLock governance, role expiry, granular pause
%%   * MiCA + GENIUS Act compliance mapping; Bangladesh regulatory reality
%%   * Over-indebtedness risk explicit in group lending
%%   * Multi-Entity and Cross-Tier Capital Operations (Chapter 3, new section):
%%       - InterBankLendingPool fully specified (rate model + settlement + default cascade)
%%       - Upward surplus repatriation fully specified (LB -> NB -> WB)
%%       - Syndicated / club lending across multiple NBs / LBs
%%       - Senior / junior tranched pools (subordination waterfall)
%%       - Treasury FX swap (cross-tier asset swap)
%%       - Multilateral settlement netting engine
%%     and the resulting ERD entities, Sprint plan, Appendix B contract list,
%%     Future Work, and unimplemented-modules list updated for consistency.
...
\hline
\end{tabular}
\TableScreenshotEnd{Tiered Borrower Access Rules by Borrower Type and Loan Size}
\end{table}
...
\item \textbf{Fractional reserve via the four-tier hierarchy.} The reserve ratio enforced at every tier (Formula RR) provides a multiplicative lending capacity: stablecoin deposits at Tier~1 fund proportionally larger downstream lending volumes, recreating the multiplier effect of traditional banking while keeping the multiplier transparent and on-chain.
\end{enumerate}

The supply concern that originally motivated this subsection is therefore reframed: scarcity is a property of base-layer crypto-assets, not of the lending denomination. Fiat-backed stablecoins issued under MiCA-/GENIUS-compliant reserves provide the elastic, audit-friendly supply a retail banking platform requires, while the hierarchical reserve architecture provides the credit-multiplier behavior. Borrower-side currency risk is removed, deposit-side regulatory compliance is preserved, and the protocol does not depend on the monetary discretion of any single issuer.
...
\hline
\end{tabular}
\TableScreenshotEnd{Market Sizing: TAM, SAM, and SOM for the Crypto World Bank}
\end{table}
...
\hline
\end{tabular}
\TableScreenshotEnd{Target Retail Customer Segment Profile and Constraints}
\end{table}
...
\hline
\end{tabular}
\TableScreenshotEnd{Partner Ecosystem: External Dependencies and Integration Points}
\end{table}
...
\hline
2 & Tan 2023 (IMF) [5] & CBDC \& inclusion & Hierarchical distribution model for CBDC in developing economies & Section~\ref{sec:bangladesh-reg} \\
\hline
...
Hassan et~al.~(2022)~[49] introduced privacy-preserving fraud detection via federated learning on the blockchain. The PrivChain-AI study~[R27] in \textit{Nature Scientific Reports} (December~2025) demonstrates the same pattern with 94.7\% fraud-detection accuracy across multiple banks, while Abbassi et~al.~(2025)~[R28] and FED-SPFD~[R29] document the regulatory benefits of FL for cross-border fraud intelligence. The Crypto World Bank's four-tier structure---where National and Local Banks are independent legal entities that cannot share raw transaction data---makes federated learning architecturally appropriate rather than merely fashionable: each Local Bank trains a local fraud model on its own borrower data, parameter updates are aggregated at the National Bank tier via FedAvg, and no raw records leave the bank. The detailed FL pipeline is given in Section~\ref{sec:fl-fraud}.
...
\hline
LOAN\_REQUEST & \texttt{borrower\_id, local\_bank\_id} $\to$ status, amount, \ldots & One active request per borrower per bank. \\
\hline
BORROWING\_LIMIT & \texttt{borrower\_id} $\to$ \texttt{six\_month\_limit, one\_year\_limit}, \ldots & 1:1 with BORROWER. \\
\hline
BORROWER & \texttt{wallet\_address} $\to$ \texttt{borrower\_id}, all attributes & Wallet address is a unique candidate key. \\
\hline
...
The zkAML circuit~[R19] (IACR ePrint 2025/465) proves three properties of a wallet without revealing its transaction graph: (1) the wallet has not transacted with any address on a public sanctions list (Chainalysis / TRM Labs); (2) the wallet's 30-day inflow does not exceed a governance-set velocity threshold; and (3) the wallet has not been flagged by the platform's behavioral biometrics layer. The published benchmark for zkAML is 55~TPS on public networks with 226\,ms proof-generation latency, both compatible with retail-tier flows on Polygon. The on-chain verifier is a sibling of \texttt{KYCVerifier} that exposes \texttt{AMLVerifier.verify(proof, [wallet, blockHeight])}. Both verifiers must return true before a wallet's BORROWER role is activated.
...
\hline
Group Borrower & Tier 4 & Level 1 + group verification & ERC-4337 abstracted & Off-chain + on-chain group contract \\
\hline
...
The Health Factor formula (HF in the List of Formulas) is operationally meaningless without a contract that monitors it and an actor incentivized to act on the result. The original v10 thesis listed HF but contained no liquidation mechanism: when HF dropped below 1.0, the system relied on a human approver noticing and reacting, which is a ledger, not a bank. The Crypto World Bank therefore specifies a dedicated \textbf{LiquidationEngine} contract that closes this gap.

\paragraph{Liquidation life-cycle.}
\begin{enumerate}[noitemsep]
\item A borrower's collateral price is updated from the Chainlink price feed at every block; HF is recomputed in view-only fashion (no gas cost to the borrower).
\item Any external participant may call \texttt{liquidate(loanId)} when HF $< 1.0$.
\item The contract verifies HF on-chain, transfers a configurable portion of the collateral (typically equal to the outstanding debt plus a 5--8\% liquidation bonus, following Aave~v3 parameters) to the liquidator, and updates the borrower's on-chain credit record.
...
The platform issues each KYC-verified borrower a non-transferable on-chain credential---a Soulbound Token (SBT) following the Buterin, Hitzig, and Weyl framework~[R30]---that encodes their cumulative repayment record in a schema that any compatible lending protocol can read. The credential is updated automatically whenever an installment is repaid in full, downgraded after a defined number of missed installments, and revoked on default. Three concrete benefits follow.

First, the SBT solves the cold-start problem at the bank-switching boundary: a borrower who has repaid three loans at a Local Bank in Sylhet can apply for a larger loan at a Local Bank in Dhaka without re-establishing trust from zero. Second, the SBT is the on-chain primitive that the over-indebtedness control (Section~\ref{sec:over-indebt}) relies on: a borrower's open-loan count across the platform is publicly readable from their SBT, so simultaneous-loan caps are enforceable without an external credit bureau. Third, the schema is intentionally minimal (`creditScore`, `openLoans`, `completedCycles`, `lastDefault`, `riskTier`) so other lending protocols that adopt the standard can read the credential, allowing progressive lending to follow a borrower across protocols. MicroSave Consulting (December 2025)~[R31] documents that Bangladesh's microfinance sector is actively shifting toward performance-based credit identity; the SBT is the technical implementation of that shift on the Crypto World Bank platform.

\section{Cross-Chain Bridge Architecture}
\label{sec:bridge}

A multi-chain deployment (Polygon Amoy + Ethereum Sepolia in the prototype, with mainnet successors planned) without cross-chain state consistency is a liability: if a borrower has an active loan on Polygon and attempts to open a second loan by routing through Ethereum, the hierarchical borrowing-limit control could be bypassed. v15 makes the bridge architecture explicit rather than leaving it as ``portable.''
...
The hierarchical control is preserved across chains by requiring that all loan state for a given borrower live on a single chain (the chain of the originating Local Bank). Cross-chain messages are restricted to (a) reserve-ratio updates from World Bank Reserve to National Banks across chains, and (b) credit-passport SBT mirroring across chains so that a borrower's credential is readable on any chain even though their loans are not. This pattern keeps borrowing-limit enforcement single-chain and avoids the consensus problem of cross-chain debt.
...
\item \textbf{On-chain consent vote.} Each Co-Lender's wallet must call \texttt{confirmTerms(uint256 syndicateId)} after subscription; this is a mandatory unanimous step that gates final disbursement. A Co-Lender that subscribes but does not confirm forfeits its escrow up to a small bond.
\item \textbf{Disbursement.} On unanimous confirmation, the contract atomically transfers the aggregate to the Borrower in a single transaction. Each Co-Lender's exposure share is recorded as \texttt{shareBps[lender][syndicateId]}.
\item \textbf{Interest accrual + repayment.} Borrower installments flow into the \texttt{SyndicatedLoan} contract; the contract distributes principal + interest pro-rata to each Co-Lender on every installment via the basis-points share registry. The Lead Arranger receives its underwriting fee on the first repayment.
...
\textbf{Role expiry timestamps.} Role grants in the system are not permanent. Every \texttt{grantRole} call records an expiry timestamp; every privileged function call checks \texttt{require(roleExpiry[msg.sender][role] > block.timestamp)}. This single-line addition resolves a real compliance concern: banks lose licenses, employees change positions, and KYC certifications must be renewed annually (Section~\ref{sec:tiered-kyc}). The borrower KYC level is bound to a one-year expiry by default; bank operator roles are bound to the institution's current registration validity; the World Bank Admin role itself is renewed annually by a governance vote so that even the top-of-hierarchy authority is not effectively permanent.
...
The system involves four primary actors---Borrower, Bank Approver, World Bank Admin, and National Bank---across 29 use cases (Figure~\ref{fig:usecase}). Note that the four primary actor classes in the use-case diagram are a simplification of the seven-user taxonomy defined in Section~\ref{sec:onboarding}: \emph{Visitor} and \emph{Registered Retail User} are pre-conditions for the BORROWER role rather than independent diagram actors, and \emph{Local Bank Operator} / \emph{Approver} are merged into the Bank Approver role at the diagram level.

\begin{figure}[H]
\centering
\OnePageDiagram{fig-usecase-actors.pdf}
\caption{Use-case diagram with the seven-user taxonomy of Section~\ref{sec:onboarding}: \emph{Visitor}, \emph{Retail User}, \emph{Borrower}, \emph{Approver}, \emph{Local-Bank Operator}, \emph{National-Bank Admin}, and \emph{World-Bank Admin}, mapped to 20 representative use cases covering KYC, deposits, lending, multi-entity operations, and regulator oversight.}
\label{fig:usecase}
\end{figure}
...
\caption{Auxiliary activity flows: (a) borrower--bank chat over authenticated WebSocket, (b) RAG-augmented AI chatbot with hallucination guard, (c) live market-data retrieval and pre-filled loan sizing.}
\label{fig:act-aux}
...
\caption{Sequence diagram for the loan-approval flow with reject alternative: borrower submits an EIP-712 signed request, ML oracle returns a SHAP-explained score via commit-reveal, and the approver either approves (revealing the commit) or rejects (with reason logged on-chain).}
...
\caption{Sequence diagrams: (a) borrower--bank chat over an authenticated WebSocket, and (b) AI chatbot pipeline (ChromaDB top-$k$ retrieval, QLoRA-tuned LLM, hallucination guard, citation-anchored answer).}
...
\hline
L3 & AI/ML security & Random Forest fraud probability + Isolation Forest anomaly score + SHAP explanation per loan, delivered via commit-reveal oracle; behavioral biometrics features (session-time, page-views, terms-acceptance latency); transaction-graph distance to flagged wallets. \\
\hline
...
\hline
Sybil abuse (retail / group) & L1+L3 & Attackers create many wallets to bypass limits or build fraudulent groups & On-chain RBAC; rate limits per wallet; ZKP-KYC gating; GNN-based wallet-graph clustering; behavioral biometrics. \\
\hline
...
\hline
LLM hallucination on regulatory questions & L3 & 9B-LLM gives a confidently wrong compliance answer to a borrower & RAG over current policy docs; refusal layer on regulated topics; red-teaming protocol (Section~\ref{sec:llm-eval}). \\
\hline
...
\item \textbf{Risk scoring.} A Random Forest classifier returns a fraud probability $p_f \in [0,1]$; an Isolation Forest returns an anomaly score $a$; the composite risk is $s = 0.7\, p_f + 0.3 \min(a/0.5,\, 1.0)$.
\item \textbf{Commit-reveal oracle.} The service publishes \(h = \mathrm{keccak256}(s \,\|\, \text{nonce})\) to the LoanController contract via \texttt{commitRiskScore}, then reveals $(s, \text{nonce})$ in a subsequent transaction. Section~\ref{sec:oracle-architecture} covers the cryptographic detail.
\item \textbf{Decision threshold.} The contract applies a governance-configured threshold: $s < 0.4$ auto-approves under the approver's discretion, $0.4 \le s < 0.7$ requires manual review, and $s \ge 0.7$ rejects the loan with an on-chain reason code.
\end{enumerate}

\paragraph{Behavioral biometrics features.}
A 2025 research-backed extension feeds a low-weight ``soft'' signal into the Isolation Forest from per-session interaction patterns---time between wallet connection and application submission, number of page views before applying, terms-acceptance latency (acceptance in under 3~s is a strong bot signal), and IP-geolocation consistency with the declared jurisdiction. None of these features require additional user data beyond what the application would already collect for usability analytics, and they are not stored on-chain.

\paragraph{SHAP explainability output to borrowers.}
Each loan decision produces a SHAP value vector, and the top-3 contributing features are stored in \texttt{AI\_ML\_SECURITY\_LOG} together with the decision. The borrower-facing UI displays a plain-language explanation rather than the raw SHAP score---for example, \emph{``Your application was declined primarily because: (1) no on-chain repayment history (60\%); (2) requested amount exceeds income estimate (30\%); (3) group membership not yet verified (10\%).''} This satisfies the EU AI Act Article~86 transparency requirement for AI-assisted financial decisions and converts the formal SHAP design choice~[R7] into an end-user benefit.

\section{Graph Neural Network Extension}
\label{sec:gnn-extension}

The Random Forest detector treats each transaction independently; DeFi fraud is largely \emph{relational}, appearing in the wallet-interaction graph. A baseline Graph Neural Network (GraphSAGE) is added as an ablation: nodes are wallets, edges are transactions or shared group-membership, node features are the same 18 behavioral features used by the Random Forest. The model is trained on a synthetic transaction graph augmented with public Elliptic-dataset patterns. Cheng et~al.~(2024)~[R24] and Wang and Wang~(2025)~[R26] both report sustained advantages of GNN over flat ML on this task (>70\% detection of illicit transactions at <1\% false positive). The expected contribution to the thesis is not a state-of-the-art result but a controlled ablation that quantifies the marginal value of relational features over flat behavioral features---a publishable comparison aligned with the SoK literature.
...
The revenue projection of \$224.6\,M in Chapter~5 assumes 1~World Bank, 5~National Banks, and 50~Local Banks at near-full capacity. v15 makes this projection empirically grounded rather than a single-point estimate by specifying a Mesa-based agent-based simulation (ABM) with three populations: depositors (arrival rate $\lambda_d$, withdrawal rate $\mu_d$), borrowers (loan-demand process, default hazard $\eta$), and banks (Tier~2 / Tier~3 with reserve-ratio policies). The ABM is calibrated against Bangladesh microfinance arrival statistics (BRAC 2025: 92\% income increase; 41.56\,M MFI accounts) and stress-tested under a 30\% simultaneous-withdrawal scenario. Aggregate outputs are reported as ranges with confidence intervals rather than a single number---the empirical grounding that the Sygnum 2026 report calls for~[R21].
...
The real-time dashboard pipeline closes the gap between contract events and the borrower / approver UI. Three components compose:
...
A Node.js service listens on \texttt{wss://polygon-amoy} for the same set of events and pushes notifications to the borrower's browser via Socket.IO. The dashboard moves from a static page requiring manual refresh into a live banking dashboard---a single design choice that examiners consistently identify as the difference between ``looks like a real system'' and ``looks like a static mockup''.
...
Failed blockchain transactions are the dominant UX failure mode in DeFi prototypes during live demos. v15 specifies an explicit five-state transaction machine on the frontend (\texttt{idle} $\to$ \texttt{pending\_signature} $\to$ \texttt{submitted} $\to$ \texttt{confirming} $\to$ \texttt{success / error}) with distinct visual feedback at each state. User-rejection (MetaMask code 4001) is distinguished from contract revert (\texttt{CALL\_EXCEPTION}) and from network error; each receives its own actionable error message. This is a thirty-line React hook that converts ``the demo failed silently'' into ``the borrower can see exactly what went wrong and retry''.
...
\item \textbf{RQ3 (analytics support):} model evaluation using standard metrics (precision, recall, F1, AUC) on available datasets and controlled synthetic cases, alongside qualitative assessment of SHAP explanations; an ablation study (GNN vs.\ Random Forest) quantifies the marginal value of relational features (Section~\ref{sec:gnn-extension}).
\item \textbf{RQ4 (technical viability):} end-to-end deployment verification on public testnets with integration tests covering wallet connection, loan request, approval, repayment, and role-based access control; Foundry invariant suite reports the proportion of fuzz runs that hold each invariant (Section~\ref{sec:foundry}).
\item \textbf{RQ5 (banking extensions):} design-level validation via specification completeness and interface contracts for deposit products and group lending, plus prototype demonstrations of selected mechanisms where implemented; the Mesa-based ABM (Section~\ref{sec:abm-sim}) produces an aggregate stress-test outcome under 30\% simultaneous withdrawal.
\end{itemize}
...
\caption{SDLC stage mapping with Agile sprints. Requirements / Architecture / Design are covered by Pre-thesis~1; Implementation, Verification, Validation, and Maintenance span Sprints~1--3 of the final thesis phase, with Foundry invariants, Certora proofs, Mesa-based agent-based simulation, and Tenderly runtime monitoring layered on top.}
...
\hline
\end{tabular}
\TableScreenshotEnd{Target Customer Segment: Retail Borrower Profile and Product Fit}
\end{table}
...
\hline
\end{tabular}
\TableScreenshotEnd{Revenue Projection Assumptions and Sensitivity Inputs}
\end{table}
...
\caption{Annual revenue projection by platform tier (USD millions, at \$2{,}500/ETH conservative mid-point).}
...
\begin{table}[H]
\centering
\caption{MiCA and GENIUS Act compliance mapping. Each row identifies a regulatory article relevant to a stablecoin-denominated retail lending platform and the corresponding CWB design control.}
...
To ground the platform's financial inclusion claims in a concrete context, this section evaluates access across six dimensions for a hypothetical borrower: a small-scale trader in rural Sylhet, Bangladesh, seeking a 50,000 BDT ($\approx$\$430) working capital loan.
...
\item[\textbf{6. Group lending fit.}] The solidarity group lending model (GroupLendingPool, planned) directly mirrors the BRAC and Grameen Bank methodology already familiar to rural Sylhet borrowers. Programmable mutual liability replaces community social pressure with smart contract enforcement, potentially improving both collection reliability and borrower protection.

\end{description}

In aggregate, the Crypto World Bank is accessible in principle to a rural Sylhet borrower today (device, connectivity, cost) with two critical gaps requiring resolution for a production deployment: Bengali-language interface and stablecoin denomination to eliminate currency risk.
...
\item \textbf{AI/ML pipeline integration.} Wire Random Forest + Isolation Forest + SHAP into the on-chain loan decision via commit-reveal oracle (Section~\ref{sec:aiml-support}); deliver borrower-facing SHAP plain-language explanations (Section~\ref{sec:aiml-support}).
...
\item \textbf{Agent-based economic feasibility simulation.} Build the Mesa-based ABM of Section~\ref{sec:abm-sim}, calibrated against Bangladesh microfinance statistics and stress-tested under a 30\% simultaneous-withdrawal scenario.
...
\item \textbf{Mainnet and regulatory sandbox.} Move from testnet to a controlled mainnet deployment under the Bangladesh Bank Regulatory FinTech Facilitation Office sandbox (Section~\ref{sec:bangladesh-reg}); pilot with a partner microfinance institution; align with the Bangladesh CBDC trajectory.
...
\item \textbf{SyndicatedLoan Contract} (Section~\ref{sec:syndicated-lending}): syndicate proposal, subscription window, unanimous on-chain consent vote, atomic disbursement, pro-rata interest and recovery distribution; supports cross-tier syndication (mixed NB / LB co-funders).
