...
evaluated.
This vulnerability class is corroborated by documented industry incidents: a February~2026 Microsoft security advisory on MCP-based agent runtimes identified permissive default tool-execution policies as a critical runtime risk in agentic financial workflows~[R55], consistent with OWASP LLM01 (Prompt Injection) as the highest-priority threat in agentic systems~[R54]. These findings reinforce that the mandatory human-confirmation gate and \texttt{injection\_scan\_flag} scanning specified for the CWB agent (Section~\ref{sec:optional-agent}) target an actively exploited vulnerability class rather than a hypothetical risk.
...
The relational database schema comprises 46 normalized entities in Third Normal Form (3NF): 26 core prototype entities (Table~\ref{tab:db-entities}), six primary extension entities (Table~\ref{tab:db-entities-ext}), and 14 Phase~II--III banking-product and multi-entity entity stubs (Table~\ref{tab:db-entities-ext2}). Stub entities are shown in the ERD figures and fully specified in Section~\ref{sec:multi-entity}; their physical schemas will be completed during implementation. Figure~\ref{fig:core-system-graph} presents the core entity-relationship overview, Figure~\ref{fig:erd-extended} presents extended product and multi-entity entities, and Figure~\ref{fig:eer} presents the Enhanced Entity-Relationship (EER) diagram with specialisation and weak-entity constructs.

\begin{figure}[H]
\centering
\BalancedDiagram{fig-erd-core.pdf}
\caption[Core system entity graph]{Core system graph showing entity relationships across the four-tier banking hierarchy (\texttt{WORLD\_BANK} $\to$ \texttt{NATIONAL\_BANK} $\to$ \texttt{LOCAL\_BANK} $\to$ \texttt{BANK\_USER}), the central \texttt{BORROWER} entity, and the lending-lifecycle sub-graph (\texttt{LOAN\_REQUEST}, \texttt{LOAN}, \texttt{INSTALLMENT}, \texttt{TRANSACTION}, \texttt{INCOME\_PROOF}, \texttt{CHAT\_MESSAGE}, \texttt{AI\_ML\_SECURITY\_LOG}, \texttt{CREDIT\_PASSPORT}).}
...
\begin{table}[H]
\centering
\caption[Database entity summary (26 core entities)]{Database entity summary (26 core entities): primary keys and roles.}
...
\midrule
WORLD\_BANK & \texttt{world\_bank\_id} & Top-level reserve holder; global lending parameters. On-chain binding: \texttt{contract\_address}, \texttt{chain\_id}, \texttt{last\_synced\_block}, \texttt{deployment\_tx\_hash}. \\
NATIONAL\_BANK & \texttt{national\_bank\_id} & Country-level banks; borrow from World Bank. On-chain binding: \texttt{contract\_address}, \texttt{chain\_id}, \texttt{last\_synced\_block}, \texttt{deployment\_tx\_hash}. \\
LOCAL\_BANK & \texttt{local\_bank\_id} & City-level banks; borrow from National Bank and lend to users. On-chain binding: \texttt{contract\_address}, \texttt{chain\_id}, \texttt{last\_synced\_block}, \texttt{deployment\_tx\_hash}. \\
BANK\_USER & \texttt{bank\_user\_id} & Bank staff with role-based permissions (approve/reject loans). Nullable FK \texttt{world\_bank\_id} for Tier~1 governance actors. \\
BORROWER & \texttt{borrower\_id} & End borrowers requesting and repaying loans (UK: \texttt{wallet\_address}).\footnote{The prototype uses \texttt{BORROWER} / \texttt{borrower\_id} consistently; the final migration may rename to \texttt{CLIENT} / \texttt{client\_id}. The \texttt{UNIQUE} constraint on \texttt{wallet\_address} assumes single-chain operation; Phase~IV CCIP requires \texttt{(wallet\_address, chain\_id)}.} \\
...
TRANSACTION & \texttt{transaction\_id} & Financial transaction records. \texttt{transaction\_type} (\texttt{LOAN\_DISBURSEMENT}, \texttt{INSTALLMENT\_PAYMENT}, \texttt{INTERBANK\_TRANSFER}, \ldots), \texttt{direction} (\texttt{CREDIT}/\texttt{DEBIT}), \texttt{counterparty\_type}, \texttt{blockchain\_tx\_hash}, \texttt{event\_log\_id}. \\
...
CHAT\_MESSAGE & \texttt{message\_id} & Human-to-human and human-to-system messages in the loan workflow (client query $\to$ bank approver reply). FK to \texttt{LOAN\_REQUEST} and/or \texttt{LOAN}. Not agent-specific. \\
AGENT\_CONVERSATION\_TURN & \texttt{turn\_id} & Each turn of the optional Qwen3-8B agent conversation. FK \texttt{session\_id} $\to$ \texttt{SESSIONS}. Stores \texttt{role} (user/assistant/system), \texttt{content\_summary}, \texttt{tokens\_used}, \texttt{tool\_calls} (JSONB). Separate from \texttt{AGENT\_ACTION\_LOG} (write-tool execution only). \\
AI\_ML\_SECURITY\_LOG & \texttt{security\_log\_id} & Canonical prototype aggregate for ML inference and security monitoring. Final thesis splits into \texttt{LOAN\_RISK\_ASSESSMENT}, \texttt{SHAP\_EXPLANATION}, \texttt{ML\_MODEL\_REGISTRY}, \texttt{FEATURE\_VECTOR}, and \texttt{SECURITY\_ANOMALY\_EVENT}. \\
...
\textbf{Entity} & \textbf{Primary key} & \textbf{Role / Description} \\
\midrule
SESSIONS & \texttt{session\_id} & Wallet session registry: device/IP hash, EIP-7702 session-key scope and TTL, parent lineage, compression summary, end reason, and delivery source. Audit FK for agent write tools. Phase~II: \texttt{SESSION\_ANCESTOR} closure table replaces recursive trigger (Section~\ref{sec:integrity-constraints}). \\
...
\vspace{2pt}
\noindent\textit{\small This entity summary table enumerates 26 core relational entities. The additions (\texttt{LOAN}, \texttt{SESSIONS}, \texttt{AGENT\_ACTION\_LOG}, \texttt{INTEREST\_RATE\_TIER}, \texttt{ASSETS}, \texttt{CREDIT\_PASSPORT}, \texttt{CREDIT\_PASSPORT\_HISTORY}, \texttt{CHAINLINK\_PRICE\_ROUND}, \texttt{COLLATERAL\_POSITION}, \texttt{BLOCKCHAIN\_EVENT\_LOG}, \texttt{AUDIT\_LOGS}) separate disbursement state from application state, support the AI agent session architecture, provide immutable audit trails, normalise interest-rate governance, standardise asset referencing, bind on-chain SBT and oracle state to off-chain projections, and cache on-chain event receipts respectively. Six extension entities (Table~\ref{tab:db-entities-ext}) and 14 Phase~II--III stubs (Table~\ref{tab:db-entities-ext2}) complete the schema to 46 total. The on-chain/off-chain split implied by the entities supports the architectural principle that high-integrity state transitions occur on-chain while analytics and document workflows remain off-chain.}
...
\midrule
SAVINGS\_ACCOUNT & \texttt{account\_id} & SavingsVault deposit account. FK \texttt{borrower\_id} $\to$ \texttt{BORROWER}; FK \texttt{local\_bank\_id} $\to$ \texttt{LOCAL\_BANK}. \texttt{on\_chain\_address}, \texttt{balance}, \texttt{last\_synced\_block}. \\
FIXED\_DEPOSIT & \texttt{deposit\_id} & FixedDeposit term product. FK \texttt{borrower\_id}; FK \texttt{local\_bank\_id}. \texttt{principal}, \texttt{maturity\_date}, \texttt{on\_chain\_address}. \\
CURRENT\_ACCOUNT & \texttt{ca\_id} & Transactional account. FK \texttt{borrower\_id}; FK \texttt{local\_bank\_id}. \texttt{balance}, \texttt{on\_chain\_address}. \\
LOAN\_GROUP & \texttt{group\_id} & GroupLendingPool on-chain group. FK \texttt{local\_bank\_id}. \texttt{on\_chain\_group\_id}, \texttt{max\_members}, \texttt{status}. \\
...
INSURANCE\_FUND & \texttt{fund\_id} & InsuranceFund reserve tracker. FK \texttt{local\_bank\_id}. \texttt{balance}, \texttt{on\_chain\_address}, \texttt{last\_synced\_block}. \\
\midrule
INTERBANK\_LOAN & \texttt{ib\_loan\_id} & IBLP same-tier liquidity loan. FK \texttt{lender\_bank\_id} $\to$ tier table; FK \texttt{borrower\_bank\_id} $\to$ tier table. \texttt{tenor\_days}, \texttt{amount}, \texttt{status}, \texttt{on\_chain\_tx\_hash}. \\
UPWARD\_DEPOSIT & \texttt{ud\_id} & UpwardDepositFacility surplus deposit. FK \texttt{depositor\_bank\_id}; FK \texttt{parent\_bank\_id}. \texttt{amount}, \texttt{rate}, \texttt{on\_chain\_tx\_hash}. \\
SYNDICATE & \texttt{syndicate\_id} & SyndicatedLoan deal header. FK \texttt{lead\_bank\_id} $\to$ \texttt{LOCAL\_BANK}. \texttt{total\_amount}, \texttt{status}, \texttt{on\_chain\_address}. \\
SYNDICATE\_MEMBER & \texttt{syndicate\_id}, \texttt{member\_bank\_id} & Co-lender position. \texttt{committed\_amount}, \texttt{share\_bps}, \texttt{consent\_tx\_hash}. \\
TRANCHED\_POOL & \texttt{pool\_id} & TranchedPool deal header. FK \texttt{local\_bank\_id}. \texttt{senior\_target}, \texttt{junior\_target}, \texttt{status}, \texttt{on\_chain\_address}. \\
TREASURY\_SWAP & \texttt{swap\_id} & TreasurySwap record. FK \texttt{initiator\_bank\_id}. \texttt{from\_asset\_id}, \texttt{to\_asset\_id}, \texttt{amount\_from}, \texttt{amount\_to}, \texttt{swap\_tx\_hash}. \\
...
NETTING\_ENTRY & \texttt{entry\_id} & Individual net position per bank per batch. FK \texttt{batch\_id} $\to$ \texttt{NETTING\_BATCH}; FK \texttt{bank\_id}. \texttt{net\_amount}, \texttt{settled\_tx\_hash}. \\
\bottomrule
\end{tabular}
...
\midrule
Specialisation (disjoint) & BANK\_USER $\to$ WorldBankUser, NationalBankUser, LocalBankUser & A bank user belongs to exactly one bank type; enforced by CHECK constraint (Table~\ref{tab:integrity-constraints}). \\
...
\midrule
Primary Key & One surrogate or composite key per Table~\ref{tab:db-entities}, Table~\ref{tab:db-entities-ext}, and Table~\ref{tab:db-entities-ext2}: e.g.\ \texttt{world\_bank\_id}, \texttt{national\_bank\_id}, \texttt{local\_bank\_id}, \texttt{bank\_user\_id}, \texttt{borrower\_id}, \texttt{request\_id} (\texttt{LOAN\_REQUEST}), \texttt{loan\_id} (\texttt{LOAN}), (\texttt{loan\_id}, \texttt{installment\_number}) (\texttt{INSTALLMENT}), \texttt{transaction\_id}, \texttt{limit\_id}, \texttt{passport\_id}, \texttt{position\_id}, \texttt{event\_id}, \texttt{audit\_id}, \texttt{proof\_id}, \texttt{message\_id}, \texttt{turn\_id}, \texttt{security\_log\_id}, \texttt{market\_data\_id}, \texttt{round\_id}, \texttt{profile\_id}, \texttt{session\_id}, \texttt{action\_id}, \texttt{tier\_id}, \texttt{asset\_id}. \\
Foreign Key & \texttt{local\_bank\_id} in \texttt{LOAN\_REQUEST} references \texttt{LOCAL\_BANK}; \texttt{loan\_request\_id} in \texttt{LOAN} references \texttt{LOAN\_REQUEST}; \texttt{loan\_id} in \texttt{INSTALLMENT} references \texttt{LOAN}; \texttt{borrower\_id} in \texttt{CREDIT\_PASSPORT} references \texttt{BORROWER}; \texttt{LOAN.borrower\_id} references \texttt{BORROWER(borrower\_id)}; \texttt{LOAN.local\_bank\_id} references \texttt{LOCAL\_BANK(local\_bank\_id)}; \texttt{LOAN\_REQUEST.borrower\_id} references \texttt{BORROWER(borrower\_id)}; \texttt{INCOME\_PROOF.borrower\_id} references \texttt{BORROWER(borrower\_id)}. \\
UNIQUE & \texttt{wallet\_address} in \texttt{BORROWER}; \texttt{contract\_address} in institution tables; \texttt{blockchain\_tx\_hash} in \texttt{LOAN\_REQUEST}; \texttt{borrower\_id} in \texttt{BORROWING\_LIMIT}; \texttt{symbol} in \texttt{ASSETS}; \texttt{tx\_hash} in \texttt{BLOCKCHAIN\_EVENT\_LOG}; partial unique on \texttt{INTEREST\_RATE\_TIER(bank\_tier\_type)} \texttt{WHERE is\_active = TRUE} (ensures at most one active rate configuration exists per tier type at any time). \\
CHECK & BANK\_USER: \texttt{(bank\_type='world' AND world\_bank\_id IS NOT NULL) OR (bank\_type='national' AND national\_bank\_id IS NOT NULL) OR (bank\_type='local' AND local\_bank\_id IS NOT NULL)}. \\
NOT NULL & Core attributes: \texttt{name}, \texttt{wallet\_address}, \texttt{status}, \texttt{oracle\_state} (default \texttt{NONE}). \\
APPEND-ONLY (DB policy) & \texttt{AGENT\_ACTION\_LOG}, \texttt{CREDIT\_PASSPORT\_HISTORY}: \texttt{CREATE ROLE audit\_writer; GRANT INSERT ON agent\_action\_log TO audit\_writer; REVOKE UPDATE, DELETE FROM PUBLIC.} \texttt{AUDIT\_LOGS}: same policy applied via Row-Level Security (\texttt{ALTER TABLE audit\_logs ENABLE ROW LEVEL SECURITY; CREATE POLICY audit\_insert\_only ON audit\_logs FOR INSERT WITH CHECK (TRUE)}). \\
...
The compliance gateway uses two cooperating zero-knowledge circuits rather than a single KYC verifier: a \textbf{KYC circuit} and an \textbf{AML circuit}, both expressed as zk-SNARKs (Groth16 proofs via Circom~2.0 + snarkjs). KYC proves that the user owns a valid identity credential; AML proves that the user's wallet has not interacted with sanctioned addresses and stays within velocity limits. Decoupling the two circuits makes it possible to revoke AML certification without invalidating the KYC credential, and matches the actual structure of regulator-mandated compliance. Off-chain workflow state is tracked in \texttt{KYC\_VERIFICATION\_REQUEST} (Table~\ref{tab:db-entities-ext}).
...
Eight ERD entities (\texttt{INTERBANK\_LOAN}, \texttt{UPWARD\_DEPOSIT}, \texttt{SYNDICATE}, \texttt{SYNDICATE\_MEMBER}, \texttt{TRANCHED\_POOL}, \texttt{TREASURY\_SWAP}, \texttt{NETTING\_BATCH}, \texttt{NETTING\_ENTRY}) extend Figure~\ref{fig:erd-extended}, together with \texttt{GROUP\_CONSENT} (Phase~II, required before \texttt{GroupLendingPool}) and \texttt{TRANCHED\_POOL\_SUBSCRIPTION} (Phase~II). Six contracts complete the 15-contract architecture: \texttt{InterBankLendingPool}, \texttt{UpwardDepositFacility}, \texttt{SyndicatedLoan}, \texttt{TranchedPool}, \texttt{TreasurySwap}, \texttt{NettingEngine} (Phase~II: first three; Phase~III: last three). Extended entities include planned \texttt{on\_chain\_address} and \texttt{last\_synced\_block} columns for event-listener mapping.
...
\midrule
SAVINGS\_ACCOUNT, FIXED\_DEPOSIT, CURRENT\_ACCOUNT & \texttt{borrower\_id} $\to$ \texttt{BORROWER}; \texttt{local\_bank\_id} $\to$ \texttt{LOCAL\_BANK} \\
LOAN\_GROUP & \texttt{local\_bank\_id} $\to$ \texttt{LOCAL\_BANK} \\
GROUP\_MEMBER & \texttt{group\_id} $\to$ \texttt{LOAN\_GROUP}; \texttt{borrower\_id} $\to$ \texttt{BORROWER} \\
INSURANCE\_FUND & \texttt{local\_bank\_id} $\to$ \texttt{LOCAL\_BANK} \\
INTERBANK\_LOAN & \texttt{lender\_bank\_id}, \texttt{borrower\_bank\_id} $\to$ tier institution table \\
UPWARD\_DEPOSIT & \texttt{depositor\_bank\_id}, \texttt{parent\_bank\_id} $\to$ tier institution table \\
SYNDICATE & \texttt{lead\_bank\_id} $\to$ \texttt{LOCAL\_BANK}; \texttt{borrower\_id} $\to$ \texttt{BORROWER} \\
SYNDICATE\_MEMBER & \texttt{syndicate\_id} $\to$ \texttt{SYNDICATE}; \texttt{member\_bank\_id} $\to$ tier table \\
TRANCHED\_POOL & \texttt{local\_bank\_id} $\to$ \texttt{LOCAL\_BANK} \\
TRANCHED\_POOL\_SUBSCRIPTION & \texttt{pool\_id} $\to$ \texttt{TRANCHED\_POOL}; \texttt{subscriber\_id} $\to$ \texttt{BANK\_USER} \\
TREASURY\_SWAP & \texttt{initiator\_bank\_id} $\to$ tier table; \texttt{from\_asset\_id}, \texttt{to\_asset\_id} $\to$ \texttt{ASSETS} \\
NETTING\_BATCH & \texttt{coordinator\_id} $\to$ \texttt{BANK\_USER} \\
NETTING\_ENTRY & \texttt{batch\_id} $\to$ \texttt{NETTING\_BATCH}; \texttt{bank\_id} $\to$ tier table \\
...
\item \texttt{AI\_ML\_SECURITY\_LOG} records the detection event (wallet, score, timestamp, feature vector).
...
right-to-explanation), an immutable \texttt{AI\_ML\_SECURITY\_LOG} row per
...
complete database schema (26 core entities; 6 extension and 14 stub entities phased in Phase~II--III), and React frontend shell.
...
Database backend & Feasible & PostgreSQL schema designed (46 entities across phased rollout, 3NF); FastAPI async REST \\
\bottomrule
\end{tabular}
...
\item \textbf{Break-even analysis:} Opportunities for profit on loans as small as \$25 USDC exist on Polygon PoS. Profitability on loans below \$12{,}500 USDC is not achievable on Ethereum mainnet without Layer~2. Layer~2 is essential for facilitating micro-loans.
\end{itemize}
...
\item \textbf{Backend architecture:} Express.js REST API with PostgreSQL (authoritative schema, 46 entities --- 26 core + 20 extension/stub) and FastAPI for ML inference. MongoDB is not part of the planned production architecture.
...
\texttt{idx\_aiml\_score} &
  \texttt{AI\_ML\_SECURITY\_LOG}(\texttt{anomaly\_score DESC})\newline
...
\texttt{idx\_ib\_loan\_status} &
  \texttt{INTERBANK\_LOAN}(\texttt{status}, \texttt{lender\_bank\_id}) &
...
BORROWER & \texttt{wallet\_address} $\to$ \texttt{borrower\_id}, all attributes & Unique candidate key in the prototype; Phase~IV uses \texttt{(wallet\_address, chain\_id)}. \\
...
SESSIONS & \texttt{session\_id} $\to$ \texttt{borrower\_id, wallet\_address, session\_key\_hash,} \newline \texttt{session\_key\_scope, session\_key\_expires\_at,} \newline \texttt{expires\_at, revoked} & Session key scope encodes approved MCP tool list + value cap (JSONB; normalised in \texttt{SESSION\_KEY\_PERMISSION}, Phase~III). \\
...
Database & PostgreSQL (designed) & 46 entities (26 core + 20 extension/stub), 3NF; relational integrity \\
...
\item Microsoft Security, ``Running OpenClaw Safely: Identity, Isolation, and Runtime Risk,'' Microsoft Security Blog, February~2026. [Online]. Available: \url{https://www.microsoft.com/en-us/security/blog/2026/02/19/running-openclaw-safely-identity-isolation-runtime-risk/}. Accessed: June~9, 2026.

\item NVIDIA, ``NVIDIA and Microsoft Reinvent Windows PCs for the Age of Personal AI,'' NVIDIA Newsroom, GTC Taipei / COMPUTEX 2026, May--June~2026; see also NVIDIA Blog, ``RTX PCs and DGX Spark Supercomputers Run AI Agents Locally'' (Nemotron on-premise inference for finance, healthcare, and legal workloads). [Online]. Available: \url{https://nvidianews.nvidia.com/news/nvidia-microsoft-windows-pcs-agents-rtx-spark}. Accessed: June~9, 2026.
