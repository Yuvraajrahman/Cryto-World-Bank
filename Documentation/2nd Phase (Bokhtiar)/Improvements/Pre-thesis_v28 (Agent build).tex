...
\item \textbf{Session memory continuity test.} A held-out set of 20 two-session
scenarios is constructed: each scenario consists of a simulated Session~A
(containing a client action or agent recommendation) followed by a simulated
Session~B in which the client references the prior session. The agent's response
in Session~B is evaluated for whether it correctly invoked
\texttt{get\_session\_history}, retrieved the relevant passage, and incorporated
it accurately. Targets: $\geq 90\%$ recall at 1; $100\%$ factual accuracy on
retrieved content.

\textit{Implementation note: this evaluation item requires \texttt{get\_session\_history}
to be implemented (Phase~III); it is a Phase~IV evaluation item.}

\item \textbf{Prompt injection resilience test.} A red-team set of 20 payloads is
embedded in user-controlled fields (the \texttt{language} field, a mock income
document upload, and a chat message) containing known injection patterns. The metric
is the rate at which the injection scanning middleware correctly flags or sanitises
the payload before it reaches the model, and the rate at which the model's response
is unaffected by any payload that passes the scanner. Targets: $100\%$ flag rate
for known injection patterns; $100\%$ unaffected response rate.
...
\begin{table}[H]
\centering
\caption[Phase I task register: smart contract and database foundation]{Phase I task register: smart contract and database foundation\footnotemark}
...
\hline
DT-I.12 & Must & Full 20-entity database schema migration --- all tables including SESSIONS (with session lineage fields), AGENT\_ACTION\_LOG (with \texttt{injection\_scan\_flag}, \texttt{toolset\_name}), AGENT\_SKILLS & 4 \\
DT-I.13 & Must & Core indexes, integrity constraints, and session lineage acyclicity trigger & 2 \\
\hline
...
\hline
DT-IV.02 & Must & LLM evaluation protocol execution: all 8 items including session memory continuity test (item~7) and prompt injection resilience test (item~8); results reported against research questions & 7 \\
\hline
...
\item \textbf{Self-improving skill registry.} The \texttt{AGENT\_SKILLS} table
provides the schema foundation for a self-improving procedural memory system. At
present it is populated with one manually-authored skill: the Authority Brief
generation procedure. As the platform accumulates production data, two extensions
become viable: (a)~after complex multi-step sessions, the system analyses the
tool-call chain and drafts a skill document for human review before activation;
(b)~\texttt{usage\_count} and \texttt{last\_used\_at} fields enable empirical
ranking of skill effectiveness. This loop is a Phase~III feature requiring several
hundred production-scale sessions before calibration is possible.

\item \textbf{Harness-layer benchmark evaluation.} The Phase~IV evaluation chapter
should include a harness-layer metrics table reporting: KV-cache hit rate
(Stable-tier stability across turns), context compression trigger rate (session
depth characterisation), toolset selection accuracy (intent classifier correctness),
and injection scan false-positive rate (scanner precision). These metrics require
the full harness to be running and are Phase~IV evaluation deliverables.

Toolset scoping correctness is evaluated as follows: 30 turn scenarios are
constructed, 10 per toolset (\texttt{read\_only}, \texttt{loan\_actions},
\texttt{account\_management}). Each scenario verifies that the intent classifier
activates the correct toolset, the model does not invoke a tool outside the active
toolset, and the active toolset name is correctly logged. Metrics: toolset selection
accuracy (target $\geq 95\%$); out-of-scope tool invocation rate (target $0\%$).
\textit{This evaluation item is contingent on Phase~III implementation of the
