...
\begin{table}[H]
\centering
\caption{Database entity summary (19 entities).}
...
\vspace{2pt}
\noindent\textit{\small This entity summary table now enumerates 19 relational entities. The four additions (\texttt{SESSIONS}, \texttt{AGENT\_ACTION\_LOG}, \texttt{INTEREST\_RATE\_TIER}, \texttt{ASSETS}) support the AI agent session management architecture, the immutable agent audit trail, normalised interest rate governance, and standardised asset referencing respectively. The on-chain/off-chain split implied by the entities supports the architectural principle that high-integrity state transitions occur on-chain while analytics and document workflows remain off-chain.}
