...
The Crypto World Bank is a Final Year research prototype: a blockchain-based institutional finance platform with four tiers (World Bank reserve, national banks, local banks, retail clients). Existing DeFi tools such as Aave, Uniswap, and MakerDAO each cover one function in a flat pool; this work specifies hierarchy, reserves, group lending, and cross-tier capital flows in one design. Tier~1--3 lending contracts and the multi-entity suite are specified in Chapter~3 (Phases~II--III). Artifacts are in our \href{https://github.com/Yuvraajrahman/Cryto-World-Bank}{GitHub repository}. Unbanked and MSME gaps~[9, 15] motivate the model; no single-country deployment is claimed.
...
DeFi automates lending on-chain (Aave~v3 $\approx$\$26.3B TVL~[21]; sector $>$\$55B~[8], March~2026) but uses flat pools without institutional roles or Local-Bank-to-client paths~[1]. Smart contracts enforce auditable rules; a shared ledger lets reserve ratios update when a Local Bank disburses~[19]. CWB adds \textit{hierarchy}, \textit{multi-entity co-funding}, and development-finance structure missing from prior DeFi~[1]. Core UI is React plus wallet signing; an optional MCP agent (Chapter~4) never gates lending.
...
Four scoped contributions: (1)~\textbf{four-tier hierarchical DeFi} with reserve enforcement at every tier~[1, 37, 90]---no prior work combines this with explainable ML governance and group lending in one open stack; (2)~\textbf{on-chain solidarity lending} with consent, installments, and over-indebtedness caps (Section~\ref{sec:over-indebt}; Phase~II PoC); (3)~\textbf{oracle-mediated ML}---fraud, anomaly, and SHAP explainability via relay or Chainlink (training after pre-thesis~2~[83, 84]); (4)~\textbf{ZKP identity}---Groth16 KYC plus W3C DID/VC~[58, 64], zkAML in future work~[59]. CWB targets development-finance and MFI use cases as a composable layer, not a bank replacement~[65].
...
Figure~\ref{fig:banking-functions} maps six banking functions (deposits, credit, payments, risk, liquidity, ancillary services) enforced on-chain across the four tiers.
...
Capital moves downward (allocation), sideways (interbank pools), upward (surplus deposits), and through syndicated, group, and tranched contracts (Figure~\ref{fig:cross-tier-lending}). Details are in Section~\ref{sec:multi-entity} and Section~\ref{sec:lit-cross-tier}. Retail clients enter via Local Banks under KYC, Credit Passport, and borrowing limits (Sections~\ref{sec:tiered-kyc},~\ref{sec:credit-passport}).
...
Agile delivery across four SDLC phases. \textbf{Pre-thesis~1} (this report) specifies architecture and design only---no ML training or live inference. \textbf{Phases~I--IV} (after pre-thesis~2): contract scaffold and data model (I), lending and deposits (II), ML oracle and extended contracts (III), formal verification (IV). Stack: Solidity~0.8.20, React/TypeScript, Express + FastAPI, scikit-learn, Circom~2.0; testnets Polygon~zkEVM~Cardona and Ethereum~Sepolia. EVM, oracle, and platform rationale are in Chapter~3 (Sections~\ref{sec:high-level-arch},~\ref{sec:oracle-architecture}).
...
\textbf{Challenges.} Sparse fraud labels, regulatory uncertainty (testnet-only mitigation), gas cost (L2 + off-chain ML), and SHAP interpretability. Retail loans default to stablecoins~[60] under MiCA/GENIUS~[75, 77] (Section~\ref{sec:stablecoin-first} in Chapter~5). Economics and market sizing are in Chapter~5 (Tables~\ref{tab:market-sizing-ch5}--\ref{tab:partner-categories-ch5}).

\paragraph{Stablecoin-first lending.}
\label{sec:stablecoin-first}
ETH-denominated micro-loans expose borrowers to exchange-rate risk; retail tiers use USDC/USDT with reserve and bridge design in Chapters~3--5.

\section{Thesis Organization}
\label{sec:thesis-organization}

Chapter~2 surveys literature and gaps; Chapter~3 architecture and contracts; Chapter~4 methodology and MVT scope; Chapter~5 feasibility; Chapter~6 conclusion. Appendices hold contract interfaces and deployment notes. \textbf{Rubric mapping:} Ch.~1 (CO1), Ch.~2 (CO9), Ch.~3 (CO2--4, CO12), Ch.~4 (CO5, CO10), Ch.~5 (CO6--7, CO11). Implementation evidence follows pre-thesis~2.
...
Core terms (DeFi, XAI, ZKP, CBDC, group lending, etc.) are in the \textbf{Glossary} appendix. Platform terms (\textit{Platform Solvency Ratio}, \textit{Provisional Credit Tier}) are defined there. Blockchain and EVM background is in Chapter~3 (Section~\ref{sec:high-level-arch}).

\subsection{Review Methodology (PRISMA-Style Frame)}
\label{sec:lit-prisma}

Evidence follows a PRISMA-style frame~[66]: peer-reviewed or institutional sources (2017--2026) mapped to eight CWB modules (DeFi, ML fraud, ZKP identity, microfinance, security, institutional DeFi, regulation, retail rails), with reusable metrics (F1, gas, reserve rules). Six databases were searched; 121 bibliography entries survived screening (Figure~\ref{fig:prisma-flow}). Table~\ref{tab:lit-top10} ranks the top ten sources~[17].
...
7 & BIS WP 905 [60] · Stablecoin risk · Secs.~\ref{sec:stablecoin-first}, \ref{sec:mica-compliance} & Algorithmic stablecoins unsuitable for EM DeFi; USDC/USDT recommended \\
\rowcolor{RowShade}
...
The survey spans DeFi lending, ML/XAI, hierarchical and group finance, smart-contract security, settlement and retail rails, and regulation. Evidence is synthesized in Tables~\ref{tab:lit-summary} and~\ref{tab:lit-summary-b} (see also Figure~\ref{fig:cross-tier-lending} and Section~\ref{sec:comparative-protocol}).

\section{Literature Review Summary}

Tables~\ref{tab:lit-summary} and~\ref{tab:lit-summary-b} summarize methodology, findings, and project relevance (synthesis format~[17]).
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.12}
\caption[Literature review summary: DeFi, ML, and security]{Literature review summary: DeFi, ML, and security}
\label{tab:lit-summary}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.2cm}>{\raggedright\arraybackslash}p{2.8cm}>{\raggedright\arraybackslash}p{3.6cm}>{\raggedright\arraybackslash}p{3.4cm}@{}}
\toprule
\tableheadcolor
\tableheadfont Author \& Focus & \tableheadfont Methodology & \tableheadfont Key Findings & \tableheadfont Relevance to Our Project \\
\midrule
Werner et al.\ (2022) [1] · DeFi SoK & Survey of 12+ protocol types & Flat, pool-based, overcollateralised lending; no hierarchy & Core gap for four-tier design \\
Bastankhah et al.\ (2024) [2] · Adaptive rates & Dual fast/slow control & Beats static Aave/Compound curves & Informs kinked-rate model \\
Palaiokrassas et al.\ (2024) [3] · DeFi fraud & ML on 54M+ transactions & F1 0.76--0.85 with behavioral features & Random Forest pipeline \\
Adom et al.\ (2022) [4] · XAI lending & LIME vs.\ SHAP & SHAP more consistent for explanations & Primary XAI method \\
Liu et al.\ (2008) [6] · Anomaly detection & Isolation Forest & Unsupervised outlier isolation & Secondary fraud detector \\
Atzei et al.\ (2017) [7] · Contract security & Attack SoK & Reentrancy, access-control risks & Security primitives \\
Wang et al.\ (2020) [43] · ContractWard & Bytecode ML & F1 0.96 access control & Audit automation \\
Hasan et al.\ (2022) [42] · Fed learning & Encrypted features & Centralised-comparable accuracy & Cross-bank fraud intel \\
Chen et al.\ (2024) [39] · Multi-chain & Cross-chain lending & Higher throughput & Multi-chain deployment \\
Kim and Kim (2024) [48] · Gas optimisation & Batching / compression & 30--50\% gas savings & L2 deployment \\
\bottomrule
\end{tabular}%
}
\TableScreenshotEnd[tab:lit-summary]{Literature Review Summary: DeFi, ML, and Security}
\end{table}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.12}
\caption[Literature review summary: Hierarchy, settlement, and retail]{Literature review summary: hierarchy, settlement, and retail}
...
\tableheadfont Author \& Focus & \tableheadfont Methodology & \tableheadfont Key Findings & \tableheadfont Relevance to Our Project \\
\midrule
Tan (2023) [5] · CBDC tiers & Developing-nation model & Central bank $\to$ banks $\to$ users & Four-tier parallel \\
Kiff et al.\ (2020) [106] · CBDC models & IMF survey & Supervised intermediaries & Wholesale-to-retail path \\
Project Pine (2025) [103] · Layered contracts & NY Fed / BIS prototype & Factory-to-facility hierarchy & \texttt{IBLP} / syndication \\
Dong et al.\ (2023) [104] · Supply-chain finance & Game-theoretic tiers & Direct financing to deep tiers & Downward client lending \\
Asaduzzaman et al.\ (2020) [47] · Microcredit & Smart contracts & $\approx$60\% lower admin cost & \texttt{GroupLendingPool} \\
BIS/FSB [16] · Cross-border & Settlement analysis & 2--5 days; $\approx$\$42/transfer & Instant tier settlement \\
Ante (2025) [108] · Remittance & User survey & 26\% stablecoin adoption & KYC ladder \\
Du et al.\ (2026) [110] · Payment rails & Corridor cost study & Stablecoin sandwich; off-ramp limits & Future ramp work \\
CPMI / FATF [114, 117] · Policy / AML & Regulatory guidance & Ramp and VASP rules & Sponsored \texttt{P2PTransfer} \\
World Bank (2025) [33] · FundsChain & Pilot tracking & On-chain disbursement audit & Institutional precedent \\
\bottomrule
\end{tabular}%
}
\TableScreenshotEnd{Literature Review Summary: Hierarchy, Settlement, and Retail}
\end{table}
...
Table~\ref{tab:protocol-comparison} answers RQ1: flat-pool protocols (Aave, Compound) do not model inter-bank relationships; Maple, Goldfinch, and Morpho~[25, 26, 37] lack full multilateral development-finance hierarchy.
