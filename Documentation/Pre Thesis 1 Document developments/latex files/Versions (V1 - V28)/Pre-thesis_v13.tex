...
%%
\documentclass[12pt,a4paper]{report}  % v13: TikZ vector diagrams via \CWBIncludeDiagram
...
% Table legend marks (math-mode symbols; safe next to & in tabular).
\newcommand{\tmarkDone}{\textcolor{green!55!black}{$\checkmark$}}
\newcommand{\tmarkNo}{\textcolor{red!75!black}{$\times$}}
\newcommand{\tmarkPartial}{\textcolor{orange!85!black}{$\triangleright$}}
\newcommand{\tmarkPlanned}{\textcolor{blue!75!black}{$\circ$}}

\usepackage{graphicx}
\graphicspath{{Diagrams/mermaid-pdf/}{./}{Tables/}{Diagrams/}{Diagrams/CSE370/}{Diagrams/CSE471/}{Diagrams/CSE470/}}
...
% Ragged-right X columns and fixed-width ragged columns (ACM-style table layout).
...
\noindent\textbf{Pre-thesis 1 emphasis:} Chapter~1 (CO1) and Chapter~2 (CO9) per the department rubric.\\
\noindent\textbf{Not claimed:} Mainnet deployment, production KYC/AML, geo-specific operational rollout, or completed fine-tuning of production AI weights.\\
\noindent\textbf{Formatting target:} ACM \texttt{acmsmall} typography and table conventions (see \texttt{Writing format/checklist.md}).
...
\begin{description}[leftmargin=0pt,labelwidth=2.2cm,align=left]
    \item[F.1 Utilization Rate ($U$):] \(U = \frac{L}{L + B}\) (lending utilization; \(L\) is lent amount, \(B\) is available balance/liquidity)
    ...
    \item[F.4 APR (simple):] \(APR = r_{\text{period}} \times N\) (annualized simple rate over \(N\) periods)
    \item[F.5 Compound Interest (Savings):] \(A = P\left(1 + \frac{r}{n}\right)^{nt}\) (deposit growth; \(P\) principal, \(r\) annual rate, \(n\) compounding frequency)
    \item[F.6 EMI (Installment Payment):] \(EMI = \frac{P \cdot r \cdot (1+r)^n}{(1+r)^n - 1}\) (installment for principal \(P\), periodic rate \(r\), \(n\) installments)
    \item[F.7 Health Factor (HF):] \(HF = \frac{C \times LT}{D}\) (liquidation trigger; if \(HF < 1.0\) liquidation is triggered, where \(LT\) is liquidation threshold)
    \item[F.8 Reserve Ratio (RR):] \(RR = \frac{\text{Reserves}}{\text{Total Deposits}}\) (minimum reserve enforcement constraint)
    \item[F.9 Capital Adequacy Ratio (CAR):] \(CAR = \frac{\text{Tier 1 Capital}}{\text{Risk-Weighted Assets}}\) (Basel-style prudential ratio)
    \item[F.10 Money Multiplier ($M$):] \(M = \frac{1}{RR}\) (fractional reserve multiplier approximation)
    \item[F.11 FX Conversion:] \(\text{Amount}_B = \text{Amount}_A \times \left(\frac{P_A}{P_B}\right)\) (oracle-priced conversion between assets \(A\) and \(B\))
    \item[F.12 Group Loan Individual Share:] \(\text{Share}_i = \frac{\text{LoanTotal}}{N_{\text{members}}}\)
    \item[F.13 On-Chain Credit Score (conceptual):] \(CS = \sum_i w_i \cdot \text{feature}_i\) (weighted features learned off-chain and stored as a score on-chain)
    \item[F.14 Isolation Forest Anomaly Score:] \(s(x, n) = 2^{-\frac{E(h(x))}{c(n)}}\) (anomaly scoring for instance \(x\); where \(h(x)\) is the path length of instance \(x\) from the root of an isolation tree and \(c(n)\) is the average path length for \(n\) samples)
    \item[F.15 Random Forest Fraud Probability:] \(P(\text{fraud}\mid x) = \frac{1}{T}\sum_{t=1}^{T} f_t(x)\) (ensemble average over \(T\) trees; where \(f_t(x)\) is the binary fraud prediction of the \(t\)-th decision tree)
    ...
    \item[F.17 SHAP (Shapley value):] \(\phi_i = \sum_{S \subseteq F \setminus \{i\}} \frac{|S|!(|F|-|S|-1)!}{|F|!}\left[f(S \cup \{i\}) - f(S)\right]\)
\end{description}
\newpage
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{financial-inclusion-gap}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{flat-vs-hierarchical}
...
\begin{figure}[t]
\centering
\CWBIncludeDiagram{blockchain-stack-layers}
...
\begin{figure}[t]
\centering
\CWBIncludeDiagram{blockchain-tx-lifecycle}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{cross-tier-flow}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{borrower-tier-access}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{correspondent-vs-onchain}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{monetary-policy-comparison}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{institutional-adoption-timeline}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{component-diagram}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{aiml-security-pipeline}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{usecase-diagram}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-loan-request}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-hierarchical-banking}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-income-verification}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-chat-system}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-ai-chatbot}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-market-data}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-profile-management}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-deposit-flow}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-group-lending}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-fx-conversion}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{activity-interbank-liquidity}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{dfd-level1-part1}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{dfd-level1-part2}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{sequence-loan-approval}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{sequence-reject-path}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{sequence-installment}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{sequence-income-verification}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{sequence-chat-system}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{sequence-ai-chatbot}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{sequence-hierarchical-banking}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{sequence-market-data}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{sequence-borrowing-limit}
...
\begin{figure}[t]
\centering
\CWBIncludeDiagram{ai-unified-9b-architecture}
...
\begin{figure}[t]
\centering
\CWBIncludeDiagram{ai-data-pipeline}
...
\begin{figure}[t]
\centering
\CWBIncludeDiagram{ai-qlora-training-flow}
...
\begin{figure}[t]
\centering
\CWBIncludeDiagram{ai-oracle-loan-sequence}
...
\begin{figure}[t]
\centering
\CWBIncludeDiagram{ai-security-ci-pipeline}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{market-sizing-funnel}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{defi-tvl-comparison}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{competitive-feature-matrix}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{competitor-quadrant}
...
\begin{figure}[H]
\centering
\CWBIncludeDiagram{interest-rate-waterfall}
