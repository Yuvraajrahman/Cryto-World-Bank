%% =============================================================================
%% Pre-thesis 1 -- Crypto World Bank -- v21 (BRACU front matter + ACM body)
%% -----------------------------------------------------------------------------
...
%%   * Fix Issue 2: Model name corrected from "Qwen3.5-9B-Instruct" to
%%     "Qwen3.5-9B" (Instruct variant) throughout -- aligned with official
%%     Alibaba March 2026 release naming convention (3 occurrences).
...
%%   * New 1: Qwen3.5-9B multimodal capability note added to LLM section.
...
\item \textbf{Latency and resource ceiling.} On the gaming-PC demo hardware (AMD Radeon RX~9060~XT 16~GB, ROCm\footnote{Evaluated on Linux with \texttt{GGML\_CUDA\_NO\_PEER\_COPY=1} and \texttt{HSA\_OVERRIDE\_GFX\_VERSION=12.0.1} environment variables to work around ROCm~6.4 memory-management limitations on RDNA4 gfx1200/gfx1201 hardware; see llama.cpp issue~\#21376.}), llama.cpp serving Qwen3.5-9B~(Instruct~variant)~Q4\_K\_M is benchmarked end-to-end (prompt $\to$ response token~1, total response). Target: <2~s time-to-first-token, $\ge$5 tokens/s sustained throughput. RAG retrieval through ChromaDB is benchmarked separately at <100~ms p95.
\end{enumerate}
...
The inference endpoint follows the \textbf{OpenAI Chat Completions} contract (\texttt{/v1/chat/completions}) with \texttt{stream: true}, proxied from the project backend to a local service such as \texttt{http://127.0.0.1:1234}. During early development, the assistant used a locally hosted \textbf{Gemma-4-E4B} checkpoint (Google mixture-of-experts model, $\sim$26B total / 4B active parameters) via LM Studio in GGUF form (e.g., \texttt{Q4\_K\_M} quantization) for a laptop-friendly footprint. For the final thesis evaluation, the assistant is replaced by a \textbf{Qwen3.5-9B} (Instruct variant, Alibaba, released March~2026), a natively multimodal dense 9B model supporting text, image, and video input across a 262K-token context window (1M extended) and 201 languages~[1-1]. The Instruct variant is the model evaluated under the protocol of Section~\ref{sec:llm-eval} (latency benchmark on AMD Radeon RX~9060~XT, RAG precision@k, and red-teaming refusal rate). The API model identifier is environment-driven; the integration layer remains identical for both models. The native multimodal capability is a Future Work extension: in a production deployment, the LLM assistant could accept KYC document images directly, reducing the round-trip between the vision OCR step and the conversational interface.
