...
def run_rf(tc: TrainConfig, bundle: dict, state: dict) -> RandomForestClassifier:
    ...
    if tc.rf_chunks > 1:
        ...
        built = 0
        while built < tc.rf_trees:
            built = min(built + chunk, tc.rf_trees)
            rf.n_estimators = built
            rf.fit(bundle["X_train"], bundle["y_train"])
            joblib.dump(rf, rf_path)
            print(f"[rf] checkpoint {built}/{tc.rf_trees} trees")
    else:
        rf.fit(bundle["X_train"], bundle["y_train"])
        joblib.dump(rf, rf_path)
...
def run_shap(tc, rf, bundle, state) -> list:
    ...
    examples = []
    for i in range(min(tc.shap_examples, n)):
        row = values[i]
    ...
    global_imp = [
        {"feature": bundle["feature_cols"][j], "mean_abs_shap": float(mean_abs[j])}
        for j in top_global
    ]
