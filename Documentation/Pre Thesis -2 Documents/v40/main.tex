...
\usepackage{xcolor}
% TikZ/pgfplots omitted: figures are pre-exported PDF vectors (faster Overleaf compile).
...
% Abbreviations use core/list_of_abbreviations.tex (BracU front-matter slot).
% Skip nomencl+makeindex: no \nomenclature entries, and makeindex costs Overleaf time.
\newcommand{\printnomenclature}{}

% IEEE-style references are authored in bibliography/references_manual.tex.
% Do not load biblatex/biber here — it adds a tool pass and caused free-tier timeouts.
...
% Prefer PDF vectors (academy checklist: SVG/EPS/PDF — not JPG/PNG screenshots).
...
\newcommand{\LandscapeDiagram}[2][]{%
  ...
  \begin{figure}[H]%
  \centering%
  % Cap height so image + caption fit one landscape page (avoids blank pages from tiny overfull \vbox).
  \makebox[\linewidth][c]{%
    \ThesisIncludeGraphics[width=\linewidth,height=0.82\textheight,keepaspectratio]{#2}%
  }%
}
...
\newcommand{\SequenceDiagram}[1]{%
  ...
  \begin{figure}[H]%
  \centering%
  % Same height cap: consecutive landscape sequence figures were leaving an empty page
  % when width=\linewidth made the graphic a fraction of a point taller than \textheight+caption.
  \makebox[\linewidth][c]{%
    \ThesisIncludeGraphics[width=\linewidth,height=0.82\textheight,keepaspectratio]{#1}%
  }%
}
...
% Nomenclature chapter omitted: abbreviations already listed above (avoids empty makeindex pass).
\cleardoublepage
...
% Ch4 — Proposed Methodology (v39 Ch4: Design Alternatives)
\chapter{Proposed Methodology}
\input{chapters/chapter_4.tex}

% Ch5 — Result Analysis (v39 Ch5: Evaluation, Analysis, and Final Design)
