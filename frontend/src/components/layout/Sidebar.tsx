  ...
  return (
    <aside className="hidden w-64 shrink-0 border-r border-ink-700/60 bg-ink-900/60 backdrop-blur-sm lg:flex lg:flex-col">
      <div className="flex flex-1 flex-col gap-1 overflow-y-auto p-4">
        {user ? (
          <div className="mb-3 rounded-xl border border-ink-600/60 bg-ink-950/70 p-3">
            <div className="text-[10px] uppercase tracking-[0.22em] text-ink-200">
              Signed in as
            </div>
            <div className="mt-0.5 truncate text-sm font-medium text-ink-100">
              {user.displayName}
            </div>
            <div className="mt-0.5 text-[10px] uppercase tracking-[0.2em] text-gold-300">
              {role.replace(/_/g, " ")}
            </div>
          </div>
        ) : null}
