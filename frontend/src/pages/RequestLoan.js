...
import { fetchCreditPassport, validateLoanAmount } from "@/lib/phase2";
    ...
    async function submit() {
        ...
        if (onChain) {
            ...
            if (nidFile && photoFile && address) {
                const upload = await api.post("/api/phase2/documents/upload", {
                    wallet: address,
            ...
            write({
                ...
                abi: localBankAbi,
                functionName: docHash === `0x${"0".repeat(64)}` ? "requestLoan" : "requestLoanWithDoc",
                args: docHash === `0x${"0".repeat(64)}`
