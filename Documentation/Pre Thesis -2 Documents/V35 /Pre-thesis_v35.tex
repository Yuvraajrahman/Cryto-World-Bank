...
\item \textbf{Economic sustainability} --- interest revenue must cover gas at all tiers (Section~\ref{sec:scopes-challenges} and Section~\ref{sec:gas-cost}).
...
Figure~\ref{fig:component-diagram} shows component-level interactions. Core lending contracts are specified in Phase~I; extended modules are documented in Appendix~B.
...
\section{Banking Modules}
\label{sec:banking-modules}

\subsection{Kinked Interest Rates}
...
\subsection{Savings and Fixed Deposits}
...
\subsection{Credit Passport (SBT)}
...
\section{Group Solidarity Lending}
...
The platform targets financial inclusion for underbanked adults through sub-cent Layer-2 transaction costs, mobile-first onboarding, and micro-loan denominations (Section~\ref{sec:societal-impact}). Societal benefits are \textit{design intent}, not empirically validated deployment outcomes at pre-thesis stage: graded evidence is limited to architecture specification, BCCC fraud-detection benchmarks, and economic feasibility analysis (Chapter~\ref{ch:evaluation}).
...
Development follows a four-phase, 16-week plan with supervisor gates G1--G4 (Chapter~\ref{ch:methodology}), MoSCoW prioritisation, and explicit zero-cost risk registers (Table~\ref{tab:zero-cost-risks}). Primary schedule risks---testnet instability, faucet scarcity, and scope creep---are mitigated by Sepolia-first deployment, \texttt{MockUSDC}, local Anvil/Hardhat simulation, and MVT pruning (Section~\ref{sec:design-limitations}).

\section{Design Limitations and Zero-Cost Demonstration Strategy}
\label{sec:design-limitations}

The full architecture remains specified for examination; the graded prototype uses zero-cost testnet substitutes (Table~\ref{tab:design-vs-demo}). Key risks---no official USDC on testnets, faucet limits, RPC rate limits---are mitigated by \texttt{MockUSDC}, local Anvil simulation, and Sepolia-first deployment.
...
\noindent Acknowledged simplifications: trusted commit--reveal relay, mock stablecoin/feeds, centralised installment cron, BCCC 21-feature benchmark (not full 22-dim CWB vector), and local simulation instead of 300 live testnet wallets. Production upgrade paths remain in Future Work.
...
DT-I.12 & Must & Phase~I schema migration --- \textbf{13 M1 tables} (Section~\ref{sec:db-impl-tiers}): \texttt{INSTITUTION}, \texttt{COUNTRY}, tier subtypes, \texttt{BANK\_USER}, \texttt{BORROWER} (incl.\ \texttt{kyc\_level}), \texttt{LOAN\_REQUEST}, \texttt{LOAN}, \texttt{INSTALLMENT}, \texttt{BLOCKCHAIN\_EVENT\_LOG}, \texttt{ASSETS}, \texttt{AUDIT\_LOGS}; M2/M3/stub tables deferred & 4 \\
...
DT-IV.02 & Should & LLM evaluation protocol: task-level accuracy, action accuracy, human-gate bypass test (Section~\ref{sec:optional-agent}; MVT item~12) & 5 \\
...
The full project scope---phase task registers (DT-I.01--IV.10), the MVT checklist (Table~\ref{tab:mvt-checklist}), and the module inventory in Chapter~3---remains the project record. This section states what we will actually build and demo in the 16-week window using free testnets and local tools.
...
Production upgrades---Chainlink DON oracles, live USDC, full 51-table schema, and mainnet deployment---stay documented in Chapter~3 and Future Work. They are not required for the graded prototype.



%% CHAPTER 5: EVALUATION, TESTING AND FEASIBILITY
\chapter{Evaluation, Testing and Feasibility}
\label{ch:evaluation}
\label{ch:market-analysis}

This chapter reports \textbf{empirical and design-level evaluation} of the Crypto World Bank against the five research questions (Section~\ref{sec:research-questions}): ML benchmarks, smart-contract verification, implementation evidence, economic feasibility (CO11), and examiner limitations. Build planning remains in Chapter~\ref{ch:methodology}.
...
RQ3 is evaluated with precision, recall, F1, and ROC-AUC on a held-out test set, plus explainability completeness (SHAP coverage and anomaly deviation lines in \texttt{LOAN\_RISK\_ASSESSMENT}). Pre-thesis evaluation uses a \textbf{50{,}000-address subsample} of BCCC-DeFiFraudTrans-2025~[99] with address-grouped hold-out (\texttt{synthetic: false}); full 1M-row retraining and 22-dimensional CWB feature mapping are scheduled for final thesis.

\begin{table}[H]
\centering
\caption[Datasets used for ML risk scoring]{Datasets used for ML risk scoring (RQ3).}
...
\midrule
BCCC-DeFiFraudTrans-2025~[99] & Primary (supervised) & 50k-address subsample; held-out metrics in Table~\ref{tab:ml-bccc-results}; full corpus deferred to Phase~III \\
Elliptic Bitcoin Dataset~[68] & Baseline comparison & Secondary benchmark only; domain mismatch acknowledged \\
On-chain CWB transaction log (future) & Production retrain & Local Bank history once MVT deployment has live traffic \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption[BCCC subsample benchmark (held-out test)]{BCCC-DeFiFraudTrans-2025 subsample benchmark on held-out test set (\texttt{evidence/mac-m4/metrics.json}).}
...
\vspace{4pt}
\footnotesize Confusion matrix (TN\,=\,4{,}621, FP\,=\,60, FN\,=\,42, TP\,=\,417); fraud rate ${\approx}8.7\%$; 21 behavioral features; leakage column \texttt{flag} removed.
\end{table}
...
\caption[ML evaluation and explainability benchmarks]{ML evaluation on BCCC subsample (held-out test: F1\,=\,0.891, ROC-AUC\,=\,0.991) and global SHAP feature importance.}
\label{fig:ml-eval}
\end{figure}

\paragraph{Domain-transfer limitation.}
BCCC~[99] contains Ethereum DeFi fraud from flat pool protocols. CWB hierarchical lending generates different transaction patterns. BCCC provides baseline fraud-detection benchmarking only; production deployment requires CWB-specific labeled data from testnet operation (Future Work, Section~\ref{sec:future-work}).

\subsection{Explainability Methodology}
\label{sec:ml-explainability}

The FastAPI scoring service combines Random Forest fraud probability $p_f$ (with SHAP TreeExplainer attributions) and Isolation Forest anomaly scores via a stacking meta-learner. Anomaly explainability uses feature-deviation reports (applicant value vs.\ training median/percentile). Every inference writes one \texttt{LOAN\_RISK\_ASSESSMENT} row; the contract never auto-disburses on ML output alone---an approver must sign disbursement (decision support, not decision authority).
...
\noindent\textit{Gate G1 demonstration package} (end of Phase~I): Sepolia-deployed tier contracts, \texttt{npm run test:contracts} green, PostgreSQL M1 tables migrated, $\leq$5-minute screen recording of \texttt{allocateCapital}.
...
FR-07 & Figures~\ref{fig:dfd-level0-lending}--\ref{fig:dfd-level1-lending} & Event listener (planned) & PostgreSQL projection tests & DT-I.12--13 \\
FR-08 & Figure~\ref{fig:seq-chat-bot} & MCP agent (Phase III) & Section~\ref{sec:optional-agent} & \#11--12 \\
...
\label{sec:economic-feasibility-chapter}

This section provides the economic analysis required by the rubric (CO11): zero-cost prototype costs, gas economics, and an illustrative pilot NPV. Broader sustainability constraints are stated in Section~\ref{sec:scopes-challenges}.
...
\noindent Year~0 covers audit and infrastructure setup; benefits assume 8\% APR interest spread on 300 loans with 8\% default. The student prototype itself operates at \$0 direct cost (Table~\ref{tab:economic-feasibility}).
...
This thesis makes four contributions: (1)~a \textit{compound} four-tier hierarchical DeFi architecture (reserve enforcement + explainable ML + mutual liability); (2)~on-chain programmable joint-liability group lending with over-indebtedness controls (Section~\ref{sec:over-indebt}); (3)~oracle-mediated AI/ML integration with SHAP and anomaly reports (Section~\ref{sec:ml-explainability}), subject to the BCCC domain-transfer gap (Section~\ref{sec:ml-workload}); and (4)~a specified compliance-aware identity pathway (zkKYC, W3C DID/VC) deferred to Future Work. Examiner limitations G1--G6 are consolidated in Section~\ref{sec:research-limitations}.

With specified architecture (Chapter~3), empirical evaluation and economic feasibility (Chapter~\ref{ch:evaluation}), and a phased implementation roadmap, CWB is a structurally distinct contribution to blockchain-based development finance at the \textit{specification and prototype} stage.

\label{sec:future-work}
\paragraph{MVT completion (final thesis Must).}
\begin{enumerate}[noitemsep]
\item End-to-end hierarchical lending on the chosen prototype testnet (World Bank $\to$ National $\to$ Local $\to$ client).
\item Stablecoin-first deployment with USDC default at retail tiers (Section~\ref{sec:stablecoin-first}).
\item Phase~II modules: \texttt{SavingsVault}, \texttt{FixedDeposit}, \texttt{GroupLendingPool}, \texttt{InterBankLendingPool}, \texttt{UpwardDepositFacility}.
\item ML pipeline integration: commit--reveal oracle, Authority Brief, full BCCC retrain (Sections~\ref{sec:ml-explainability},~\ref{sec:ml-workload}).
\item MCP conversational agent with human confirmation gate (Section~\ref{sec:optional-agent}).
\item Foundry fuzz / invariant suite (Section~\ref{sec:foundry}).
\item Runtime monitoring: event-listener index and reserve dashboard (Section~\ref{sec:realtime}).
\end{enumerate}
...
This appendix catalogues supplementary industry-research documents prepared for the Crypto World Bank project. They informed Chapter~3 (actor taxonomy), Chapter~\ref{ch:evaluation} (economic feasibility), and the no-native-token design constraint. All files reside in the project repository under \texttt{Documentation/2nd Phase (Bokhtiar)/Improvements/}.
