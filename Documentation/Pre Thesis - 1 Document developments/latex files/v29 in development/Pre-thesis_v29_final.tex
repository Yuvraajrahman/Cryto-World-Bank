...
\begin{tikzpicture}[
  ...
  \node[prisma, fill=black!12, font=\scriptsize\bfseries] (incl) at (0,-4.6)
    {Included in synthesis\\(114 entries in \texttt{references.bib})};
  \draw[arr] (id) -- (dedup);
...
\end{tikzpicture}
\caption[PRISMA-style literature screening flow]{PRISMA-style literature screening flow for the 2017--2026 evidence base. Counts reflect the scoped CWB modules and inclusion criteria in this subsection; the full bibliography contains 114 synthesised entries.}
...
Figure~\ref{fig:protocol-radar} visualises the same comparison on eight differentiating architectural axes (0\,=\,absent, 1\,=\,fully present; partial / planned CWB capabilities scored at 0.65 pending Phase~II--III implementation).
...
\begin{tikzpicture}[scale=0.92, transform shape]
  ...
  \foreach \i/\lbl in {
    ...
    \pgfmathsetmacro{\ang}{90-\step*(\i-1)}
    \draw[black!25, line width=0.35pt] (0,0) -- (\ang:\R);
    \node[font=\scriptsize, align=center, inner sep=1pt]
      at (\ang:\R+0.52) {\lbl};
  }
  \foreach \r in {0.5,1.0,1.5,2.0} {
    \draw[black!18, line width=0.25pt] (90:\r)
      \foreach \k in {1,...,\N} { -- ({90-\step*\k}:\r) } -- cycle;
  }
  % Aave (all zeros on differentiating axes)
  \draw[black!45, line width=0.55pt, dashed]
    (90:0) -- (90-\step:0) -- (90-2*\step:0) -- (90-3*\step:0)
    -- (90-4*\step:0) -- (90-5*\step:0) -- (90-6*\step:0) -- (90-7*\step:0) -- cycle;
  % Maple: institutional focus = 1.0
  \draw[black!55, line width=0.55pt, dash pattern=on 3pt off 2pt]
    (90:0) -- (90-\step:0) -- (90-2*\step:0) -- (90-3*\step:0)
    -- (90-4*\step:0.35) -- (90-5*\step:0) -- (90-6*\step:0) -- (90-7*\step:1.0) -- cycle;
  % Goldfinch: partial group lending + partial institutional
  \draw[black!55, line width=0.55pt, dotted]
    (90:0) -- (90-\step:0) -- (90-2*\step:0) -- (90-3*\step:0.35)
    -- (90-4*\step:0.35) -- (90-5*\step:0) -- (90-6*\step:0) -- (90-7*\step:0.35) -- cycle;
  % CWB (planned / partial on all eight axes)
  \draw[black, line width=0.85pt, fill=black!18, fill opacity=0.35]
    (90:0.65) -- (90-\step:0.65) -- (90-2*\step:0.65) -- (90-3*\step:0.65)
    -- (90-4*\step:0.65) -- (90-5*\step:0.65) -- (90-6*\step:0.65) -- (90-7*\step:0.65) -- cycle;
  \node[font=\scriptsize, anchor=west] at (-3.8,-3.35) {%
    \tikz{\draw[black!45, dashed, line width=0.55pt] (0,0) -- (0.55,0);} Aave\quad
    \tikz{\draw[black!55, dash pattern=on 3pt off 2pt, line width=0.55pt] (0,0) -- (0.55,0);} Maple\quad
    \tikz{\draw[black!55, dotted, line width=0.55pt] (0,0) -- (0.55,0);} Goldfinch\quad
    \tikz{\draw[black, line width=0.85pt, fill=black!18] (0,0) -- (0.55,0);} CWB%
  };
\end{tikzpicture}
\caption[Protocol comparison on eight architectural axes]{Protocol comparison on eight architectural axes that differentiate institutional development finance from flat DeFi pools. CWB scores reflect pre-thesis specification status (planned / partial = 0.65); competitor scores derive from Table~\ref{tab:protocol-comparison} (partial / manual = 0.35).}
\label{fig:protocol-radar}
\end{figure}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-multi-entity-ops.pdf}
...
When a Local Bank's (or National Bank's) single-name exposure exceeds its governance-configured prudent lending ceiling---a threshold set by reserve policy and approver risk limits, not by direct client access to higher tiers---multiple banks at the same or adjacent tier co-fund a single loan through a \texttt{SyndicatedLoan} contract. This is the on-chain analogue of a real-world syndicated loan or club deal, and it directly implements the multi-entity co-lending and co-funding capability described in the introduction (Section~\ref{sec:intro}). In the syndicated model, \textbf{a group of institutional entities pools capital together to fund a borrower that no single entity can or should serve alone}---distributing both the risk and the return pro-rata across all co-lenders. This pattern has no equivalent in any existing DeFi protocol, which universally pool anonymous liquidity without distinguishing institutional relationships or enforcing on-chain consent among named co-lenders.

\paragraph{Roles.}
A syndicated loan has three distinct on-chain roles: a \textbf{Lead Arranger} (typically the originating Local Bank or its parent National Bank), \textbf{Co-Lenders} (participating Local and/or National Banks that supply capital), and the \textbf{Borrower} (the end client or obligor, always onboarded through a Local Bank interface). End clients never contract directly with Tier~1 or Tier~2 institutions. The Lead Arranger holds a small underwriting fee (10--20~basis points) plus a portion of the interest spread; Co-Lenders earn pro-rata interest by capital share.

\paragraph{Lifecycle.}

\begin{enumerate}[noitemsep]
\item \textbf{Proposal.} Lead Arranger calls \texttt{proposeSyndicate(uint256 totalAmount, uint8 exposureClass, uint256 minSubscription)} which mints a syndicate identifier and emits a \texttt{SyndicateProposed} event (\texttt{exposureClass} encodes bank risk classification, not client routing to higher tiers). Off-chain, the deal terms (covenants, schedule) are pinned to IPFS and the content hash is stored in \texttt{syndicateDocHash[id]} for immutable cross-reference.
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-usecase-actors.pdf}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-activity-lending.pdf}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-activity-onboarding-id.pdf}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-activity-aux.pdf}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-dfd-suite.pdf}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-banking-modules.pdf}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-realtime-dashboard.pdf}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-tx-state-machine.pdf}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-sdlc-agile.pdf}
...
\begin{figure}[H]
\centering
\CompactDiagram{fig-optional-agent-addon.pdf}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-dev-toolchain.pdf}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-design-decisions.pdf}
...
\begin{figure}[H]
\centering
\HalfWidthDiagram{fig-revenue-mix-pie.pdf}
\hfill
\HalfWidthDiagram{fig-market-tam-pie.pdf}
\caption[Revenue mix and market-sizing funnel]{Left: revenue mix at full deployment (spread-based interest income dominates). Right: illustrative market-sizing funnel (TAM/SAM/SOM) showing the institutional DeFi whitespace the platform targets.}
\label{fig:revenue-mix-pie}
\label{fig:market-tam-pie}
\end{figure}
...
\midrule
IR-1 & \texttt{Binance\_FTX\_Full\_Report.md} & CEX vs.\ CWB table (Table~\ref{tab:cex-vs-cwb}); FTT/BNB token lesson; SAFU and Binance Earn analogues; PoR and commingling safeguards \\
IR-2 & \texttt{blockchain platforms research.csv} & Five-exchange comparative matrix (Binance, Bybit, Coinbase, OKX, Kraken) for Table~\ref{tab:cex-vs-cwb} \\
IR-3 & \texttt{Binance Business Analysis.md} & Exchange revenue taxonomy (trading fees, listing, OTC, \$16.8B revenue benchmark) for Table~\ref{tab:revenue-taxonomy} \\
IR-4 & \texttt{binance\_software\_engineering\_architecture.md} & Nine-actor formal taxonomy (Section~\ref{sec:onboarding}); use-case and sequence-diagram conventions \\
IR-5 & \texttt{binance\_architecture\_research.md} & Off-chain PostgreSQL/Redis layer design patterns (background only; not formally cited in body text) \\
\bottomrule
\end{tabular}
\end{table}
