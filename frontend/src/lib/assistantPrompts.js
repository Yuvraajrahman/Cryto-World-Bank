export function getFeatureKeyFromPath(pathname) {
    const p = pathname.toLowerCase();
    if (p === "/" || p.startsWith("/#"))
        return "landing";
    if (p.startsWith("/app/dashboard"))
        return "dashboard";
    if (p.startsWith("/app/reserve"))
        return "reserve";
    if (p.startsWith("/app/banks"))
        return "banks";
    if (p.startsWith("/app/loans/new"))
        return "requestLoan";
    if (p.startsWith("/app/loans"))
        return "loans";
    if (p.startsWith("/app/installments"))
        return "installments";
    if (p.startsWith("/app/approvals"))
        return "approvals";
    if (p.startsWith("/app/market"))
        return "market";
    if (p.startsWith("/app/chat"))
        return "messages";
    if (p.startsWith("/app/assistant"))
        return "assistant";
    if (p.startsWith("/app/risk"))
        return "risk";
...
export function getRecommendedPrompts(featureKey, role) {
    const common = [
