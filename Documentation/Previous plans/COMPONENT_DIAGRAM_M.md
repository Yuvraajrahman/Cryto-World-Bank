...
```mermaid
%% Portrait (10:16) layout: reduce horizontal sprawl, improve PDF readability
%%{init: {'flowchart': {'nodeSpacing': 60, 'rankSpacing': 80, 'curve': 'basis'}, 'themeVariables': {'fontSize': '14px'}}}%%
graph TB
    ...
    subgraph SCL["⛓️ SMART CONTRACT LAYER"]
        ...
        subgraph SC_Contracts["Contracts (Solidity 0.8.20)"]
            direction TB
            WBReserve["WorldBankReserve<br/><i>IReserve</i>"]
            WBReserveOps["depositToReserve( )<br/>registerNatBank( )<br/>lendToNatBank( )<br/>pause( ) / unpause( )<br/>emergencyWithdraw( )"]

            NatBankC["NationalBank<br/><i>INationalBank</i>"]
            NatBankOps["registerLocalBank( )<br/>borrowFromWB( )<br/>lendToLocalBank( )"]

            LocBankC["LocalBank<br/><i>ILocalBank</i>"]
            LocBankOps["requestLoan( )<br/>approveLoan( )<br/>rejectLoan( )<br/>payInstallment( )<br/>setApprover( )"]
        end
