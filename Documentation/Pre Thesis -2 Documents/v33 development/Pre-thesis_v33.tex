...
\noindent\textit{Future Work.} A two-circuit privacy-preserving compliance gateway---Groth16 zkKYC (Circom~2.0), zkAML velocity proofs~[R19], and W3C DID/VC anchoring---is motivated by Contribution~4 (Chapter~\ref{sec:research-contribution}) and the literature synthesis in Section~\ref{sec:lit-zkp-defi}. Circuit-level design, verifier deployment, and on-chain integration are deferred to Section~\ref{sec:future-work}. At MVT scope, retail and institutional onboarding uses tiered off-chain KYC, hash-linked credentials in PostgreSQL, and on-chain RBAC role gates (Section~\ref{sec:tiered-kyc}).
