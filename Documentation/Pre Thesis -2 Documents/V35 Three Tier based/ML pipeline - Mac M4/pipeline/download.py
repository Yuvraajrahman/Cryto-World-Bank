...
def _merge_bccc_split(cfg: dict) -> Path | None:
    """Merge official BCCC fraud + legitimate CSVs into one labelled file."""
    split = cfg["dataset"].get("bccc_split")
    if not split:
        return None
    fraud_path = _resolve_path(split["fraud"])
    legit_path = _resolve_path(split["legitimate"])
    if not fraud_path.is_file() or not legit_path.is_file():
        return None

    cache_rel = cfg["dataset"].get("merged_cache", "data/bccc.csv")
    out = _resolve_path(cache_rel) if Path(cache_rel).is_absolute() else (ROOT / cache_rel).resolve()
    out.parent.mkdir(parents=True, exist_ok=True)

    newest_src = max(fraud_path.stat().st_mtime, legit_path.stat().st_mtime)
    if out.is_file() and out.stat().st_mtime >= newest_src:
        print(f"[dataset] Using cached merge: {out}")
        return out

    print(f"[dataset] Merging BCCC split files → {out}")
    print(f"  fraud:       {fraud_path}")
    print(f"  legitimate:  {legit_path}")
    fraud = pd.read_csv(fraud_path, low_memory=False)
    fraud["fraud"] = 1
    legit = pd.read_csv(legit_path, low_memory=False)
    legit["fraud"] = 0
    merged = pd.concat([fraud, legit], ignore_index=True)
    merged.to_csv(out, index=False)
    meta = {
        "synthetic": False,
        "source": "BCCC-DeFiFraudTrans-2025",
        "fraud_rows": len(fraud),
        "legitimate_rows": len(legit),
        "total_rows": len(merged),
        "merged_path": str(out),
    }
    (ROOT / "artifacts").mkdir(exist_ok=True)
    (ROOT / "artifacts" / "dataset_source.json").write_text(json.dumps(meta, indent=2))
    print(f"[dataset] Merged {len(merged):,} rows → {out}")
    return out
...
def acquire(cfg: dict) -> Path:
    _load_dotenv()
    merged = _merge_bccc_split(cfg)
    if merged:
        validate_csv(merged, cfg)
        return merged
