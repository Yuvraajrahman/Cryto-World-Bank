%% =============================================================================
%% Pre-thesis 1 -- Crypto World Bank -- v22 (BRACU front matter + ACM body)
%% -----------------------------------------------------------------------------
...
\paragraph{Multi-signer attestation (interim trust upgrade).}
Until Chainlink Functions is wired, the current design presents a residual single-point trust assumption: a single compromised FastAPI service key can submit an arbitrary score hash, and the commit-reveal pattern proves only that the score was not changed \emph{after} commitment, not that it was computed correctly.  To address this without introducing new infrastructure dependencies, the \texttt{commitRiskScore} function is upgraded to require a \textbf{2-of-3 multi-signer attestation} before the commitment is accepted on-chain.  The three designated signers are: (1) the FastAPI ML service key, (2) the originating Local Bank operator key, and (3) a World Bank governance key.  Each signer independently signs the score hash \(h = \mathrm{keccak256}(s \,\|\, \text{nonce})\); the contract verifies that at least two of the three signatures are valid over the same hash before recording the commitment.  This is implemented using the existing Safe multisig infrastructure (Section~\ref{sec:defense-in-depth}): the World Bank Admin Safe holds the governance key, so no new key-management infrastructure is required.  The attestation requirement does not change the commit-reveal state machine or increase reveal-phase gas cost; it adds one \texttt{ecrecover}-based signature verification at commit time.  This aligns with the human-in-the-loop model already described in the $0.4$--$0.7$ manual-review band: borderline scores require a bank operator to co-sign the commitment, making the attestation requirement structurally consistent with the approval workflow rather than an anomalous addition to it.
...
\item \textbf{On-chain economic feasibility simulation (Sprint~3 completion).} The scripted Hardhat simulation of Section~\ref{sec:abm-sim} is the designated empirical tool for the pre-thesis phase; Sprint~3 completes its full deployment with 50--200 clients across a six-institution hierarchy on Polygon~Amoy, produces the gas-cost table and reserve-dynamics output, and publishes the deterministic seed and transaction manifest to Appendix~C. A separate agent-based model (Mesa or equivalent) calibrated against real on-chain data remains an optional extension for the final thesis, subject to data availability.
...
The current prototype treats the assistant as a read-only product guide over static policy documents; it cannot answer questions about a specific user's account state.  A straightforward enhancement---requiring no change to the LLM's read-only, no-signing stance---is a \textbf{live context injection layer} in the Express.js backend.  Before forwarding the user's message to the LLM, the backend queries the user's authenticated on-chain state via a batch \texttt{eth\_call} to the relevant contracts and appends a structured JSON context block to the system prompt:
\begin{verbatim}
{
  "loanStatus": "PENDING_REVIEW",
  "riskTier": 2,
  "openLoans": 1,
  "poolUtilisation": 0.71,
  "sbtLastDefault": null,
  "kycLevel": 2
}
\end{verbatim}
This context block is injected only when the user is authenticated and the \texttt{eth\_call} round-trip completes within a 200~ms timeout (falling back to a static-context response otherwise).  The LLM remains stateless and read-only; it uses the injected context to ground its answers in the user's actual account data rather than generic policy text.  For example, a user whose loan is in the \texttt{PENDING\_REVIEW} state and whose risk tier is~2 would receive a response such as: \emph{``Your application is currently under manual review because your composite risk score fell in the 0.4--0.7 range, which triggers the mandatory review band described in our lending policy.  Your risk tier is 2, which means a Senior Approver must countersign before disbursement.''}\  This response is grounded in the injected state rather than generated from training-set generalizations, materially reducing hallucination risk for account-specific queries.  The backend change is approximately 20~lines of additional TypeScript in the chat route handler and does not affect the model's safety posture, token budget, or latency target.
