...
\begin{description}[leftmargin=0pt]
    \item[Utilization Rate (U):] \(U = \frac{L}{L + B}\) (lending utilization; \(L\) is lent amount, \(B\) is available balance/liquidity; drives the kinked rate model in Formulas~18--19)
    \item[Kinked Borrowing Rate (Formula~18):] \(r_b(U) = r_0 + \frac{U}{U^*} \cdot r_1,\quad U \leq U^*\) (gentle slope below optimal utilization \(U^*\); base rate \(r_0\), below-kink slope \(r_1\); enforced on-chain in retail and interbank pools; Section~\ref{sec:kinked-rate})
    \item[Kinked Borrowing Rate (Formula~19):] \(r_b(U) = r_0 + r_1 + \frac{U - U^*}{1 - U^*} \cdot r_2,\quad U > U^*\) (steep slope above \(U^*\) to incentivise repayment and new deposits; jump multiplier \(r_2\); on-chain parameter guards apply before updates)
    ...
    \item[Health Factor (HF):] \(HF = \frac{C \times LT}{D}\) (liquidation trigger for \textit{over-collateralized retail loans only}; if \(HF < 1.0\) liquidation is triggered, where \(LT\) is liquidation threshold and \(C\) is oracle-priced collateral; Section~\ref{sec:liquidation})
    \item[Group Health Factor (\(HF_{\text{group}}\)):] \(HF_{\text{group}} = \frac{\text{total\_group\_collateral} \times LT}{\text{total\_group\_outstanding\_debt}}\) (pool-level liquidation score for \texttt{GroupLendingPool}; shared shortfall across members)
    \item[Institutional Health Factor (\(HF_{\text{inst}}\)):] \(HF_{\text{inst}} = \frac{\text{on\_chain\_reserve} - \text{min\_reserve}}{\text{outstanding\_borrowing}}\) (Tier~1--3 reserve-surplus analogue to collateral; parent-tier surplus above mandatory minimum reserve)
    ...
    \item[Interbank Rate Cap:] \(r_{\text{IB}}(U) \leq r_{\text{downward}}(U) - \delta\) (same-tier \texttt{InterBankLendingPool} borrowing rate capped below parent-tier downward rate; \(\delta\) is governance-set anti-arbitrage spread; Section~\ref{sec:interbank-pool})
    \item[Upward Deposit Rate Cap:] \(r_{\text{up}} < r_{\text{down}} - \delta\) (asymmetric tier spread for \texttt{UpwardDepositFacility}; upward yield stays below downward lending rate; Section~\ref{sec:upward-flow})
    \item[FX Conversion:] \(\text{Amount}_B = \text{Amount}_A \times \left(\frac{P_A}{P_B}\right)\) (oracle-priced conversion between assets \(A\) and \(B\); Chainlink \texttt{AggregatorV3Interface} feeds)
    \item[Treasury Swap (oracle-priced):] \(\text{Amount}_{\text{to}} = \text{Amount}_{\text{from}} \times \frac{P_{\text{from}}}{P_{\text{to}}} \times (1 - \text{spread})\) (atomic cross-asset swap in \texttt{TreasurySwap}; \(P\) from Chainlink feeds; treasury spread 5--10~bps; Section~\ref{sec:treasury-fx})
    \item[Oracle Commit--Reveal Hash:] \(h = \text{keccak256}(s \,\|\, \text{nonce})\) (prototype-phase ML score commitment to \texttt{LoanController}; contract verifies \(h = \text{keccak256}(s \,\|\, \text{nonce})\) before storing score \(s\); immutable on-chain audit trail; Section~\ref{sec:oracle-architecture})
...
The system is designed to serve three categories of participants who are systematically underserved by existing DeFi infrastructure---not as end consumers of a blockchain application, but as institutional actors who intermediate between wholesale capital and underserved borrowers. \textbf{First,} national development banks and regional financial institutions in emerging markets that depend on correspondent banking for cross-border capital allocation face the \$42-per-transaction settlement cost and 2--5~day delays documented by BIS~[25]---not because they lack technology, but because they rely on pre-funded nostro/vostro accounts in opaque bilateral relationships. \textbf{Second,} local microfinance institutions (MFIs) and cooperative banks---institutions like those in Bangladesh's regulated MFI sector, where female-to-male account parity reached 49\%/49\% in 2025~[R44]---possess formal licensing and KYC infrastructure but lack programmable, auditable credit facilities and reserve management tools. \textbf{Third,} the semi-formal clients of those MFIs---borrowers with some digital identity or mobile account access but without structured installment-based credit---are reachable through MFI-sponsored onboarding without requiring the platform to resolve fiat on-ramp infrastructure in the prototype scope. Development-finance literature cites large unbanked and MSME financing gaps~[14, 20] as evidence of structural intermediation failure; the CWB contribution targets the institutional layer above end users. Evidence from CBDC deployments targeting financial inclusion---including Nigeria's eNaira, which saw 98.5\% wallet inactivity one year post-launch~[161]---confirms that primary barriers are infrastructural and trust-based, not technical, reinforcing the institutional-intermediary design focus. Design artifacts, specifications, and supporting material for this project are available in our \href{https://github.com/Yuvraajrahman/Cryto-World-Bank}{GitHub repository}.
...
A \textbf{smart contract} is a program deployed to a blockchain's state machine---on the Ethereum Virtual Machine (EVM), this means a deterministic function executed by every validating node simultaneously. Its key properties distinguish it sharply from conventional software: (1)~\textbf{Governance-bounded execution}: once deployed, contract logic cannot be modified by any single party unilaterally. Modifications require a recorded, multi-signature governance vote subject to a minimum 48-hour timelock delay between proposal and execution (Section~\ref{sec:governance-framework}). This prevents flash-loan governance attacks---the attack class responsible for the \$182M Beanstalk exploit (April~2022), in which an attacker acquired a supermajority of governance tokens in a single block and immediately executed a malicious proposal~[158]. The CWB governance specification requires: (a)~a snapshot of voting power at the proposal creation block, not at execution time; (b)~a 60\% quorum floor for contract upgrade proposals; (c)~a 48-hour \texttt{TimelockController} delay before any upgrade becomes executable; and (d)~a 2-of-3 emergency pause multisig that can halt operations but cannot bypass the upgrade timelock. (2)~\textbf{Atomic settlement}: a multi-step operation---such as releasing a loan, recording a repayment, and updating a reserve ratio---either completes entirely or reverts entirely; there is no half-settled state. (3)~\textbf{Governance-bounded trustless enforcement}: the contract enforces its rules against \textit{any} caller, including the institution that deployed it, within the bounds of the governance process specified in Section~\ref{sec:governance-framework}; no administrator can unilaterally override a reserve-ratio check or bypass a confirmation gate outside that process. (4)~\textbf{Transparent audit}: every function call, parameter, and state change is permanently recorded on the chain and queryable by any participant.
...
Primary interaction is through the React UI and wallet signing; a locally hosted conversational agent (Qwen3-8B + MCP tool server, 17 banking tools) is a \textbf{planned Phase~III deliverable} that answers policy questions and executes banking operations on behalf of retail clients, subject to a mandatory human confirmation gate and EIP-7702 scoped session keys. Every write operation available through the agent is equally available through the web interface; the agent does not replace core banking workflows. Application-layer enforcement (confirmation audit hook, HTTP~403 on missing confirmation) is specified in Chapter~4.
...
\item \textbf{Absence of programmable reserve-management tools.} National and local MFIs lack real-time, auditable reserve-ratio enforcement and programmable capital-allocation rules. Reserve adequacy is predominantly self-reported and audited at most quarterly, providing no mechanism for wholesale lenders or regulators to verify institutional financial health between audit cycles.

\item \textbf{Inaccessibility of programmable group credit mechanisms.} Many borrowers lack individual collateral but could access credit through solidarity group structures demonstrated in microfinance literature (BRAC, Grameen Bank, ASA). No existing DeFi protocol models programmable mutual liability or on-chain group repayment enforcement with MFI-appropriate stress sizing.
\end{enumerate}
...
\item To plan testnet deployment on Polygon zkEVM Cardona (with Ethereum Sepolia retained only as a Certora formal-verification target in Phase~IV) and evaluate hierarchical controls, transparency properties, and operational workflows in the final thesis phase.
...
We propose an architectural pattern for integrating off-chain Random Forest fraud detection, Isolation Forest anomaly detection, and SHAP- plus deviation-report explainability (Section~\ref{sec:ml-explainability}) into an on-chain lending decision workflow via a trusted oracle relay, providing a blueprint for auditable, AI-assisted credit governance in DeFi. The design addresses the oracle problem---the challenge of securely bridging on-chain logic with off-chain data---and specifies three concrete integration options (centralized relay, Chainlink Functions, and commit-reveal scheme) with explicit trust trade-offs. The centralized relay is a Phase~I--II development tool only; Chainlink Functions is the production oracle from Phase~III onward. Oracle calls are priced at approximately \$0.10--\$1.00 per invocation; at a \$25{,}000 loan this is negligible, but at a \$500 micro-loan a single \$1.00 oracle call can exceed 0.2\% of annual interest---establishing a practical minimum ticket size of approximately \$500 for oracle-mediated ML scoring. Explainability outputs are audience-specific: plain-language decline reasons for retail clients, an Authority Brief dashboard for bank approvers, and a full inference audit trail in \texttt{LOAN\_RISK\_ASSESSMENT} (FK to \texttt{MODEL\_REGISTRY}) for compliance reviewers; system-wide AML alerts append to \texttt{SECURITY\_EVENT\_LOG}.

\textit{Implementation scope:} The Random Forest fraud detector is trained and benchmarked on the BCCC-DeFiFraudTrans-2025 dataset~[R47] (1,026,867 annotated DeFi transactions, 79 features), with cross-validation against the Elliptic Bitcoin Dataset~[R48]. Datasets, training steps, feature mapping, and pre-submission deliverables are documented in Section~\ref{sec:ml-workload}; explainability is formalised in Section~\ref{sec:ml-explainability}. Oracle wiring to the \texttt{LocalBank} contract via Chainlink Functions is scoped to Phase~III; the conversational agent (MCP + Qwen3-8B) is likewise a Phase~III Must deliverable (MVT item~11). The architectural specification, ML workload plan, agent pipeline, benchmark, and explainability design together constitute the Contribution~3 claim at pre-thesis stage.

\paragraph{Extension --- Conversational agent architecture.}
Contribution~3 is extended by a conversational agent layer (Qwen3-8B + MCP, human confirmation gate, EIP-7702 session keys). The six-step pipeline, Authority Brief for approvers, and \texttt{agent\_action\_log} audit trail are specified in Chapter~4; implementation is a \textbf{Phase~III Must deliverable} (LLM evaluation in Phase~IV), running alongside---not replacing---the React UI and on-chain lending core.

\item[\textbf{Contribution 4 --- Compliance-Aware ZKP / zkAML Identity Pathway.}]
We design a two-circuit privacy-preserving compliance architecture combining a zk-SNARK KYC verifier (Groth16 proofs via Circom~2.0 + snarkjs) with a zkAML circuit~[R19] that proves a wallet has not transacted with sanctioned addresses and remains within velocity limits, without revealing the underlying transaction graph. Both circuits compose with a W3C Decentralized Identifier (DID) and Verifiable Credential (VC) layer~[R20] anchoring user identity off-chain. On-chain Soulbound Tokens (SBTs) for credit passports create a complementary privacy risk: repayment history becomes permanently public on-chain. This risk is mitigated by composing off-chain VC storage with ZKP selective disclosure (Contribution~4), so only eligibility proofs---not raw credit attributes---appear on-chain. This positions the platform's compliance gateway against the broader Self-Sovereign Identity (SSI) framework and is grounded in the Piper et al. (2025) TU Berlin permissioning paper~[R8].

\textit{Implementation scope:} The Groth16 KYC age-range circuit is \textit{specified} in Circom~2.0 (Poseidon hash commitment for proof freshness); on-chain verifier deployment on Polygon~zkEVM~Cardona and zkAML velocity proofs~[R19] are scoped to Future Work (Section~\ref{sec:future-work}). At MVT scope, retail and institutional onboarding uses tiered off-chain KYC, hash-linked credentials in PostgreSQL, and on-chain RBAC role gates (Section~\ref{sec:tiered-kyc}).
...
\item \textbf{Programmable, governance-bounded rule enforcement via smart contracts.} The lending rules---reserve ratios, interest rate curves, borrowing limits, approval workflows, multi-entity consent requirements---are deployed as code on the EVM. Once deployed, modifications require the governance process in Section~\ref{sec:governance-framework} (48-hour timelock, 60\% quorum, snapshot voting). Within those bounds, rules execute \textit{identically for every participant and cannot be selectively overridden by any party}, including the institution that deployed the contract.
...
\end{itemize}
Each installment repayment involves at minimum one \texttt{SSTORE} for updating the loan balance, one \texttt{SSTORE} for recording payment history, and one event emission. A 12-installment loan therefore involves at minimum 24 \texttt{SSTORE} operations. A complete retail loan lifecycle involves approximately 27--32 individual on-chain \textit{state changes} (\texttt{SSTORE} writes and event emissions), versus 10--15 user-facing \textit{transactions} (loan request, approval, disbursement, and installment payments)---the figures are reconciled in Section~\ref{sec:gas-cost}.
...
Smart contracts are deterministic and isolated from external data by design. This is a feature---consensus would break if different nodes observed different external states---but it creates a fundamental architectural gap for any system that requires off-chain data. The oracle problem, formally identified by Beniiche~(2020)~[R1] and Pasdar et al.~(2023)~[R2], is the challenge of securely bridging on-chain logic with off-chain information. For the Crypto World Bank, the AI/ML risk scores produced by the FastAPI Random Forest service are off-chain data that must influence on-chain lending decisions. Three architectural options exist: (1)~a centralized relay (trusted backend calls \texttt{updateRiskScore} on-chain---a Phase~I--II development tool only); (2)~a decentralised oracle network such as Chainlink Functions (production oracle from Phase~III; 30--60~seconds latency and \$0.10--\$1.00 per call; the DON requires consensus across multiple independent nodes before a score is committed); and (3)~a commit-reveal scheme (the ML service commits a hash of the risk score before the loan decision window, reveals it after). The pre-thesis design uses the centralized relay for early integration testing; Chainlink Functions is the primary production mechanism from Phase~III.

An empirical study of Chainlink price-feed accuracy across eight blockchain networks, using 50~million data points collected in December~2024, finds that accuracy varies meaningfully by chain~[157]: Polygon achieves consistently high accuracy, while Ethereum mainnet and zkSync exhibit greater deviations under market volatility.
This measurement result supports consolidating all live deployment on Polygon zkEVM Cardona (single-chain architecture, Section~\ref{sec:bridge}) and quantifies the oracle accuracy trade-off that the commit-reveal fallback is designed to hedge.

In addition to ML score commitment, Chainlink Price Feeds supply fiat/USD pairs (e.g.\ EUR/USD) and ETH/USD rates to the \texttt{FXModule} contract, Chainlink Automation replaces the centralised cron job for overdue installment detection, and Chainlink Proof of Reserve publishes the \texttt{WorldBankReserve} balance for external cryptographic verification.
...
Our approach combines a lightweight Agile delivery process with an explicit Software Development Life Cycle (SDLC) frame.
Pre-Thesis~1 covers the requirements, architecture, and design phases;
the implementation, verification, and validation phases run across four
subsequent phases during the final thesis stage.
Table~\ref{tab:phase-deliverables} maps each phase to its primary
deliverables and target networks.
...
III &
  ...
  (Chainlink Functions oracle), SyndicatedLoan,
...
\rowcolor{RowShade}
IV &
  Certora reserve-invariant formal proofs,
  zkAML velocity circuit, full audit pipeline &
  All contracts; zkSNARK verifier on-chain &
  Polygon zkEVM Cardona; Ethereum Sepolia (Certora only) \\
\bottomrule
\end{tabular}%
}
\end{table}

\noindent The technology stack is Solidity~0.8.20 (smart contracts),
React~18 with TypeScript and ERC-4337 account abstraction (frontend),
Express.js + FastAPI (backend), scikit-learn + PyTorch Geometric (ML),
and Circom~2.0 + snarkjs (ZKP); optionally ChromaDB with a locally
hosted 8B-parameter LLM (RAG assistant, Phase~III--IV Must/Should deliverable).
All planned live deployments target Polygon~zkEVM~Cardona;
Ethereum~Sepolia is retained only as a Certora formal-verification target in Phase~IV.
Stablecoin pools use testnet USDC.
...
The most important problem facing any blockchain-based lending system is whether the revenue earned by the lending activities will be in a sustainable position to cover the total gas expenses of the transactions on-chain. Every level in the hierarchy will have deposits, loan applications, approvals, disbursements, repayment and installment processing gas fees. One complicated smart contract interaction can cost between \$5 and \$50 on Ethereum mainnet, based on network congestion, but around \$0.001 to \$0.01 on Polygon zkEVM Cardona. At an average retail loan of \$25{,}000 USDC with an 8\% APR, the client pays \$2{,}000 in annual interest. The loan lifecycle comprises 10--15 user-facing transactions (request, approval, disbursement, monthly installments) but approximately 27--32 on-chain state changes when \texttt{SSTORE} writes and event emissions are counted (Section~\ref{sec:gas-cost}); overall gas on Polygon remains under \$0.15---less than 0.01\% of interest obtained. Oracle-mediated ML scoring adds \$0.10--\$1.00 per Chainlink Functions call; this is negligible above approximately \$500 minimum ticket size but material for smaller loans. The platform thus provides a sustainable margin on Layer~2 networks, but mainnet implementation would need gas optimisation or batched processing for micro-loans.
...
\item \textbf{Single-chain Layer~2 deployment.} An initial design considered multi-chain deployment bridging Polygon zkEVM Cardona and Ethereum Sepolia; this was revised after security analysis identified cross-chain bridges as the highest-risk DeFi attack surface (Ronin, Wormhole, Nomad exploits collectively exceeded \$1.1B in 2022). All live contracts deploy on Polygon zkEVM Cardona, whose ZK validity proofs are verified on Ethereum L1 without a live asset bridge. Ethereum Sepolia is retained only for Certora formal verification in Phase~IV (Section~\ref{sec:bridge}).
...
The Crypto World Bank targets Tier~1--3 institutional participants (national development banks, regional MFIs, local cooperative banks) and Tier~4 retail clients onboarded through MFI-sponsored pathways on Polygon zkEVM Cardona. The hierarchical architecture (World Bank $\to$ National $\to$ Local $\to$ Client) mirrors development-finance intermediation while providing blockchain-enforced transparency and oracle-mediated risk assessment. Structural unbanked and MSME financing statistics~[14, 20] inform the literature context for intermediation failure; they do not define the prototype's primary deployment target. Full client segment profiling is presented in Chapter~5 (Table~\ref{tab:customer-segment-ch5}).
...
\end{tikzpicture}
\caption[PRISMA-style literature screening flow]{PRISMA-style literature screening flow for the 2017--2026 evidence base. After title/abstract screening, 114 records underwent full-text assessment ($614 - 500$); 91 met inclusion criteria and 23 were excluded as tangential to CWB modules. Seventeen additional entries were identified outside the systematic database search (policy documents and grey literature), plus eight sources added in the 2024--2026 IEEE/ACM security and ZKP update, yielding 116 synthesised bibliography entries.}
\label{fig:prisma-flow}
\end{figure}

\section{Review of Existing Research}

The Crypto World Bank design draws on peer-reviewed materials and institutional reports across DeFi lending, ML fraud detection, hierarchical development finance, smart-contract security, stablecoin regulation, and---where relevant---scoped future extensions (GNN, federated learning, retail payments). Each subsection below establishes a gap the current build fills, justifies a specific design decision, or benchmarks a deliverable against prior work.

\subsection{DeFi Lending and the Structural Gap}

In their SoK paper, Werner et al.~[1] systematize DeFi protocol design and confirm that current lending markets are homogeneous, pool-based, and over-collateralized, with no institutional hierarchy. Their taxonomy demonstrates that no current DeFi system can represent multi-tier capital flow in the manner of development finance---establishing the design space Contribution~1 enters. Bastankhah et al.~[2] present a data-driven lending protocol with adaptive dual fast/slow control, demonstrating that dynamic interest rate adjustment outperforms the static utilization curves used by Aave and Compound~[59]; this validates the kinked-rate design in Section~\ref{sec:kinked-rate}. Aspris and Svec~[118] formalise institutional risk curation as a modular curator layer in decentralised credit markets---the closest architectural precedent to tier-governed capital allocation---but without multilateral development-finance hierarchy or reserve-ratio enforcement at four tiers. Yang and Cui~[47] provide a quantitative evaluation framework for DeFi lending protocols---utilization ratio stability, liquidation efficiency, and interest rate responsiveness---applicable as benchmarks for the CWB hierarchical tiers. Dao et al.~[60] demonstrate that credit scoring models can be adapted for DeFi borrowing limits within decentralized protocols.

\subsection{Fraud Detection and Explainable AI}
\label{sec:lit-ml-oracle}

Palaiokrassas et al.~[3] use machine learning on multichain DeFi fraud over 54~million transactions across 23 protocols, showing that behavioral characteristics raise Neural Network and XGBoost classifiers to F1-scores of 0.76--0.85 versus 0.08 with transactional features alone---directly informing the Random Forest feature-engineering plan. The BCCC-DeFiFraudTrans-2025 dataset (1,026,867 annotated transactions, 79 features)~[108] serves as the primary supervised training corpus (Section~\ref{sec:ml-workload}). A 2025 systematic review spanning 1,585 peer-reviewed studies~[155] reports XGBoost at approximately 95\% accuracy and FedAvg-based federated approaches at approximately 91\%, positioning the CWB pipeline against the broadest available evidence base. LookAhead~[152] (ACM FSE~2025) achieves F1~0.8966 on labelled DeFi attack datasets with 0.16\% false-positive rate---the current benchmark ceiling for the security audit pipeline.

Liu et al.~[6] introduce Isolation Forest, which identifies anomalies via recursive partitioning and shorter path lengths for outliers. This algorithm is specified as the secondary unsupervised model when labeled fraud data are limited, enabling detection of attack patterns absent from the training set. Caldarelli~(2025)~[123] notes that ML-score delivery for lending governance---not price feeds---via blockchain oracles remains under-researched, validating the novelty of the oracle-mediated risk-scoring pattern in Contribution~3.

Adom et al.~[4] compare LIME and SHAP on loan approval datasets: SHAP offers more consistent, deeper feature attributions via Shapley values, while LIME is faster but less stable across repeated evaluations---motivating SHAP as the primary explainability method. Yuan~[56] demonstrate SHAP-based credit default models achieving AUC above 0.92 with full per-client interpretability, informing the Authority Brief dashboard where approvers review SHAP waterfall plots before lending decisions.

\subsection{Hierarchical Capital Distribution}
\label{sec:lit-cross-tier}

Tan~[5] constructs an IMF model in which CBDCs use a two-tier distribution architecture (central bank $\to$ commercial banks $\to$ users) that parallels the CWB four-tier hierarchy. The joint BIS report~[53] identifies the same two-level distribution model as the preferred retail CBDC architecture. Kiff et al.~[134] survey retail CBDC operating models in which the central bank issues CBDC but delegates wallet and payment services to supervised intermediaries; Auer and B\"{o}hme~[135] propose hybrid CBDC architectures combining direct central-bank claims with intermediary-delivered payment services---paralleling the CWB pattern in which retail clients access credit only through Local Banks while reserve authority remains at Tier~1.

Bindseil~[133] analyses tiered CBDC remuneration as a mechanism to preserve bank intermediation while granting public access to central-bank liabilities, directly informing the asymmetric upward/downward rate spread planned for the \texttt{UpwardDepositFacility} (Section~\ref{sec:upward-flow}).

On programmable implementation, the New York Fed / BIS \textit{Project Pine} prototype~[131] demonstrates a hierarchy of smart-contract factories, exchange contracts, and collateral pools for central-bank market operations in tokenized wholesale markets---validating layered contract design for \texttt{InterBankLendingPool} and \texttt{SyndicatedLoan}. Dong, Qiu, and Xu~[132] model blockchain-enabled cross-tier direct financing in a three-tier supply chain, showing that transparent ledger visibility enables capital to reach capital-constrained deep-tier participants---a structural parallel to downward allocation reaching retail clients who cannot access wholesale tiers directly.

Together, these sources establish multi-directional capital flows in policy and operations research but not yet composed into a single DeFi lending specification; the gap the CWB architecture addresses (Figure~\ref{fig:cross-tier-lending}).

\subsection{Group Lending and Microfinance}

The solidarity group lending model, pioneered by Grameen Bank and scaled by BRAC, demonstrates that peer-monitored mutual liability can achieve repayment rates exceeding 95\% among borrowers with no individual collateral---attributable to weekly meetings, field-officer oversight, and community reputation that on-chain contracts cannot replicate. The CWB \texttt{GroupLendingPool} encodes mutual liability in programmable logic but expects lower initial on-chain repayment rates until credit history accumulates. Asaduzzaman et al.~[54] show that smart contract-based microcredit management can reduce administrative overhead by up to 60\% versus manual processes---the economic case for programmable group lending at the Local Bank tier.

A 2025 IEEE study introduces a blockchain-powered microloan platform using Ethereum smart contracts, MetaMask, and React.js for on-chain application, ranking, repayment, and wallet management~[156]---the most directly analogous published prototype to the CWB Local Bank interface and the closest technical precedent to \texttt{GroupLendingPool}. The economic foundation for mutual liability is Jaffer's solidarity lending mechanics~[119]. Sherratt~[120] documents that mutual liability in analogue MFIs can become coercive---a risk the on-chain design mitigates by eliminating human collection pressure, at the cost of losing the social monitoring that drives analogue repayment rates. G\"{u}\c{c}\"uk et al.~[61] confirm that privacy and trust mechanisms are critical in blockchain-based microlending, reinforcing ZKP-based credential design for group member onboarding.

\subsection{Smart Contract Security and Governance}

Atzei et al.~[7] provide the foundational taxonomy of Ethereum smart-contract attack vectors---reentrancy, access control failures, and integer overflow---informing \texttt{ReentrancyGuard}, Solidity~0.8.20 overflow protection, and RBAC modifiers. Chaliasos et al.~[150] (IEEE/ACM ICSE~2024) find that existing security tools could have prevented only 8\% of real-world DeFi exploit losses in their dataset---all preventable cases being reentrancy---forcing a layered pipeline rather than reliance on any single tool. Chen et al.~[151] (IEEE/ACM ICSE~2026) demonstrate that layered contract architectures introduce new control-flow attack surfaces requiring runtime monitoring beyond pre-deployment analysis---motivating Tenderly alerts (Phase~III) alongside Slither and Mythril in the defense-in-depth stack (Section~\ref{sec:defense-in-depth}).

ContractWard~[50] (F1~0.96 reentrancy, 0.93 access control), SoliAudit~[51] (70\% audit-time reduction), and VERISMART~[52] (98.4\% accuracy) form the benchmark hierarchy for static and formal tooling; LookAhead~[152] supersedes them on DeFi-specific adversarial-contract detection (F1~0.8966). Flash-loan governance attacks---in which an attacker borrows a supermajority of governance tokens within one block and passes a malicious proposal atomically (Beanstalk, \$182M, April~2022)~[158]---and the three primary DAO threat vectors classified in \textit{SoK: Attacks on DAOs}~[159] directly generate the snapshot voting, 48-hour timelock, and 60\% quorum floor in Section~\ref{sec:governance-framework}. MEV front-running on loan approvals and liquidations~[160] is mitigated via commit-reveal liquidation triggers and Polygon zkEVM Cardona deployment, where ZK validity constraints reduce (though do not eliminate) sequencer-ordering risk.
...
The EU MiCA Regulation (fully effective December~2024)~[R36, R37] requires authorized issuers, 1{:}1 reserve backing, and regular audits for electronic-money tokens covering USDC and USDT at retail tiers. The United States GENIUS Act (July~2025)~[R38] creates the first federal payment-stablecoin framework with similar reserve and disclosure obligations---together making USDC-first denomination a regulatory requirement, not merely a preference (Section~\ref{sec:stablecoin-first}). BIS WP~905~[R13] excludes algorithmic stablecoins from emerging-market DeFi deployment.

Beniiche~[R1] and Pasdar~[R2] establish the oracle problem for DeFi lending governance. An empirical Chainlink accuracy study across eight chains~[157] finds Polygon achieves consistently high oracle accuracy versus Ethereum and zkSync under market volatility---justifying single-chain Polygon zkEVM Cardona deployment and the Chainlink Functions oracle choice (Section~\ref{sec:blockchain-fundamentals}).

BIS and FSB analyses~[21, 33] document correspondent-banking inefficiencies: 2--5 day settlement and approximately \$42 per cross-border transaction. The World Bank remittance brief~[26] reports 6.49\% average cost for a \$200 transfer. These figures motivate on-chain settlement with near-instant finality on Layer~2.

Kim and Kim~[55] show that transaction batching, calldata compression, and tactical timing reduce gas costs by 30--50\% on Ethereum mainnet; Hartmann and Hasan~[48] analyse peer-to-peer lending gas minimisation strategies that inform the four-tier transaction cost model. Chen et al.~[46] validate multi-chain lending throughput even though the CWB revised to single-chain deployment after bridge risk analysis (Section~\ref{sec:bridge}).

Buterin, Hitzig, and Weyl~(2022)~[R30] introduce Soulbound Tokens (SBTs) as non-transferable on-chain credentials; MicroSave Consulting~(December~2025)~[R31] documents the microfinance sector's shift toward performance-based credit identity---together motivating the on-chain credit passport in Section~\ref{sec:credit-passport}.

Ozili~[161] documents Nigeria's eNaira at 98.5\% wallet inactivity one year post-launch, confirming that inclusion barriers are trust-based and institutional rather than purely technical---justifying MFI-intermediary distribution over direct end-user enrollment.

Sygnum Bank's \textit{Institutional DeFi in 2025} report~[R21] finds that pension-fund allocators withhold participation until legal enforceability is resolved. BIS mBridge~[R39] and \textit{Project Agora}~[R40] provide settlement rails without retail lending hierarchies; World Bank FundsChain~[39] and JPMorgan Kinexys~[40] confirm institutional blockchain adoption for fund tracking and settlement---positioning the CWB as a composable lending-layer specification on top of such infrastructure.

\subsection{Zero-Knowledge Proofs for Compliance-Aware DeFi Identity}
\label{sec:lit-zkp-defi}

Two 2026 \emph{ACM Transactions on the Web} papers demonstrate that combining Zero-Knowledge Proofs, Self-Sovereign Identity, DIDs, and smart contracts enables privacy-preserving KYC across platforms without revealing personal data~[153, 154]. Piper et al.~(2025)~[R8] grounds the on-chain permissioning layer. Together with zkAML velocity proofs~[R19], these works motivate Contribution~4's two-circuit compliance gateway; circuit implementation (Groth16 in Circom~2.0) is scoped to Future Work (Section~\ref{sec:future-work}).
...
A parallel research stream demonstrates that DeFi fraud is fundamentally relational---coordinated borrowing rings and sybil attacks appear in the wallet-interaction graph, not in individual transaction features. Cheng et al.~[R24], Chang et al.~[121], and DeFiGuard~[R25] consistently outperform flat ML baselines, with detection above 70\% of illicit transactions at sub-1\% false-positive rates on the Elliptic dataset. The CWB's planned GNN extension treats this as an ablation isolating the marginal value of tier-relationship edges over flat wallet graphs---scoped to Future Work (Section~\ref{sec:future-work}); this pre-thesis report does not specify GNN pipeline details.

Hasan et al.~[49] and the PrivChain-AI study~[R27] demonstrate that federated learning allows institutions to collaborate on fraud detection without sharing raw transaction data, reaching 94.7\% accuracy across multiple banks. Alhasawi et al.~[125] report F1$=$0.90 on credit-card fraud with trust-aware FL aggregation---the direct baseline for a cross-tier CWB extension. The four-tier structure, where National and Local Banks are independent legal entities that cannot share raw client data, makes FedAvg with differential privacy architecturally appropriate; this extension requires dedicated FL infrastructure beyond the Phase~I--IV build and is scoped to Future Work.

A substantial literature examines stablecoin-denominated retail payment rails---client-to-client remittance, merchant checkout, and fiat on/off-ramps---under compliance constraints [Ante~136, Du~138, Li~137, Zhang~140, Fr\"{o}hlich~139, Lin~141, CPMI~142, FATF~145]. These works establish that retail payment extensions are empirically studyable but that no prior work composes hierarchical development-finance lending with compliance-gated remittance and PSP-mediated ramps in a single four-tier architecture. The CWB post-MVT roadmap addresses this gap in Section~\ref{sec:future-retail-payments}; the literature is not further reviewed here as none of these papers shapes the current system design.

The conversational agent layer faces two documented risks from the LLM-in-finance literature~[R35]: regulatory hallucination on compliance questions without domain fine-tuning, and prompt injection through user-controlled input fields [Alizadeh~R53]. OWASP LLM Top~10~[R54] classifies prompt injection as the highest-priority threat in agentic financial systems, corroborated by a February~2026 Microsoft advisory on MCP-based agent runtimes~[R55]. The scanning middleware and human confirmation gate in Section~\ref{sec:optional-agent} address both risks; implementation is a Phase~III Must deliverable.

\section{Literature Review Summary}

Table~\ref{tab:lit-summary} consolidates the fifteen highest-impact sources. Papers below Tier~1 relevance are cited at point of use in the subsections above.
...
\rowcolor{RowShade}
Jaffer [119] / IEEEMicroloan 2025 [156] & Solidarity mechanics + most recent blockchain microloan prototype & GroupLendingPool mutual liability design & Contribution 2 \\
...
\noindent\textit{Papers below Tier~1 relevance are cited at point of use in the subsections above. The full bibliography of 116 entries is available in the references section.}

\section{Comparative Protocol Analysis}
\label{sec:comparative-protocol}

Table~\ref{tab:protocol-comparison} compares the Crypto World Bank against representative DeFi protocols on eleven architectural dimensions. No existing protocol simultaneously enforces a four-tier reserve-ratio hierarchy, oracle-mediated explainable ML governance, and programmable mutual-liability group lending---directly answering RQ1 and supporting the compound novelty claim in Section~\ref{sec:research-contribution}.
...
The institutional tiers (Tiers~1--3) use bespoke RBAC (Section~\ref{sec:defense-in-depth}) for transfer-level access control. ERC-3643 gates token movements at the contract level for regulated institutional transfers; the platform's RBAC role gates serve the same functional purpose at the application layer. A production deployment under MiCA~[R36] would evaluate full ERC-3643 compliance as the interoperability path; the RBAC structure maps directly onto the ERC-3643 identity registry model.

\subsection{Literature Synthesis}

\textbf{(1) The gap.} Werner et al.~[1] establish that DeFi lending is uniformly flat and pool-based; Tan~[5] and the BIS CBDC literature~[53, 134] confirm that tiered distribution is the institutional standard; Aspris and Svec~[118] provide the closest modular-curator precedent but stop short of multilateral development-finance hierarchy. No prior work combines these into a single four-tier lending specification---the gap Contribution~1 addresses and RQ1 tests.

\textbf{(2) The ML and security baseline.} Palaiokrassas et al.~[3] show behavioral ML achieves workable fraud detection (F1 0.76--0.85); Adom et al.~[4] confirm SHAP gives regulatory-grade explainability; Atzei et al.~[7] and Chaliasos et al.~[150] together show that existing security tooling alone is insufficient (8\% prevention rate), requiring the layered Slither--Mythril--Foundry--Tenderly pipeline in Section~\ref{sec:defense-in-depth}. This grounds Contribution~3 and RQ3.

\textbf{(3) The institutional and regulatory context.} MiCA~[R36] and the GENIUS Act~[R38] mandate stablecoin reserve transparency; mBridge~[R39] and Agora~[R40] provide settlement rails without lending hierarchies; Ozili~[161] documents CBDC adoption failures (eNaira 98.5\% inactivity); FundsChain~[39] and Kinexys~[40] confirm institutional blockchain momentum. Stablecoin-first, institution-intermediated design is therefore both regulatorily defensible and empirically necessary---answering RQ2.

\textbf{(4) Scoped future directions.} GNN relational fraud analysis~[R24, 121, R25], cross-tier federated learning~[49, R27, 125], Groth16 zkKYC/zkAML circuits~[R8, 153, R19], \texttt{NettingEngine} settlement, Certora reserve-invariant proofs, and retail payment rails~[136--145] are each studyable in the literature but explicitly scoped to Future Work (Section~\ref{sec:future-work}) or post-MVT deliverables (Section~\ref{sec:future-retail-payments}); none shapes the current MVT build.

\section{Summary of Key Findings}

The literature review establishes why the CWB requires a four-tier architecture, an oracle-mediated RF + Isolation Forest + SHAP pipeline, multi-directional capital-flow modules, layered smart-contract security, and stablecoin-first denomination---while deferring GNN, federated learning, ZKP circuit deployment, and retail payment rails to explicitly scoped future work.
...
Evaluators should distinguish between what is \textbf{specified} in this document (formal design), what is \textbf{planned} for the implementation phases of the final thesis, and what is \textbf{deferred Future Work} (e.g.\ bank-authority agent sessions). Nothing in the pre-thesis submission is deployed to a public network; Table~\ref{tab:prototype-scope} records design status only.
...
ZKP KYC compliance layer & \tmarkPlanned{} Future Work (circuit specified; Section~\ref{sec:future-work}) \\
\rowcolor{RowShade}
Testnet deployment (Polygon zkEVM Cardona) & \tmarkPlanned{} Planned (Phases~I--IV on Cardona; manifest Phase~IV) \\
AI Agent / MCP tool server (17 banking tools) & \tmarkPlanned{} Planned (Phase~III Must; \textbf{retail client only}) \\
\rowcolor{RowShade}
Bank-authority AI agent (NB/WB aggregate read tools) & \tmarkPlanned{} Future Work (post-MVT) \\
EIP-7702 session key management & \tmarkPlanned{} Planned (Phase~III Must) \\
...
\texttt{agent\_action\_log} table schema & \tmarkPlanned{} Planned (Phase~II--III) \\
\rowcolor{RowShade}
...
The remainder of this chapter is organised as follows. Sections~\ref{sec:high-level-arch}--\ref{sec:oracle-architecture} describe the platform stack, contract inventory, blockchain selection, and oracle architecture. Sections~\ref{sec:data-model}--\ref{sec:data-partitioning} present the relational data model and on-chain/off-chain partitioning. Sections~\ref{sec:operating-context}--\ref{sec:funnel} cover operating context, identity, onboarding, and the retail conversion funnel. Sections~\ref{sec:kinked-rate}--\ref{sec:bridge} specify interest-rate, liquidation, deposit, credit-passport, and single-chain deployment designs. Section~\ref{sec:multi-entity} details multi-entity capital operations. Sections~\ref{sec:system-modeling}--\ref{sec:banking-products} provide UML-style models and the banking product suite. Sections~\ref{sec:governance-framework}--\ref{sec:threat-model} close with governance and the five-layer security architecture.

\paragraph{Specification voice.}
Throughout this chapter, \textbf{specified} denotes a formally documented design in this pre-thesis; \textbf{planned} denotes a Phase~I--IV implementation deliverable. The conversational agent is a \textbf{planned Phase~III Must deliverable} for \textbf{retail clients only}---it mirrors the same Express.js API and smart contracts as the primary React interface and does not extend to National Bank or World Bank authority workflows (bank-authority agent sessions remain Future Work; Table~\ref{tab:prototype-scope}). Architectural descriptions use the present tense for specification clarity; Table~\ref{tab:prototype-scope} takes precedence when scope could be ambiguous.
...
\caption[Component diagram (planning phase)]{Component diagram (planning phase): React UI is the primary path; MCP agent (Phase~III, dashed) shares the Express.js API; smart contracts and Chainlink integrations are specified for Phase~I--IV deployment.}
...
\begin{figure}[H]
\centering
\resizebox{!}{0.68\textheight}{\ThesisIncludeGraphics{fig-blockchain-stack}}
\caption[Layered blockchain and application stack]{Layered blockchain and application stack: L1 settlement (Polygon~zkEVM Cardona for all live contracts; Ethereum~Sepolia for Certora verification only); L2 smart-contract platform (Solidity 0.8.20, OpenZeppelin~v5, UUPS, TimeLock, RBAC); L3 data (PostgreSQL~16 in 3NF, Redis, IPFS); L4 API and services (Express, FastAPI, WebSocket, EIP-712); L5 presentation (React + ERC-4337 account abstraction).}
...
Network & Polygon zkEVM Cardona (live); Ethereum Sepolia (Certora only, Phase~IV) & Zero-cost deployment; single-chain live architecture; Sepolia retained only for formal verification, not bridged assets. \\
...
\caption[Core system entity graph]{Core system graph showing entity relationships across the four-tier banking hierarchy (\texttt{INSTITUTION} $\to$ \texttt{WORLD\_BANK} / \texttt{NATIONAL\_BANK} / \texttt{LOCAL\_BANK} $\to$ \texttt{BANK\_USER}), the central \texttt{BORROWER} entity, and the lending-lifecycle sub-graph (\texttt{LOAN\_REQUEST}, \texttt{LOAN}, \texttt{INSTALLMENT}, \texttt{TRANSACTION}, \texttt{INCOME\_PROOF}, \texttt{CHAT\_MESSAGE}, \texttt{LOAN\_RISK\_ASSESSMENT}, \texttt{CREDIT\_PASSPORT}).}
...
\caption[Extended ERD entities]{Extended ERD entities: the banking-product entities (\texttt{SAVINGS\_ACCOUNT}, \texttt{FIXED\_DEPOSIT}, \texttt{CURRENT\_ACCOUNT}, \texttt{LOAN\_GROUP}, \texttt{GROUP\_MEMBER}, \texttt{GROUP\_CONSENT}, \texttt{INSURANCE\_FUND}) and the multi-entity / cross-tier operational entities (\texttt{INTERBANK\_LOAN}, \texttt{UPWARD\_DEPOSIT}, \texttt{SYNDICATE}, \texttt{SYNDICATE\_MEMBER}, \texttt{TRANCHED\_POOL}, \texttt{TRANCHED\_POOL\_SUBSCRIPTION}, \texttt{TREASURY\_SWAP}, \texttt{NETTING\_BATCH}, \texttt{NETTING\_ENTRY}, \texttt{NETTING\_COORDINATOR\_STATE}). Foreign-key relationships into the core entities of Figure~\ref{fig:core-system-graph} are preserved.}
...
\caption[Enhanced EER model]{Enhanced Entity-Relationship (EER) model showing generalization / specialization (\texttt{INSTITUTION} $\to$ \texttt{WORLD\_BANK} / \texttt{NATIONAL\_BANK} / \texttt{LOCAL\_BANK}; \texttt{BANK\_USER} $\to$ World-/National-/Local-bank-admin subtypes), the weak entity \texttt{INSTALLMENT} identified by its parent \texttt{LOAN}, the multi-valued attribute \texttt{INCOME\_PROOF}, and participation constraints (total / partial).}
\label{fig:eer}
\end{figure}
...
\caption[First normal form (1NF) demonstration]{First normal form (1NF): multi-valued income-proof attributes are extracted from \texttt{BORROWER} into a separate \texttt{INCOME\_PROOF} entity with a foreign-key link (1:N).}
\label{fig:1nf-demo}
\end{figure}
...
\caption[Second normal form (2NF) demonstration]{Second normal form (2NF): partial dependency on \texttt{loan\_id} alone is removed by decomposing borrower attributes into \texttt{LOAN}; \texttt{INSTALLMENT} non-key columns depend on the full composite key.}
\label{fig:2nf-demo}
\end{figure}
...
\caption[Third normal form (3NF) demonstration]{Third normal form (3NF): transitive dependency of interest-rate parameters on \texttt{bank\_tier\_type} is removed by extracting \texttt{INTEREST\_RATE\_TIER}.}
\label{fig:3nf-demo}
\end{figure}
...
\caption[BCNF verification on INTEREST\_RATE\_TIER]{Boyce--Codd normal form (BCNF): after extraction, \texttt{tier\_id} is the determinant of all non-trivial functional dependencies and is a superkey.}
\label{fig:bcnf-demo}
\end{figure}
...
The Crypto World Bank is specified as a \textbf{blockchain-based institutional finance platform} on EVM Layer~2, not as a geographic pilot for a single rural market. The primary contribution is programmable hierarchical banking---reserve enforcement, tiered capital flow, on-chain auditability, and modular lending products---deployable on Polygon zkEVM Cardona (live) with Ethereum Sepolia retained only for Certora formal verification in Phase~IV.
...
\noindent\textit{Future Work.} A two-circuit privacy-preserving compliance gateway---Groth16 zkKYC (Circom~2.0), zkAML velocity proofs~[R19], and W3C DID/VC anchoring---is motivated by Contribution~4 (Section~\ref{sec:research-contribution}) and the literature synthesis in Section~\ref{sec:lit-zkp-defi}. Circuit-level design, verifier deployment, and on-chain integration are deferred to Section~\ref{sec:future-work}. At MVT scope, retail and institutional onboarding uses tiered off-chain KYC, hash-linked credentials in PostgreSQL, and on-chain RBAC role gates (Section~\ref{sec:tiered-kyc}).
...
\caption[Actor permission matrix (primary actions)]{Actor permission matrix (primary actions): clients submit loans and pay installments via the React UI or the MCP agent (Phase~III); Local Bank approvers approve and freeze accounts; National and World Bank admins set rates and reserve parameters; the AI Agent may call read tools freely and write tools only after human confirmation (\dag).}
...
\caption[Conversational agent pipeline (Phase III MVT)]{Conversational agent pipeline (Phase~III Must deliverable): six-step pipeline with on-chain context injection, read/write routing, mandatory human confirmation gate, MCP write-tool execution, and \texttt{AGENT\_ACTION\_LOG} audit trail. React dashboard remains the co-primary interface.}
...
An initial design considered multi-chain deployment bridging Polygon zkEVM Cardona and Ethereum Sepolia to distribute retail and institutional traffic across separate networks. This approach was revised after security analysis identified cross-chain bridges as the highest-risk attack surface in DeFi---accounting for over \$1.1~billion in documented losses through Ronin (\$625M), Wormhole (\$320M), and Nomad (\$190M) exploits in 2022 alone. The revised architecture consolidates \textbf{all live contract deployment on Polygon zkEVM Cardona}. Polygon zkEVM derives settlement finality from ZK validity proofs verified on Ethereum L1, eliminating the need for a separate Ethereum contract layer and the bridge required to connect them.

Ethereum Sepolia is retained in the testing plan \textbf{exclusively as a Certora formal-verification target} (Phase~IV), with no live asset bridge between networks. This simplification reduces attack surface and keeps Phases~I--II deliverable within the project timeline. Chen et al.~[46] validate multi-chain lending feasibility in the literature; the CWB adopts their throughput lessons without deploying a live CCIP or LayerZero bridge. Credit Passport SBT state, borrowing-limit enforcement, and loan lifecycle data remain single-chain on Cardona.
...
\caption[Use-case diagram (nine-actor taxonomy)]{Use-case diagram reflecting the nine-actor taxonomy of Table~\ref{tab:actor-taxonomy}: primary actors (A1--A5, top), external actors (A6--A9, top), and eleven grouped use cases inside the system boundary. Shaded rows denote multi-entity operations (UC5--UC8); dashed nodes and links denote the MCP agent path (A5, UC10), a Phase~III Must deliverable.}
\label{fig:usecase}
\end{figure}
...
\caption[Client banking session activity flow]{Client banking session activity flow: authenticated session bootstrap with Chainlink price prefetch, then channel routing to the React dashboard path, human WebSocket chat, or the MCP agent path (Phase~III; confirmation gate and \texttt{AGENT\_ACTION\_LOG} audit before session close).}
\label{fig:act-aux}
...
\caption[Sequence diagrams]{Sequence diagrams: (a) client--bank chat over an authenticated WebSocket, and (b) MCP agent pipeline (on-chain context injection, read/write routing, human confirmation gate, EIP-7702 session-key signing; Phase~III Must deliverable).}
...
\textbf{Checks-Effects-Interactions (CEI) pattern.} The CEI pattern is applied to all state-mutating functions in the lending contracts. The three functions most vulnerable to reentrancy are: (1)~\texttt{disburseLoan} --- update \texttt{loanStatus} before any ETH transfer and apply \texttt{nonReentrant}; (2)~\texttt{processInstallment} --- check $\to$ mark paid $\to$ emit event $\to$ release interest; (3)~\texttt{allocateCapital} --- apply \texttt{nonReentrant} and allocation bounds before any state change. The DAO hack (2016) and Curve Finance exploit (2023) demonstrate that reentrancy remains an active threat~[R6]; Slither static analysis and Mythril symbolic execution are planned for the final thesis security audit. Certora formal verification of reserve invariants is scoped to Future Work (Section~\ref{sec:future-work}).
...
MCP conversational agent & Must (Ph.~III) & Qwen3-8B + 17-tool MCP server; alternate path to same Express.js API \\
...
MCP conversational agent & \textbf{Primary (C3 ext.)} & Must deploy in Phase~III: 17-tool MCP + chat UI + confirmation gate; one write operation demonstrated E2E (MVT item~11) \\
...
11 & MCP conversational agent: 17-tool server + chat UI + human confirmation gate & III & Screen recording: client submits loan via agent; \texttt{AGENT\_ACTION\_LOG} row + confirmation turn recorded; write blocked without consent \\
12 & LLM evaluation protocol (task accuracy, action accuracy, zero bypass) & IV & Section~\ref{sec:llm-eval} metrics reported; 100\% human-gate compliance on red-team set \\
\multicolumn{4}{l}{\textit{Optional (not graded for MVT pass): GNN ablation, Certora proof, zkAML, CCIP bridge.}} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Pre-thesis vs.\ final thesis.} At \textbf{pre-thesis submission}, items 1--8 and 11 may remain \textit{specified} rather than \textit{deployed}; this report satisfies that stage by providing contract interfaces, the ML workload plan (Section~\ref{sec:ml-workload}), agent pipeline specification (Section~\ref{sec:optional-agent}), diagrams, and the checklist above. At \textbf{final thesis submission}, items 1--8 and 11 are \textbf{Must} for a defensible pass; items 9--10 and 12 are \textbf{Should}.
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-agile-process}
...
\begin{figure}[H]
\centering
\BalancedDiagram{fig-aiml-pipeline}
...
obligations applying from August~2026~[126]---a deadline that falls
...
\begin{figure}[H]
\centering
\ActivityDiagram{fig-realtime-dashboard}
...
\begin{figure}[H]
\centering
\BalancedDiagram{Ch4_loan-lifecycle-state-machine}
...
\item \textbf{RQ3 (analytics support):} model evaluation using standard metrics (precision, recall, F1, AUC) plus explainability completeness (SHAP coverage, anomaly deviation reports, Authority Brief audit rows in \texttt{LOAN\_RISK\_ASSESSMENT}) is reported in two explicitly separated phases. \textit{Phase~(a) --- synthetic-data evaluation:} model performance on the procedurally-generated synthetic dataset (documented in Section~\ref{sec:abm-sim}), labeled ``proof-of-concept evaluation only.'' These metrics reflect the upper-bound performance achievable given the synthetic distribution and do not make claims about real-world fraud detection. The stacking meta-learner weights are hardcoded initial placeholder values (approximately 0.7 for $p_f$, 0.3 for the anomaly signal) pending real labeled data; when no labeled validation set is available, the system degrades gracefully to a calibrated Random Forest alone. \textit{Phase~(b) --- real-data evaluation:} deferred to future work once actual user transaction data is available. GNN ablation and federated learning are likewise Future Work (Section~\ref{sec:future-work}).
...
DT-I.12 & Must & Full 51-entity database schema migration (31 core + 6 extension + 14 stubs) --- all tables including SESSIONS, AGENT\_ACTION\_LOG (with \texttt{injection\_scan\_flag}, \texttt{toolset\_name}), and banking-product / multi-entity stubs & 4 \\
...
\vspace{2pt}
\noindent\textit{\small Phase~I establishes the full 51-entity schema (31 core tables created; extension and stub tables documented with PK/FK anchors) at schema-creation time and delivers all Must-have infrastructure before banking features are built on top.}
...
Isolation Forest, and SHAP pipeline (core blockchain risk gate), and implement the conversational agent harness (MCP tool server, chat UI, confirmation gate, EIP-7702 session keys).
...
DT-III.03 & Must & MCP tool server --- 17 banking tools wired to Express.js API & 8 \\
DT-III.04 & Must & Agent chat interface (Qwen3-8B + tool calling + human-gate) & 8 \\
DT-III.05 & Must & EIP-7702 session key management (scope enforcement + TTL) & 5 \\
...
DT-III.10 & Should & The Graph subgraph + WebSocket event listener for Reserve Transparency Dashboard & 4 \\
DT-III.11 & Should & SAR workflow: Isolation Forest $\to$ aml-alert $\to$ \texttt{freezeAccount}; compliance review queue integration & 4 \\
DT-III.12 & Should & Reserve Transparency Dashboard: live queries to The Graph subgraph & 3 \\
...
300-client simulation, testnet deployment, and the LLM agent evaluation protocol; finalise the paper.
...
III & AI/ML oracle pipeline + conversational agent & 10--13 & DT-III.01--14 & Chainlink Functions, ML pipeline, MCP agent, dashboard \\
...
\begin{figure}[H]
\centering
\ThesisIncludeGraphics[width=0.75\linewidth,height=0.54\textheight,keepaspectratio]{fig-optional-agent-addon}
\caption[Conversational agent architecture]{Conversational agent (Phase~III Must): React UI and MCP agent are co-primary paths; both call the same Express.js API and smart contracts.}
...
\midrule
G0 & Week~0 (kickoff) & Ch.~3 architecture PDF; ERD; phase plan (this section); empty monorepo with README run instructions & Supervisor confirms phase boundaries and single-chain Cardona target \\
G1 & End Week~4 & Cardona-deployed World/National/Local Bank contracts; WalletConnect demo; PostgreSQL migration showing 51-entity schema; Foundry/Hardhat RBAC tests & MVT~\#1; DT-I.01--04, I.12--15 complete \\
G2 & End Week~9 & Screen recording: loan request $\to$ approver approve $\to$ disbursement $\to$ one installment; reserve-ratio revert test; SavingsVault or GroupLendingPool minimal PoC & MVT~\#2--3; core Phase~II contracts on Cardona \\
G3 & End Week~13 & FastAPI \texttt{/v1/score} live; Authority Brief on approver dashboard; Chainlink Functions score commit before \texttt{approveLoan}; agent chat submits loan with confirmation gate & MVT~\#4--8, \#11; DT-III.01--07 complete \\
...
\item \textbf{Track B --- Database:} DT-I.12--13 (51-entity migration, indexes, FK constraints). Deliverable: \texttt{prisma migrate} or SQL dump; ERD screenshot matching Ch.~3 entity count.
\item \textbf{Track C --- DevOps:} DT-I.11 (Cardona RPC, chain ID, faucet script). Deliverable: \texttt{/deployments/cardona/} folder stub and deploy script dry-run.
\end{enumerate}

\paragraph{Weeks 3--4 (integration + first deploy).}
\begin{enumerate}[noitemsep]
\item DT-I.14--15: React shell + WalletConnect; show connected address and network switch to Cardona.
\item DT-I.06--09 (Should): InsuranceFund, reserve views, Chainlink PoR --- implement if schedule allows; not blocking G1.
\item \textbf{Cardona deploy:} publish contract addresses to Appendix~C manifest template; verify on block explorer.
\end{enumerate}
...
\begin{enumerate}[noitemsep]
\item DT-III.02: train RF + IF on BCCC; export \texttt{.pkl}; FastAPI \texttt{/v1/score} and \texttt{/v1/brief}.
\item Wire Express loan submission to inference service; populate \texttt{LOAN\_RISK\_ASSESSMENT} (MVT~\#6).
\item DT-III.01: Chainlink Functions \texttt{commitRiskScore}; enforce \texttt{SCORE\_REVEALED} before \texttt{approveLoan} (MVT~\#7).
\item Populate BCCC benchmark table (MVT~\#8).
\end{enumerate}

\paragraph{Week 12 (MCP server).}
\begin{enumerate}[noitemsep]
\item DT-III.03: 17-tool MCP server wired to Express.js API (read tools first, then write tools behind confirmation hook).
...
\begin{enumerate}[noitemsep]
\item DT-III.04--05: Qwen3-8B chat UI; EIP-7702 session keys with scope + TTL.
...
\begin{enumerate}[noitemsep]
\item DT-IV.07: thesis consistency pass; populate Appendix~C manifest with all Cardona addresses.
\item DT-IV.03 (Should): Certora CVL specs on Sepolia --- no bridged assets (Section~\ref{sec:bridge}).
...
Agent session auth & EIP-7702 session keys & ERC-4337 paymasters & Key-level scope; Phase~III Must \\
...
\begin{figure}[H]
\centering
\BalancedDiagram{Ch4_design-decisions-alternatives}
\caption[Key design decisions and alternatives considered]{Key design decisions and alternatives considered: smart-contract platform (EVM vs.\ Cosmos / Solana), chain choice (single Polygon zkEVM Cardona vs.\ multi-chain vs.\ L1-only), upgradeability (UUPS vs.\ Transparent Proxy vs.\ immutable), identity (DID/VC + zkKYC vs.\ raw on-chain KYC vs.\ off-chain-only), ML oracle (commit-reveal vs.\ trusted-backend vs.\ on-chain ML), and cross-chain bridge (considered then retired per Section~\ref{sec:bridge}).}
...
\item \textbf{EVM (Solidity)} is regarded as the most thoroughly examined smart contract system. Over \$100~billion is secured across various blockchains by it. Building and testing the project without incurring costs is possible on Polygon zkEVM Cardona (all live contracts) and Ethereum Sepolia (Certora formal verification only, Phase~IV); Amoy PoS is retained as a documented fallback. Security tools provided by OpenZeppelin have been thoroughly tested to ensure the protection of contracts.
...
Blockchain deployment & Feasible (planned) & Polygon zkEVM Cardona (Phases~I--IV live testnet); Ethereum Sepolia (Certora formal verification only, Phase~IV; no bridged assets) \\
...
\caption[Annual revenue projection (10-year maturity)]{Annual revenue projection at full 10-year deployment maturity (USD millions, spread-based accounting on a fixed \$1.72B USDC loan base). This represents the theoretical capacity ceiling at full institutional scale; see the break-even analysis above for the 3-year adoption ramp toward this target, with break-even occurring at approximately 300~active loans.}
...
\item \textbf{Transaction cost reduction:} On Polygon zkEVM Cardona, transaction fees remain below one cent---a reduction of over \textbf{99\%} relative to the average \$42 cost of a correspondent banking transaction~[25]. This cost compression makes financially viable a large class of micro-transactions and small-loan servicing operations that are currently uneconomical through traditional payment rails.
...
Score-at-origination-only mitigates latency/cost; oracle collusion and model staleness remain open (Section~\ref{sec:ml-workload}). Certora reserve-invariant proofs are Future Work (Section~\ref{sec:future-work}); Foundry fuzz tests should list what is asserted, assumed, and out of scope~[127].
...
This thesis makes four original contributions, as formalized in Section~\ref{sec:research-contribution}: (1) a \textit{compound} four-tier hierarchical DeFi architecture (reserve-ratio enforcement + explainable oracle ML + programmable mutual liability) that mirrors multilateral development finance capital flows---partial counterexamples exist on individual axes but not in combination (Section~\ref{sec:research-limitations}); (2) an on-chain \textit{programmable joint-liability} group lending specification with over-indebtedness controls (Section~\ref{sec:over-indebt}), honestly scoped against analogue solidarity-lending limits; (3) an oracle-mediated AI/ML integration pattern with SHAP- and anomaly-report explainability for fraud and AML governance (Section~\ref{sec:ml-explainability}), committed on-chain via Chainlink Functions at loan origination, extended by a Phase~III MCP conversational agent (Section~\ref{sec:optional-agent}), with BCCC benchmarking subject to an acknowledged domain-transfer gap (Section~\ref{sec:ml-workload}); and (4) a compliance-aware identity pathway (zkKYC, zkAML, W3C DID/VC) motivated in the literature and scoped to Future Work (Section~\ref{sec:future-work}). GNN ablation, cross-tier federated learning, \texttt{NettingEngine}, and Certora proofs remain outside the MVT minimum. These contributions are framed against the institutional-DeFi landscape (Sygnum, mBridge, Agora) in Section~\ref{sec:lit-institutional} and against the MiCA / GENIUS Act / EU AI Act frontier in Section~\ref{sec:mica-compliance}.
...
\item \textbf{Conversational agent (Phase~III MVT).} MCP tool server (17 tools), chat UI, human confirmation gate, and one E2E write demonstration (Section~\ref{sec:optional-agent}; MVT item~11); LLM evaluation protocol (Section~\ref{sec:llm-eval}; MVT item~12).
...
Isolate contribution of hierarchical tier-relationship edges vs.\ flat wallet edges on BCCC~[108]; target AFT or IEEE Blockchain.
\item \textbf{Federated learning across banking tiers.}\label{sec:fl-fraud}
FedAvg + differential privacy across Local Banks; benchmark against FedFraud~[125] and BCCC; target IEEE TIFS.
...
\item \textbf{Certora formal verification of reserve invariants (Phase~IV).}\label{sec:certora}
CVL proofs that \(\mathrm{totalReserve} \ge \mathrm{minReserveRatio} \cdot \mathrm{totalDeposited}\) and that downstream allocation never exceeds available reserve; target FMBC~[127].
...
sectors handling sensitive financial records~[R56].
...
\caption[AI banking agent request path]{AI banking agent request path (Phase~III): browser UI $\to$ Vite dev proxy $\to$ CWB Express API (SSE) $\to$ MCP tool server (17 banking tools) $\to$ Qwen3-8B inference $\to$ human confirmation gate $\to$ write-tool execution $\to$ on-chain transaction.}
...
\caption[AI agent data flow]{AI agent data flow (Phase~III): the browser UI streams from the CWB Express API; read-tool requests return immediately; write-tool requests pass through the human confirmation gate and are signed with the EIP-7702 session key before on-chain execution.}
...
Target Network & Polygon zkEVM Cardona (single-chain live) & ZK validity proof security; Ethereum Sepolia = Certora only (Phase~IV); Amoy PoS = fallback \\
...
The following table is the \textbf{consolidated planned deployment manifest template} for Polygon zkEVM Cardona testnet. Incremental contract deployments begin in Phase~I on Cardona (Table~\ref{tab:phase-deliverables}); Ethereum Sepolia is used only for Certora formal verification in Phase~IV with no bridged assets (Section~\ref{sec:bridge}). No contracts are claimed as live-deployed at pre-thesis submission. Addresses will be recorded in the project repository under \texttt{/deployments/cardona/} as phases complete and can be verified on the Cardona block explorer at \url{https://explorer-ui.cardona.zkevm-rpc.com}.

\begin{table}[H]
\centering
\caption[Planned Cardona testnet deployment manifest]{Planned Cardona testnet deployment manifest (consolidated Phase~IV template; incremental Phase~I--III deploys on single-chain Cardona).}
...
\item BCOLBD. 2025. Blockchain Olympiad Bangladesh: Guideline and Evaluation Scheme. \url{https://bcolbd.org/rules}
