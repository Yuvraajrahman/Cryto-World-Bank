...
\begin{table}[H]
\centering
\caption[Prototype scope and implementation status]{Prototype scope: specification vs.\ planned implementation (pre-thesis design record). \textit{Voice:} \textit{specified} = documented design; \textit{planned} = Phase~I--IV deliverable. Target chain: Ethereum Sepolia (primary) and/or Polygon Amoy (secondary).}
...
Table~\ref{tab:functional-requirements} and Table~\ref{tab:nonfunctional-requirements} consolidate the functional and non-functional requirements that drive the architecture in this chapter and the implementation plan in Chapter~\ref{ch:methodology}. They subsume scattered capability rows from Table~\ref{tab:prototype-scope} and the MVT checklist (Chapter~\ref{ch:methodology}) into a single requirements view suitable for traceability (Section~\ref{sec:traceability}).

\begin{table}[H]
\centering
\caption[Functional requirements (representative)]{Functional requirements (representative subset). Full module inventory remains in Table~\ref{tab:prototype-scope}.}
...
7 & Commit-reveal score commit before approval & III & Contract in \texttt{SCORE\_REVEALED} before \texttt{approveLoan} succeeds \\
8 & BCCC-DeFiFraudTrans benchmark table (precision, recall, F1, AUC) & II--III & Table~\ref{tab:ml-bccc-results} + Figure~\ref{fig:ml-eval}: 50k-address subsample; full corpus retrain deferred \\
9 & 300-client Foundry simulation OR documented gas-cost table & IV & Local Anvil/Hardhat run with deterministic seed; gas table + manifest in Appendix~C; live testnet demo optional (5--10 clients) \\
...
13 & Slither + Mythril audit reports on World Bank / National Bank / Local Bank contracts & IV & Reports archived in Appendix~B; zero unresolved Slither high-severity findings \\
14 & Foundry fuzz + invariant suite on reentrancy-critical functions (\texttt{disburseLoan}, \texttt{processInstallment}, \texttt{allocateCapital}) & II--IV & \texttt{forge test} fuzz + invariant suite green in CI; gas table reproducible \\
15 & 2-of-3 Safe multisig holding \texttt{WorldBankAdmin} before first inspectable testnet deployment & I & Safe address in Appendix~C manifest; privileged calls route through Safe, not an EOA \\
16 & Tenderly alert rules live for 3 demo-critical triggers (large disbursement, reserve-ratio drop, governance pause) & III--IV (Should) & Alert fires on a recorded test transaction; screenshot/log archived in Appendix~C \\
\multicolumn{4}{l}{\textit{Optional (not graded for MVT pass): GNN ablation, Certora proof, Echidna fuzzing, Halmos verification, zkAML, CCIP bridge, full 6-trigger Tenderly set, hardware-wallet signer, key-rotation schedule.}} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Pre-thesis vs.\ final thesis.} At \textbf{pre-thesis submission}, items 1--8 and 11 may remain \textit{specified} rather than \textit{deployed}; this report satisfies that stage by providing contract interfaces, the ML workload plan (Section~\ref{sec:ml-workload}), agent pipeline specification (Section~\ref{sec:optional-agent}), diagrams, and the checklist above. At \textbf{final thesis submission}, items 1--8, 11, 13, 14, and 15 are \textbf{Must} for a defensible pass; items 9--10, 12, and 16 are \textbf{Should}. Figure~\ref{fig:mvt-status} summarises which MVT items are satisfied at pre-thesis versus deferred to the final thesis.
...
Table~\ref{tab:process-model-comparison} records why \textbf{Agile/Scrum within an incremental four-phase plan} was chosen over rigid Waterfall or pure Spiral models for this academic prototype. The decision follows Software Engineering process-model criteria: requirements stability, risk profile, and need for demonstrable increments at supervisor gates.
...
\subsection{Model--View--Controller and Version Control}
\label{sec:mvc-pattern}

The repository root follows a conventional \textbf{Model--View--Controller (MVC)} separation taught in Software Engineering:

\begin{itemize}[noitemsep]
\item \textbf{View} --- \texttt{frontend/}: React components render loan forms, reserve dashboards, and the Authority Brief; wallet signing is initiated from the view layer via Wagmi/RainbowKit.
\item \textbf{Controller} --- \texttt{backend/}: Express.js routes validate input (Pydantic-equivalent middleware), enforce RBAC, orchestrate ML scoring calls, and submit signed transactions to the EVM node.
\item \textbf{Model} --- \texttt{contracts/} (on-chain state machines) and PostgreSQL (off-chain projections): authoritative lending state lives on-chain; the database is an eventually consistent read model updated by the event listener.
\end{itemize}

\noindent\textbf{Version control.} The monorepo uses Git with \texttt{main} as the integration branch. Smart-contract changes are validated by \texttt{npm run test:contracts} (Hardhat) before merge; API changes by \texttt{backend/} Vitest suites. Commit hooks strip automated attribution trailers (root \texttt{scripts/install-git-hooks.sh}). Deployment addresses and phase evidence are recorded under \texttt{Documentation/Pre Thesis -2 Documents/V35 /evidence/}.
...
\noindent Measured BCCC benchmarks, explainability outputs, on-chain simulation design, smart-contract tests, and research-question evaluation criteria are reported in Chapter~\ref{ch:evaluation} (Sections~\ref{sec:eval-method}--\ref{sec:research-limitations}). This chapter retains the \textit{pipeline specification} only.
...
\multicolumn{3}{l}{\textit{Phase~I total estimated effort (Must-have):}} & \textit{30 days} \\
...
\midrule
I & Foundation and infrastructure & 1--4 & DT-I.01--17 & Contract hierarchy, \texttt{MockUSDC}, core DB schema, React shell \\
II & Core banking and on-chain lifecycle & 5--9 & DT-II.01--19 & Lending workflow, SBT limits, cron/Automation triggers \\
...
G1 & End Week~4 & Sepolia-deployed World/National/Local Bank + \texttt{MockUSDC}; WalletConnect demo; PostgreSQL \textbf{13 M1 tables} migrated (Table~\ref{tab:db-implementation-tiers}); RBAC tests; $\leq$5\,min screen recording of \texttt{allocateCapital} & MVT~\#1; DT-I.01--04, I.04a, I.12--15 complete \\
...
\item \textbf{Track C --- DevOps:} DT-I.11 (Sepolia primary RPC via Alchemy, chain IDs, faucet script). Deliverable: \texttt{/deployments/testnet/} folder stub and deploy script dry-run.
\end{enumerate}
